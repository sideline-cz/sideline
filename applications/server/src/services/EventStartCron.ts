...
// `now` drives ONLY the two deferred sweeps below (the missed-RSVP claim and
// the "Dnes" started-post claim) — it is not threaded anywhere else.
//
// The flip path deliberately stays on the Postgres clock: `findEventsToStart`
// (`start_at <= NOW()`) and `startEvent` (`SET … = now()`, the arming stamps)
// must agree with each other inside one statement, not with this parameter.
// Do not extend injection to `findEventsToStart`.
//
// Taking `now` as a parameter rather than reading a Clock inside makes
// "one instant per cycle" structural — there is no second place to sample.
// A long cycle therefore evaluates the deferred sweeps against the
// cycle-start instant; worst case a sweep slips one 60-second cycle, which
// is strictly better than today's two independent samples.
//
// It also keeps `effect/testing/TestClock` out of the integration suite,
// which matters: a `SqlSchema` call ahead of an `Effect.sleep` under a
// virtual clock deadlocks — see
// `test/integration/services/BankSyncPoller.test.ts:536-546` and
// `test/integration/services/FioApiClient.test.ts:312-326`.
export const makeEventStartCronEffect = (now: Date) =>
  Effect.Do.pipe(
    ...
    Effect.tap(({ eventsRepo, eventsRsvpsRepo }) =>
      eventsRepo.findAllDayEventsPastLastLocalDay(now).pipe(
    ...
    Effect.tap(({ eventsRepo, syncRepo }) =>
      eventsRepo.findAllDayEventsNeedingStartedPost(now).pipe(
...
// Production entry point: one `new Date()` per run. `Effect.suspend` is
// load-bearing here — without it the instant would freeze at module-load
// time instead of being sampled fresh on every invocation.
