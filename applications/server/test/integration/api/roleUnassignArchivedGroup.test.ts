...
//
// Expected: `role_unassigned` IS emitted (a `role_sync_events` row is written) and the
// `role_removed` notification IS created, because an archived group no longer grants —
// M genuinely lost R when the direct grant was deleted. Before the fix, the unmigrated
// "still held?" query saw R through the archived group (it didn't filter `is_archived`)
// and suppressed the emit, so Sideline reported "role removed" while Discord silently
// kept the role forever.
...
describe('role.ts unassignRole — archived-group guard, end to end', () => {
  it('emits role_unassigned when the only remaining grant is through an ARCHIVED group', async () => {
    ...
    // Before the fix, the unmigrated "still held effectively?" re-check saw the role
    // through the archived group and suppressed this — Sideline said "removed" but
    // Discord kept the role forever.
