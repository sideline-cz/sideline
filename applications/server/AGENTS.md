...
4. **A dead token answers with a bodyless HTTP 500 — there is no 401 and no 403.** A blanket retry-on-5xx therefore hammers Fio forever on a revoked or expired token, so `FioServerError` is NEVER retried; `409` (`FioRateLimited`) is the only retried status. Every Fio error body is empty (`content-length: 0`) — dispatch on the status code alone and never parse an error body.
