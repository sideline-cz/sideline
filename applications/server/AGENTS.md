...
4. **A team's four built-in roles (`roles.is_built_in` — Admin / Captain / Player / Treasurer) are NEVER mirrored into Discord, in either direction.** They describe Sideline permissions, not guild membership. The enforcement point is `RoleSyncEventsRepository`'s `findSyncableRoleIds`, spliced into BOTH statements that write `role_sync_events` (`_emitIfGuildLinked` and `emitRoleEventsBatch`), so a built-in role drops out exactly the way an unlinked team already does: nothing written, no error, every emit call site keeps its best-effort shape. It is deliberately NOT a check at the seven emit call sites — `api/role.ts`'s `createRole` / `deleteRole` / `assignRole` / `unassignRole` plus the three diff utils — so that an eighth inherits the exclusion for free. Of those seven, only `assignRole`, `unassignRole` and the three diff utils can carry a built-in role today: `createRole` emits for a row `RolesRepository.insertRole` just wrote with `is_built_in = false`, and `deleteRole` fails `RoleApi.CannotModifyBuiltIn` (`api/role.ts:197`) before it reaches its emit. That is a property of today's call sites, not an invariant — never delete the writer's guard because the reachable callers happen to be clean. Match on `is_built_in`, never on the four names: a soft-deleted (`is_archived = true`) custom role must still emit `role_deleted`, and a team's own custom role must stay syncable even if it shares a name. Integration coverage: `test/integration/repositories/RoleSyncEventsRepository.test.ts` → "built-in roles never reach role_sync_events" (both single-event directions, a same-team custom role still emitting, a mixed batch, an all-built-in batch — which must not fall through to an `INSERT ... VALUES` with an empty list — and an archived custom role's `role_deleted`).

   **The three diff utils ALSO drop built-in roles from their `desired` / snapshot sets, and that is not redundant with the writer's guard.** The guard alone would let built-in roles consume the fan-out budgets before being discarded: `reconcileMemberDiscordRoles`'s `toAssign` re-selects a role with no `discord_role_mappings` row on EVERY pass (rule 3), so on every pass built-in roles would consume that member's `MAX_ROLE_SYNC_EMISSIONS_PER_MEMBER` slots and, on a `Guild/ReconcileMembers` sweep, the `MAX_ROLE_SYNC_EMISSIONS_PER_GUILD_RECONCILE` budget `reserveFromBudget` shares across every member of that sweep — starving the members processed after them; `syncGroupRoleMembers` would spend `MAX_ROLE_SYNC_EMISSIONS_PER_GROUP_OPERATION` slots on a built-in role attached to a group via `role_groups` (fanned out across every member of that group and its subgroups); and `syncMemberDiscordRoles` would report an `addedCount` it never enqueued, leaving a member whose only effective role is `Player` reading "queued 1" forever. `findEffectiveRoleIdsForMember` / `findEffectiveRolesForMembers` project `is_built_in` for exactly this; `syncGroupRoleMembers` filters inside `rolesByMember`, which both the BEFORE and the AFTER snapshot funnel through, so the diff stays symmetric and a built-in role can never surface as a gain or a loss. **In `reconcileMemberDiscordRoles` and `syncMemberDiscordRoles` this covers the ASSIGN side only**: their removal candidates are built from `discord_role_mappings` ∩ `member_role_grants` (rule 2), not from `desired`, so a built-in role that a pre-guard `role_assigned` already got mapped and granted now falls OUT of `desired` and becomes a removal candidate on every pass — it consumes cap slots and inflates the reported `removedCount`. Both utils therefore ALSO drop built-in roles where they resolve each candidate's name (`findRoleById` already returns `is_built_in`, so the filter is free) — but understand what that second filter is and is not: it is bookkeeping, keeping cap slots and `removedCount` honest for the existing production cohort. `findSyncableRoleIds`, not the diff, is what keeps a `role_unassigned` from reaching the bot and stripping the guild role, and it stays the only thing that may be relied on for that. Do NOT push the filter down into `effectiveRolesFrom` — that fragment is the single source of truth for PERMISSIONS, and `api/role.ts`'s `unassignRole` re-check reads the same per-member query to decide whether a role is still held.
