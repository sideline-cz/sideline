...
Reference: `LlmClient` (`src/services/LlmClient.ts`) — `make` decides between `makeStub()` (a deterministic, offline summary) and `makeReal(...)` (an OpenAI-compatible HTTP call) by the **presence of `HttpClient.HttpClient` in the layer context**, read via `Effect.serviceOption(HttpClient.HttpClient)`. Both satisfy the same `LlmClientService` interface.
...
2. **Select the provider on `HttpClient.HttpClient` presence, and inject the client into `makeReal` from that scope — never self-provide `FetchHttpClient.layer` inside the service.** `make` reads `Effect.serviceOption(HttpClient.HttpClient)`: `Option.isNone` → `makeStub()`; `Option.isSome` → `makeReal(..., httpClient)` with the client passed in as a `makeReal` argument and used via `httpClient.execute(request)`. This keeps `LlmClient.Default` self-service-free so production wires the real client explicitly (`LlmClient.Default.pipe(Layer.provide(FetchHttpClient.layer))` in `run.ts`) and tests can inject a mock `HttpClient` layer instead (rule 4). The gating env vars stay optional — `Schema.OptionFromNullishOr(Schema.RedactedFromValue(...))` for the API key, `Schemas.Optional(() => '<default>')` for the URL/model — but missing config no longer forces the stub: `makeReal` substitutes a placeholder URL/key (`'http://localhost'` / `Redacted.make('no-key-configured')`) and the request fails gracefully into the per-method fallback. Log one `Effect.logWarning` at construction when the client is absent (stub) and another when config is missing (real provider will fall back).
...
4. **Tests exercise the real provider by providing `LlmClient.Default` plus a mock `HttpClient.HttpClient` layer** — `LlmClient.Default.pipe(Layer.provide(makeMockHttpClientLayer(responseBody, status)))` where the mock is built with `HttpClient.make((request) => Effect.succeed(HttpClientResponse.fromWeb(...)))`. To exercise the stub path, provide `LlmClient.Default` with NO `HttpClient` layer (the vitest env sets `LLM_API_URL`/`LLM_API_KEY` empty so even an accidental real path cannot hit a live endpoint). Reference: `src/services/LlmClient.test.ts`. Consumers that only need a canned result may still provide a hand-rolled fake via `Layer.succeed(LlmClient, { summarizeEmail: () => Effect.succeed('...') } as any)`.
