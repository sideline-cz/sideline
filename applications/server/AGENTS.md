...
1. **The whole renumber MUST be one UPDATE statement, and its row source MUST be the `ROW_NUMBER() OVER (ORDER BY position)` CTE.** Postgres checks a non-deferrable `UNIQUE` constraint **per row, as each row is written** — never at statement end — so a mid-statement collision is entirely a function of the order the plan feeds rows to the UPDATE. This form is safe because the window's `ORDER BY position` forces `renum` to be produced ASCENDING by position and that ordered output drives the join, so a shift-down (e.g. `[0,2,3]` → `[0,1,2]`) always vacates a slot before the next row claims it. Do NOT flip the window to `ORDER BY position DESC`, do NOT feed the UPDATE from an unordered row source (a `VALUES` list, a plain table join), and do NOT split the renumber into per-row UPDATEs — each of those reaches the same final state through a row order that fails with `duplicate key value violates unique constraint`. When you cannot control the row order (a bulk rewrite of a uniquely-indexed value across many rows), drop and recreate the index around the UPDATE instead: `packages/migrations/AGENTS.md` → "A Bulk `UPDATE` That Changes A Uniquely-Indexed Value Must Drop And Recreate That Index".
2. **Keep the `AND po.position <> renum.new_pos` guard.** It only skips no-op writes (rows already at their correct position); it is an optimization, not a correctness requirement — what makes even the unguarded form safe is the ascending row order of rule 1, not the guard.
...
3. **Archiving a group severs role inheritance — keep the `is_archived = false AND team_id = ${tm}.team_id` filter inside the RECURSIVE TERM.** An archived ancestor stops the walk, so nothing above it grants either. Do NOT "simplify" this by moving the filter to the final join only: that would let an archived group's own ancestors keep granting, and it would disagree with the three other walks that MUST sever identically: `GroupsRepository.findDescendantMembersWithDiscordIdByGroupId` (decides Discord channel membership), `GroupsRepository.findActiveAncestors` (backs `getActiveAncestors`, which picks the ancestor groups `applyInviteGroup` emits `member_added` for), and `GroupsRepository.findActiveGroupsWithAncestorsForMember` (backs `emitMemberGroupChannelRoles`, the join-time group-channel-role diff — rule 6 of "`Guild/RegisterMember` and Welcome Metadata"). All three apply the same filter inside their own recursive term. Role inheritance and channel membership MUST agree — editing the severing semantics here means editing all four. Two requirements follow, and both are mandatory for any walk you ADD:
   - **A query whose semantics must match another query's must name that other query in its own header comment, at BOTH sites** — a one-way reference only warns the agent who happens to open the right file first. `findActiveAncestors` and `findActiveGroupsWithAncestorsForMember` name each other and both name `effectiveRoles.ts`; `effectiveRoles.ts` and `findDescendantMembersWithDiscordIdQuery` do not yet name each other, and should gain that comment the next time either is touched.
   - **A comment is not coverage: ship an integration test that ASSERTS the new walk returns the same group set as an existing walk over the same fixture.** Two walks drifting apart silently is exactly what caused the shipped "group roles invisible on the roster" bug. Reference: `test/integration/repositories/GroupsRepository.findActiveGroupsWithAncestorsForMember.test.ts` — it builds an archived-middle-ancestor chain and asserts the returned group set equals the set `findEffectiveRoleIdsForMember` grants roles through, rather than only documenting the intent in a comment. The `deleteGroup` handler (`src/api/group.ts`) only calls `GroupsRepository.archiveGroupById` — `UPDATE groups SET is_archived = true`, never a delete of `group_members` / `role_groups` rows — so this filter is the only thing that revokes a deleted group's grants.
4. **Archiving a ROLE revokes it everywhere — keep `AND r.is_archived = false` on BOTH `JOIN roles r` clauses inside `effectiveRolesFrom` (direct and group-inherited).** `deleteRole` (`src/api/role.ts`) is a soft delete: `RolesRepository.archiveRoleById` runs `UPDATE roles SET is_archived = true` and deletes nothing, so `member_roles`, `role_groups` and `role_permissions` rows all survive. Without this filter an archived role keeps granting its permissions through `effectivePermissionsAgg` — which is what `findMembershipQuery` feeds to every `requirePermission` guard. `deleteRole`'s `RoleInUse` guard (`api/role.ts` binds `roles.getMemberCountForRole(roleId)` and fails with `RoleInUse` when it is `> 0`; the query behind it, `RolesRepository.countMembersForRole`, splices THIS fragment and therefore counts group-inherited holders too) makes that state rare, not impossible. It is a point-in-time count and `archiveRoleById` deletes no `role_groups` row, so the likeliest path in practice is a role attached to a group that is EMPTY at delete time: the guard sees zero holders, the role is archived, and it starts granting again the moment someone joins that group. Rows predating that guard's own group-inheritance fix, a holder reachable ONLY through an archived group (`countMembersForRole` excludes those by construction), and a concurrent `assignRole` landing between the guard's read and `archiveRoleById` (`deleteRole` spans the two with no transaction and no advisory lock) each still produce it. **Put the filter in the fragment, never in a caller.** A now-removed batched query (`TeamMembersRepository`'s former `findEffectiveRolesForMembersQuery`, deleted along with the Sideline-role → Discord sync it existed for) used to bolt on its own `JOIN roles r ON r.id = er.role_id AND r.is_archived = false`; that made it the only EFFECTIVE-ROLES query that revoked an archived role — `RolesRepository.findRoleById` / `findRolesByTeamId` / `findRoleByTeamAndName` carry an `is_archived = false` predicate too, but none of them derive effective roles — so it disagreed with its own per-member sibling `findEffectiveRoleIdsForMemberQuery`. Integration coverage: `test/integration/repositories/archivedRoleGrantsNothing.test.ts` (both grant paths plus the `source: 'both'` shape, which a filter applied to only one join would still leak).
5. **`string_agg(DISTINCT x, sep ORDER BY y)` requires `y` to be a member of the `DISTINCT` argument list.** Do not reorder `effectiveRoleNamesAgg`'s `ORDER BY er.name` to e.g. `is_built_in DESC, name` — that is a Postgres syntax error, not a style choice. When an aggregate's `ORDER BY` and `DISTINCT` sets must differ, aggregate over a `SELECT DISTINCT` subquery instead.
6. **A soft-archived group must ALSO be filtered out of NON-recursive joins to `groups`, not just out of the recursive walks in rule 3.** `deleteGroup` is `UPDATE groups SET is_archived = true` on ONE row (`GroupsRepository.archiveGroupById`, `:143-145`) — it deletes no rows anywhere, and nothing cascades: descendant `groups` rows keep `is_archived = false`, and every table with a `groups(id)` foreign key keeps its rows, because `ON DELETE CASCADE` never fires on an `UPDATE`. So any query that reaches a group through a plain `JOIN groups` and then drives a Discord write, a membership change, or a captain-visible list MUST carry `is_archived = false` in that join or its `WHERE`. Reference regression: `AgeThresholdRepository.findRulesByTeamId`'s `JOIN groups g ON g.id = atr.group_id` had no filter, so `age_threshold_rules` rows pointing at an archived group stayed live and the nightly `AgeCheckCron` (`Schedule.cron('0 2 * * *')`) kept auto-assigning members INTO that archived group and emitting `member_added` for it; the fix is `JOIN groups g ON g.id = atr.group_id AND g.is_archived = false` (`:88`, inside `findByTeamIdQuery` `:81-92`). That one query has three callers — `AgeCheckService.evaluateTeam`, the `evaluateAgeThresholds` "evaluate now" endpoint, and the `listAgeThresholds` GET — so the filter also removes archived-group rules from the web list, which is the intended behaviour (a rule targeting a deleted group is not actionable). Integration coverage: `test/integration/repositories/AgeThresholdRepository.archivedGroup.test.ts`.

Reference: fragment `src/repositories/effectiveRoles.ts`; call sites `TeamMembersRepository.ts` (`findMembershipQuery`, `findMembershipByDiscordQuery`, `findByUserQuery`, `findEffectiveRoleIdsForMemberQuery`, `findRosterByTeamQuery`, `findRosterMemberQuery`, `hasOtherActiveManagerQuery`), `RostersRepository.findMemberEntries`, `RolesRepository.countMembersForRole`, `EventRsvpsRepository` (both non-responder queries), `AgeThresholdRepository.findMembersForAutoAssignmentQuery`. Integration coverage: `test/integration/repositories/effectiveRolesProvenance.test.ts`, `TeamMembersRepository.groupRoles.test.ts`, `middleGroupArchivedChain.test.ts`, `GroupsRepository.findActiveGroupsWithAncestorsForMember.test.ts` (cross-walk agreement), `archivedRoleGrantsNothing.test.ts` (archived roles grant nothing, rule 4).

### Recursive `groups.parent_id` Walks Must Carry a `depth < 32` Guard

`groups.parent_id` has NO database-level acyclicity constraint. `moveGroup`'s application-level cycle check is now atomic — it runs inside the same transaction as the `UPDATE`, under `pg_advisory_xact_lock(hashtext(teamId))`, and both `parent_id` writers reject a cross-team parent so the per-team key is sufficient (`src/api/group.ts`, `requireSameTeamParent`) — so no API path constructs a cycle any more. Rows predating that fix, and anything written by direct SQL or a future writer that forgets the lock, still can be cyclic. An unguarded `WITH RECURSIVE` walk over `groups.parent_id` then loops forever and pins a connection — and there is NO `statement_timeout` configured anywhere in this repo, so "forever" is literal.
...
1. **The guard is required even on the query that PREVENTS cycles.** `GroupsRepository.findAncestors` backs `getAncestorIds`, which is `moveGroup`'s cycle check (`src/api/group.ts`); unguarded, an already-corrupted chain would hang the one operation an operator needs to repair it. The same argument covers the walks the repair path CALLS: `moveGroup` invokes `getMemberCount` immediately after the move, so an unguarded `countMembersForGroup` would hang on exactly the data it was asked to fix.
2. **Guarded today:** the ancestor walk in `src/repositories/effectiveRoles.ts`, `GroupsRepository.findAncestors`, `GroupsRepository.findActiveAncestors`, `GroupsRepository.findDescendantMembersWithDiscordIdQuery`, `GroupsRepository.countMembersForGroup`, `GroupsRepository.findDescendantMembers`, `GroupsRepository.findActiveGroupsWithAncestorsForMemberQuery`, `GroupsRepository.findByTeamId`'s `group_tree` walk, `EventAttendanceRepository.findByEventId`, and all three `descendant_groups` walks in `EventRsvpsRepository.ts`. **Every recursive `groups.parent_id` walk in the repo now carries the bound.** Keep it that way: a new walk without one is a regression, not a gap.
3. **`PersonalEventChannelsRepository._getGuildsNeedingProvisioning` is the reason the bound matters beyond one team, and it is now guarded.** Its `Request` is `{ limit }` — no team scope — and it scans `FROM teams t` with a correlated recursive CTE per candidate, so ONE cyclic `parent_id` in ANY team hung personal-channel provisioning for EVERY guild on the instance. All four walks in that file now carry the bound. What they still do NOT carry is an `is_archived` filter, and that remains a separate, DESTRUCTIVE change: adding it deprovisions existing channels and purges `personal_event_messages`, and all four must change together or provision and deprovision disagree and fight each other every tick. A regression test for the poll must set `discord_personal_events_category_id`, not just the group id — the outer `WHERE` requires it, so a fixture that omits it is filtered out before the recursive CTE runs and the test passes with or without the guard.
4. **Writing a cycle regression test? Make sure the query actually RUNS.** Three of these tests first passed with the guard removed, each for a different reason, and a green suite said nothing. `_getGuildsNeedingProvisioning` is gated on `discord_personal_events_category_id IS NOT NULL`, so a fixture that sets only the group id is filtered out before the CTE. `findScopedTrainingTypeIds`' walk is a LATERAL joined to `role_groups`/`role_training_types`, so with those tables empty the planner never executes it. Always confirm the test goes RED with the bound removed — if it stays green, the fixture is wrong, not the guard.
5. **Adding the `is_archived` filter is a separate decision from adding `depth`, and it is NOT always safe.** `depth` is pure availability — a cycle is corrupt data, and no healthy tree reaches 32 — so it never changes behaviour. An archived filter changes WHO gets something. Decide per query, and note that in `EventsRepository.findScopedTrainingTypeIds` it **inverts**: `api/scoping.ts`'s `checkCoachScoping` treats an EMPTY allowed-set as "allow everything" (`if (Array.isArrayEmpty(allowed)) return Effect.void`), so filtering archived groups out of a coach's scope can take them from "may create type X only" to "may create ANY type". Do not add that filter there without deciding the empty-set semantics first.
6. **Where the archived filter goes is also per query — anchor vs recursive term.** In the RECURSIVE term it severs an archived subtree. In the ANCHOR it additionally means "the root group is archived, so match nothing". `GroupsRepository.findDescendantMembers` (recursive term only) and `findDescendantMembersWithDiscordIdQuery` (both terms) sit in the same file with opposite anchors, and each is right for its consumer — the latter backs a channel/role GRANT, where an archived group should grant nothing. `EventRsvpsRepository`'s walks deliberately leave the anchor unfiltered: filtering it would mean "this event's own member group is archived, so remind NOBODY", silently muting a live event.

### `fee_assignment_status_v` Carries No `archived_at` Filter — Every Consumer Adds Its Own

The view (`packages/migrations/src/before/1783000000_create_finance.ts`) is a plain `fee_assignments fa JOIN fees f` with **no** `archived_at` predicate, and it does not expose the column — inside the view a row on an archived fee is indistinguishable from a live one. Any query that reads the view to decide **collectable debt** (reminder candidates, bank-match candidates, credit-coverage totals, settlement plans) MUST carry `AND f.archived_at IS NULL` on its own `fees` join. Archiving is the treasurer's "stop collecting this", and every other money path already honours it: `api/finance.ts` fails `feeArchived` in `recordPayment`, `assignFee` and `updateAssignment`, `BankTransactionMatcher` and `api/bank-sync.ts`'s manual-match candidates filter it, `MemberCreditsRepository.settle` filters it.

Two things make the omission invisible, and both bit `FeeAssignmentsRepository._findReminderCandidates`:

1. **The query usually already joins `fees`** — for `f.currency` or `f.name` — so the join reads as if it carried the filter. Both CTE branches joined `fees f` and still selected assignments on archived fees.
2. **An outer `f.archived_at IS NULL` does not reach a nested subquery over the view.** The credit-suppression `NOT EXISTS` re-reads `fee_assignment_status_v v2` to total the member's outstanding balance, so that subquery needs its own `JOIN fees f2 ON f2.id = v2.fee_id AND f2.archived_at IS NULL`. Without it an archived fee inflates the outstanding total, credit never reaches full coverage, and the member is reminded about the *live* fee forever.

Queries that list history for a human rather than compute what is owed (`FinanceOverviewRepository.overviewByTeamQuery`, `myStatusQuery`) do not carry the filter; decide which kind yours is before copying either shape. Regression coverage: `test/integration/repositories/FeeAssignmentsRepository.reminderArchived.test.ts` (both CTE branches, plus the credit-coverage total).
...
### Wire-value projection & effective-value guards
...
4. **Widening a stored enum that appears in a response DTO is a breaking change for every already-loaded browser tab — the read schema must be tolerant BEFORE the first new value is ever written.** Web deploys last in the rolling order (bot → server → web), so a server deployed with the new literal can serve it to a browser tab still running the previous web bundle; if that response field is a closed `Schema.Literals([...])`, the whole array fails to decode client-side and the page shows a generic error with an empty network tab (nothing looks "wrong" server-side). The fix is not the projection pattern above (there is no legacy value to project down to — the new value is the real, permanent answer) — it is to make the RESPONSE schema a tolerant reader (`Schema.String`) while keeping every WRITE payload on the closed, narrow union. `payments.method` gaining `'credit'` (member-credit settle-all) is the worked example: `PaymentView.method` is `Schema.String`, but `RecordPaymentRequest.method` and `CreateSettlementRequest.method` stay `ManualPaymentMethod` (`'cash' | 'bank_transfer'`, never `'credit'` — that literal is minted server-side only, by `MemberCreditsRepository.settle`, in the same transaction as the account debit). A DTO that is never widened again (`MemberCreditDepositView.method`) stays on the closed union — tolerance is only for a field that WILL see a new stored value before every client has redeployed.
5. **The mirror of rule 4 for REQUEST payloads: a newly REQUIRED field on a request DTO 400s every call from an already-loaded browser tab.** Because web deploys LAST (bot → server → web), a new server serves old web bundles for the whole rollout window, and those bundles send the payload without the new key. Declare the field as an OPTIONAL KEY with **absent = keep the stored value**, never a plain required field and never a required field with a client-side default. Use `Schema.OptionFromOptional(...)` (`packages/domain/AGENTS.md` → Schema Patterns), which decodes an absent key straight to `Option.none()`.

   Implement "keep the stored value" by `COALESCE`ing **the parameter**, not `EXCLUDED`, in the upsert's `DO UPDATE` clause. `EXCLUDED` has already been `COALESCE`d to the insert-time default by the `VALUES` list, so reading it in `DO UPDATE` silently resets the column on every save from an old bundle — the exact bug the optional key was supposed to prevent:
   ...
   The `::boolean` cast is required at BOTH occurrences: the `sql` template emits a distinct placeholder per interpolation, so the second parameter inherits no type from the first (see "Postgres Type Conventions" → cast a bind parameter explicitly). Carry the field as `Option.Option<boolean>` end to end so no layer re-wraps it: `Schema.OptionFromOptional(Schema.Boolean)` on the request DTO, `Option.Option<boolean>` on the repository input, `Schema.OptionFromNullOr(Schema.Boolean)` on the repository `Request` schema (the row read back carries an explicit `null`, not a missing key). The handler then passes `payload.auto_credit_enabled` straight through — no `Option.fromNullishOr`, no `??` — and the web sends `Option.some(values.autoCreditEnabled)` from a form value that is always a concrete `boolean`. Reference implementation: `bank_sync_config.auto_credit_enabled` — `BankSyncApi.UpsertBankSyncConfigRequest`, `BankSyncConfigRepository.upsertQuery`, `api/bank-sync.ts`'s `upsertConfig` handler, `fioBankForm.ts`'s `fioBankRequestFrom`. The same contract on the same kind of endpoint already existed: `EmailForwardingApi.UpsertEmailForwardingConfigRequest.imap_secret` (`Schema.OptionFromOptional(Schema.NonEmptyString)`), where an absent key likewise means "keep the stored secret".

   Adding any column to `bank_sync_config` also means extending `BankSyncConfigRepository.SELECT_COLUMNS`, the INSERT column list, the `DO UPDATE SET` list, `UpsertBankSyncConfigInput`, `BankSyncConfig.BankSyncConfig`, `BankSyncApi.BankSyncConfigView`, `toConfigView`, `defaultConfigView`, and `applications/web/src/components/organisms/team-settings/fioBankForm.ts` (`FioBankFormValues` + `fioBankFormFrom` + `fioBankRequestFrom`) plus the owning `FioBankCard.tsx`. Two test sites must move with it: the `fullUpsertInput` literal and its per-column assertion in `test/integration/repositories/BankSyncConfigRepository.test.ts` (the full-column round-trip that exists precisely to catch a column dropped from one of the SQL lists — see "Hand-written INSERT / UPDATE Column Lists" rule 4), and the hand-written `INSERT INTO bank_sync_config` inside `enableBankSync` in `test/integration/bankSyncFixtures.ts`.
...
5. If the parent invite has a `group_id`, `applyInviteGroup` binds the new member to that group in BOTH halves every other group-add in this codebase applies: the `group_members` row (`ON CONFLICT DO NOTHING`, since `joinViaInvite` may already have written it) AND `member_added` channel-sync events for the group and its active ancestors, so the bot grants the group's own Discord role (`discord_channel_mappings.discord_role_id`, created by `createGroup`'s `emitChannelCreated`). This bind runs, and the role diff (`reconcile`) is bound below it, on every branch — including "already active" — before `buildWelcomeMeta` separately fetches the group's color via `groups.findGroupById` → `sanitizeHexColor` to populate `group_color_int`.
...
1. **Every group-add writes `group_members` AND emits `member_added` for the group and its active ancestors.** There are exactly two exceptions, both because the member is not a guild member the emit could target, or already holds the role:
   - `setupNewMember`'s mapping-derived add — it matched on the member already holding the Discord role, so there is nothing new to grant.
   - `api/invite.ts` `joinViaInvite`'s group bind — the member has accepted the invite on the web but has NOT joined the guild yet, so `member_added` would target a non-member and fail at the bot. The emit for that group happens on the Discord join itself, in `applyInviteGroup`. Any NEW group-add that is not one of these two must emit.

   `Guild/ReconcileMembers` passes `invite_code: None`, so every member it processes hits `findRecentByUserAndGuildWithContext`'s 15-minute recency window. Since `applyInviteGroup` now emits `member_added`, that window is load-bearing for **Discord writes**, not just for welcome metadata: anyone with an acceptance inside it gets a fresh `emitMembersAddedBatch` on every bot reconnect. `channel_sync_events` has no uniqueness constraint (`packages/migrations/src/before/1741000000_create_channel_sync.ts:21-33`), so those are real duplicate rows — bounded and idempotent at the bot, but do not widen that window without accounting for the fan-out.
2. **Inside `registerMemberWithReconcile`, every write that changes a member's effective roles must be bound above `reconcile`.** `reconcile` diffs `findEffectiveRoleIdsForMember`, which reads `member_roles` UNION the group-inherited walk from the database at call time — a group bound after the diff runs is invisible to it. This is scoped to `registerMemberWithReconcile`, not universal: `api/group.ts`'s `addGroupMember` (`:445-527`) has no reconcile at all and only emits channel-sync, so a captain adding someone to a group from the web UI does not get that member's `role_groups`-linked Sideline roles onto Discord until the next `GUILD_CREATE`. Within `registerMemberWithReconcile`, this ordering is a compile error, not a convention — but not because of the `boundGroupIds` parameter itself. The enforcement is the `{ newMember, boundGroupIds }` **destructure at the `reconcile` bind**: `Effect.Do` only exposes keys bound earlier, so hoisting `reconcile` above `boundGroupIds` stops type-checking. The parameter gives that destructure a reason to exist — `noUnusedParameters` is `false` (`tsconfig.base.json:22`), so deleting the `logDebug` call that consumes the parameter would NOT produce an error, it would just make the parameter look dead, and removing it would silently take the guarantee with it. The parameter now ALSO carries real behaviour: it is passed to `emitMemberGroupChannelRoles` as `alreadyEmittedGroupIds` to suppress duplicate `member_added` rows (rule 6). Do not remove that destructure to "simplify" the chain.
3. **Ancestor walks that drive Discord writes must use `getActiveAncestors`, never `getAncestorsIncludingArchived`.** `getAncestorsIncludingArchived`/`findAncestors` is intentionally archived-blind because `moveGroup`'s cycle check needs to see archived nodes; emitting `member_added` for an archived ancestor would make the bot's self-healing `createRoleOnly` branch recreate a Discord role for a group that was deleted. `getActiveAncestors`'s predicate placement is a verbatim copy of `effectiveRoles.ts`'s walk — change one, change both, or role sync and channel sync will disagree about which ancestors still grant. Every add-path site (`api/group.ts`'s `addGroupMember` `:504` and `syncRoleMembers` `:1045`, `rpc/guild/index.ts`'s `applyInviteGroup` `:306`) now uses `getActiveAncestors`. The two wrappers have DIFFERENT signatures — `getAncestorsIncludingArchived(groupId)` vs `getActiveAncestors(groupId, teamId)` — so migrating a call site means threading a `teamId` into scope; that extra argument is the point, not friction to route around with a `group.team_id` re-read the handler has not already done. The archived-blind wrapper carries `IncludingArchived` in its name for exactly one reason: under the old name `getAncestors` it was the shorter, alphabetically-first autocomplete hit next to `getActiveAncestors`, and that mis-pick shipped the same archived-ancestor leak twice. Never rename it back, and never add a third ancestor wrapper whose name does not state its archived stance. `deactivateMemberCascade.ts` is the one deliberate holdout and stays on `getAncestorsIncludingArchived` — that is a recorded decision, not an omission: a full deactivation must revoke everywhere, and step 7 of that cascade hard-deletes every `group_members` row, so over-revoking there cannot be wrong (see the call-site comment above the ancestor walk in that file for the full reasoning). Separately, `api/group.ts`'s `addGroupMember` also runs no role diff at all (see rule 2's scoping above) — that's a missing reconcile, not an archived-blind ancestor walk, but it belongs on the same "still owed" list.
4. **A liveness check on a group reached from `team_invites` must come from `groups.findGroupById`, not the invite row.** `TeamInvitesRepository.findByCode` is a bare `SELECT * FROM team_invites` with no join to `groups`, so `invite.group_id` can point at an archived group; `findGroupById` carries the `is_archived = false` filter that both acceptance-context queries already apply, keeping the web and Discord paths in agreement.
5. **`GuildsRpcLive`'s top-level `Effect.Do.pipe` is at 18 of a maximum 20 arguments** (`Pipeable.d.ts:40`, the last typed overload is `ab`…`tu`). A 21st argument silently drops to the untyped rest overload and degrades every `deps` destructure to `unknown`. Resolve a newly-needed repository inline with `<Repository>.asEffect()` inside the function that needs it (as `applyInviteGroup` does for `ChannelSyncEventsRepository`), or split the chain. Do not append a 20th/21st top-level bind.
6. **A third producer of group `member_added` exists, and it is sighting-driven, not invite-driven** (`utils/emitMemberGroupChannelRoles.ts`, wired into `observeGuildMembership`). Where `applyInviteGroup` requires a resolved invite context and `setupNewMember`'s channel-mapping-derived add requires the member already holding the Discord role, this one runs on every `GUILD_MEMBER_ADD` observation (`payload.source === Some('member_add')`) and diffs the member's `findActiveGroupsWithAncestorsForMember` set against `payload.roles`, independent of any invite — this is what fixes a member who joins Discord manually or past the invite's 15-minute acceptance window (`InviteAcceptancesRepository.ts:343`). Rules that apply specifically to it:
   - **Only a group whose `discord_channel_mappings` row has `discord_role_id IS NOT NULL` is ever emitted for**, same reasoning as `applyInviteGroup`'s ancestor walk but stated for this producer directly: the bot's `handleMemberAdded.ts` has no existing role to attach the member to for a role-less or unmapped group, so it falls to `createRoleOnly(event.guild_id, event.group_name)` (`:39-53`, `createRoleOnly` at `:42`; or `:55-66`, `createRoleOnly` at `:57`) — a raw-named, colourless role — after which `findGroupsMissingRole`'s selection predicate (`m.id IS NULL OR m.discord_role_id IS NULL`, `DiscordChannelMappingRepository.ts:404`) excludes the group from ever being revisited by the real group-role backfill. A role-less group is left for `Channel/BackfillMissingGroupRoles` on purpose.
   - **`Guild/ReconcileMembers` (`source: 'reconcile'`) deliberately does not call it.** Emitting per-member on that path would re-derive a whole page of already-active members' entire group trees on every bot reconnect or `GUILD_CREATE` — the N+1 fan-out this feature's implementation plan rejected (see "Fan-out arithmetic" reasoning: `N` members × `M` groups incl. ancestors turns a 1 000-member guild into thousands of queue rows). This means a member first observed by reconcile — someone who **joined the guild while the bot was disconnected past the gateway resume window** (so Discord never replayed their `GUILD_MEMBER_ADD`) **and** whose invite acceptance, if any, is now older than the 15-minute recency window — gets nothing from this producer, and recurs on every future outage. This is an accepted, documented gap, not an oversight: no signal anywhere today distinguishes a bot outage from ordinary uptime (a three-week healthy gap between `GUILD_CREATE`s looks identical to a three-week outage), so any reconcile-side gap threshold either fires for every guild on every deploy or never fires at all. The prerequisite that would let this be closed automatically is a **persisted per-guild disconnect signal** — something that records "the bot was disconnected from guild X between t0 and t1" — which does not exist yet. Until then, the remedy is the manual per-group "Sync role members" action (`api/group.ts` `syncRoleMembers`, `:1018-1102`).
   - **Its call site MUST keep the `Effect.catchCause` + `logWarning` wrap.** `emitMemberGroupChannelRoles` declares `Effect.Effect<…, never, …>`, but every repository call inside it pipes `catchSqlErrors`, so a transient database failure is a DEFECT, not a typed error — see root `AGENTS.md` → "Effect-TS Patterns" → "Error Handling" → "Rules" rule 8. Unwrapped, that defect short-circuits the `Effect.tap` it sits in and skips the welcome message sequenced after it, i.e. an additive heal would break pre-existing join behaviour. Same convention as `reapplyGroupGrants` at `rpc/channel/index.ts:220-221` and `:245-246`.
   - **`MAX_GROUP_CHANNEL_EMISSIONS_PER_MEMBER = 25` is a PERMANENTLY LOSSY cap on this path.** This producer fires exactly once per member, on their join, with no later re-derive to catch what the cap dropped. That is why `findActiveGroupsWithAncestorsForMember` carries `ORDER BY min(depth), name, id` (nearest-first, deterministic): the groups kept when the cap bites must be the member's own groups and closest ancestors, never an arbitrary slice of an unordered result. Never remove that `ORDER BY`, and never raise the cap without first giving this path a re-derivation trigger.
   - **`boundGroupIds` is subtracted from the desired set (`alreadyEmittedGroupIds`), and this is now a functional argument, not just a type-level ordering guard.** `applyInviteGroup`, bound earlier in the same `Guild/RegisterMember` call, may already have emitted `member_added` for the invite's group and its active ancestors; `channel_sync_events` has no uniqueness constraint, so without the subtraction a group-scoped invite accepted and joined in the same call writes real duplicate rows. See rule 2 for the second, independent reason that parameter must not be deleted.
   - **`applyInviteGroup` itself still emits unconditionally into a group that may not be provisioned yet** (no `discord_role_id IS NOT NULL` guard on that path) — a pre-existing hazard, not introduced by this producer: a group-scoped invite accepted and joined before the group's own `channel_created` has been processed can still reach `handleMemberAdded.ts:38-64`'s raw-named-role fallback today. Human-rate (bounded by how fast a captain can create a group and mint an invite to it before the channel queue drains), accepted, out of scope for this fix.

Two facts worth keeping in mind when touching this handler: the group-channel-role emit is bot-version-gated — `payload.source: None` (a pre-PR-8 bot that doesn't send `source`) skips `observeGuildMembership` entirely, so an old bot never triggers it at all. And nothing in `registerMemberWithReconcile` is transactional: the group insert, the `channel_sync_events` insert, and `markDiscordJoined` are three independent commits (the file's only `sql.withTransaction` is elsewhere, unrelated to this path). Fixing the ordering buys correctness of what gets written when everything succeeds — it does not buy atomicity if one of the three fails partway through.
...
`joinViaInvite`'s group bind: when `invite.group_id` is `Some`, the handler calls `groups.findGroupById(groupId)` and, only if that returns `Some`, `groups.addMemberById(groupId, membership.id)`. Three rules hold here:

1. **`findGroupById` is the liveness check, never the invite row.** `TeamInvitesRepository.findByCode` is a bare `SELECT * FROM team_invites` with no join to `groups`, so `invite.group_id` can point at an archived group. `findGroupById` carries the `is_archived = false` filter that both acceptance-context queries already apply, so the web and Discord paths agree on which groups still bind.
2. **Emit nothing to Discord here.** See "`Guild/RegisterMember` and Welcome Metadata" rule 1 — the member has not joined the guild yet. `applyInviteGroup` emits `member_added` on the join; its insert is `ON CONFLICT DO NOTHING`, so this write costs it nothing.
3. **Do not add a third write to this `Effect.tap` chain without a transaction.** Both taps are `E = never` after `catchSqlErrors`, so a failure is a defect that kills the join after the membership and Player role are committed but before the `invite_acceptances` row exists.
...
2. Call `repo.emitIfGuildLinked(teamId, eventType, ...)` — runs `SELECT guild_id FROM teams WHERE id = ${teamId}`; on `Some` it inserts the event row, on `None` it is a silent no-op (`Effect.void`). **`teams.guild_id` is `NOT NULL` and `UNIQUE`** (`packages/migrations/src/before/1741200000_guild_linking.ts:30`, `:33`), so the `None` branch does NOT mean "the team exists but has no guild" — that state is unrepresentable. It means the team ROW IS ABSENT (deleted team, or a `teamId` that never existed), and the no-op silently swallows that. Never write a code path or a comment that treats `None` here as an unlinked team.
...
4. **Emit only when the primary operation actually changed the state the bot mirrors.** A `DELETE`/`UPDATE` that matched nothing, or that removed one of several grants that all map to the same Discord state, MUST NOT emit — the bot applies the event unconditionally and would drive Discord out of sync with the database. `src/api/role.ts` `unassignRole` deletes only the DIRECT `member_roles` row, so after the delete it re-reads `findEffectiveRoleIdsForMember(memberId)` and skips BOTH `emitRoleUnassigned` and the `role_removed` notification when the member still holds the role through a group. Without that check, clicking remove on an inherited role stripped the member's real Discord role while Sideline still considered them to hold it.
   ...
   **This rule covers writes that NARROW Discord state (`DELETE`/`UPDATE`); it does not apply to idempotent adds.** Never skip an emit because an `INSERT ... ON CONFLICT DO NOTHING` reported no change: the row existing does not mean the Discord grant ever happened. `applyInviteGroup` (`src/rpc/guild/index.ts`) emits `member_added` for the group and its active ancestors regardless of whether the `group_members` insert was a no-op, precisely because `joinViaInvite` may have written that row while the member was still outside the guild. When the primary write is an idempotent add and the bot handler is itself idempotent (`handleMemberAdded` → `addGuildMemberRole`), always emit.
...
Group→Discord-role provisioning is event-driven, so a group whose `channel_created` event was never processed (bot down, bot not in the guild, or the row was marked permanently failed — see "Channel Sync Event Lifecycle (Server side)"), or whose `channel_created` emit never happened at all (the emit in `createGroup`, `src/api/group.ts:180-188`, is NOT defect-wrapped — a deviation from "Sync Event Pattern" step 3 — so a failed insert 500s the request AFTER the `groups` row has already committed) — can stay role-less, and any managed-channel access grant referencing that group silently never applies. (It is NOT caused by an unlinked team: `teams.guild_id` is `NOT NULL`, so `emitIfGuildLinked` always finds a guild for a team that exists — see "Sync Event Pattern" step 2.) Two server mechanisms heal this:
...
Roster→Discord-role MEMBER provisioning is event-driven (a member is added to a role when a `member_added` event fires), so members whose `member_added` event was never processed (bot down, bot not in the guild, or the row was marked permanently failed — see "Channel Sync Event Lifecycle (Server side)") can stay off the Discord role forever. This is the roster analogue of the group-role backfill above, but with two deliberate differences: it is **on-demand and team-scoped** (NOT polled by `slowPollLoop`), and it re-attaches MEMBERS to an EXISTING role rather than provisioning a missing role.

- **Trigger:** `POST /teams/:teamId/rosters/backfill-role-members` (endpoint `backfillRosterRoles`, `packages/domain/src/api/Roster.ts`), handled in `src/api/roster.ts`, gated by `requirePermission(membership, 'roster:manage', ...)`. There is no cron/poll path — a team captain clicks the "Re-sync roster role members" button on the web rosters list page. Returns `BackfillRosterRolesResult { processedCount, remainingCount }`. **The web button invokes the endpoint exactly ONCE per click — it does not loop.** `RostersListPage.tsx`'s `handleBackfillRoles` (`applications/web/src/components/pages/RostersListPage.tsx:46-75`) awaits one call and only varies the toast: `roster_backfillRolesNone` when both counts are 0, `roster_backfillRolesQueued` when `remainingCount === 0`, `roster_backfillRolesQueuedPartial` (which shows the remainder) otherwise. Draining a team with more than `BACKFILL_LIMIT = 50` eligible rosters requires the captain to click again.
...
### Group-role member backfill (cohort healing for already-provisioned groups)

This is the group analogue of "Roster-role member backfill" above, healing the **existing production cohort**: members already in a group whose Discord channel role was never granted (bot down at the time, or the member's join happened outside the live path's reach — see `applications/bot/AGENTS.md` → "Guild/ReconcileMembers" accepted gap, and `applications/server/AGENTS.md:345`). The group-role backfill above (`findGroupsMissingRole`) heals groups with no role at all; this sweep heals groups that already have a role but whose members never got it.

- **The branch-1 trap — read this before writing any similar sweep.** `emitGroupRoleMemberBackfill` (`src/utils/backfillGroupRoleMembers.ts:31-48`) MUST call `channelSync.emitChannelCreated` with `Option.none()` for `existingChannelId` and `undefined` for the channel name — never `row.discord_channel_id`. `handleCreated.ts:42-58` (bot side) matches `existing_channel_id` **first**, with **no** `Channel/GetMapping` read at all, and unconditionally calls `createRoleForChannel` in that branch — so emitting with an existing channel id for an already-provisioned group would create a **duplicate Discord role and overwrite the mapping**. Passing `Option.none()`/`undefined` instead routes the event to `handleCreated.ts`'s branch 3 (mapping already has a role → reuse it, no creation, no mapping write), which then runs the shared member backfill. `backfillRosterRoleMembers` (the roster analogue) *can* safely pass `row.discord_channel_id` — that is NOT a discrepancy to "fix" here — because `handleRosterChannelCreated.ts:52-55` binds `Channel/GetRosterMapping` **before** inspecting `existing_channel_id`, so reusing the channel id there is safe. Anyone copying the roster util as a template for a new sweep will hit this trap; copy the group util's `Option.none()` shape instead when the target handler is not mapping-first on the channel-id branch.
- **Selection partitions cleanly against `findGroupsMissingRole`.** That query needs `m.id IS NULL OR m.discord_role_id IS NULL` (`DiscordChannelMappingRepository.ts:366`); this sweep's `findActiveGroupsWithRole`/`countActiveGroupsWithRole` need `m.discord_role_id IS NOT NULL`. No group is ever selected by both queries.
- **Dedup guard checks `processed_at IS NULL` ONLY — never `AND error IS NULL`.** Same reasoning as the roster sweep above: a row with `processed_at IS NULL AND error IS NOT NULL` is a transient failure the bot is still retrying (see "Channel Sync Event Lifecycle (Server side)" → `markFailed`), so it is still mid-provision and must still block a duplicate re-emit.
- **No `teams` join and no `guild_id` predicate — do not "harden" this back in.** `teams.guild_id` is `NOT NULL` and `UNIQUE` (`packages/migrations/src/before/1741200000_guild_linking.ts:30`), and both `groups.team_id` and `discord_channel_mappings.team_id` are `REFERENCES teams(id) ON DELETE CASCADE` — a "team present but its guild unlinked" row is unrepresentable, and a deleted team cascades away every row this query could select. (Never write that `teams.guild_id` can be `NULL` — a `lookupGuildId`/similar helper returning `None` means the **team row is absent**, not that a present team has no guild.)
- **10011 (Unknown Role) healing clears the role column, it never deletes the mapping row, and only when a channel still exists.** `channelUtils.ts`'s `clearStaleRoleOnUnknownRole` calls the new RPC `Channel/ClearMappingRole` → `DiscordChannelMappingRepository.clearGroupRole`, which is gated `WHERE ... AND discord_channel_id IS NOT NULL`. Deleting the whole row on a 10011 would be wrong here: `discord_channel_mappings` is a shared channel+role row, so dropping it would also destroy a still-valid channel link. Two reasons the column-clear is gated on `discord_channel_id IS NOT NULL`, both required:
  - Deleting the mapping row instead of clearing the role column would make `findGroupsMissingRole`'s "no mapping row" branch create a **second** Discord channel next sweep, orphaning the first (`emitGroupRoleBackfill.ts:61-69`).
  - The `discord_channel_id IS NOT NULL` gate stops this from ever clearing a **detached** group's role into the both-NULL state: a group a captain deliberately detached (`clearGroupChannel`) has `discord_channel_id = NULL, discord_role_id = Some`, and since `create_discord_channel_on_group` defaults to `true`, clearing that role too would make `findGroupsMissingRole` create a brand-new Discord channel for a group somebody intentionally detached.
  - **Accepted consequence:** a detached-or-role-only group whose role is dead (deleted directly in Discord) is not auto-healed by this gate — clearing is a no-op — so it is re-selected by `findActiveGroupsWithRole` and its `channel_created` re-emitted on every sweep, forever, for that group. Narrow and non-destructive (the bot's branch-3 reuse just no-ops on the dead role id rather than erroring), so left as-is rather than widening the gate with a channel predicate.
- **The true fan-out bound — and why the admin action MUST stay paged.** One `channel_created` row expands, on the bot, into an uncapped `Channel/GetGroupMembers` read (the group's **direct members PLUS every member of every non-archived descendant subgroup** — see "Group-role backfill and grant reapply" → point 3 above) followed by one sequential `addGuildMemberRole` per member at `concurrency: 1` on the **single global channel queue**. A 1 000-member group is one row that blocks every other channel event (new group/roster channels, managed-channel access, `channel_updated`) for minutes. The real bound of one invocation is `Σ (direct + descendant members) over the ≤ 50 selected groups` — for the install-base-wide sweep, `Σ ... over every eligible group across the page's teams`. This is why `BACKFILL_LIMIT = 50` (`backfillGroupRoleMembers.ts`) and `TEAMS_PER_INVOCATION = 1` (`runGroupRoleBackfillPage.ts`) both exist, and why NEITHER may be raised, nor may an all-teams-at-once action ever be added, without first giving this path a way to bound or parallelize the per-member Discord write loop.
- **The operator runbook.** Three entry points share the identical page/sweep logic, so they never drift:
  - Captain-facing, team-scoped: `POST /teams/:teamId/groups/backfill-role-members` (endpoint `backfillGroupRoles`, `packages/domain/src/api/GroupApi.ts`), gated by `requirePermission(membership, 'group:manage', ...)`, one click = one call to `backfillGroupRoleMembers(teamId)` (`BACKFILL_LIMIT = 50` per call, same non-looping web-button contract as the roster button above).
  - Global-admin, install-base-wide, paged: `POST /auth/global-admins/group-role-member-backfill` (endpoint `backfillGroupRoleMembers`, `packages/domain/src/api/GlobalAdminApi.ts`), gated by `requireGlobalAdmin`, one call = one page of `runGroupRoleBackfillPage(after)` (`TEAMS_PER_INVOCATION = 1` team per call).
  - CLI driver (`applications/server/src/scripts/backfillGroupRoleMembersCli.ts`, run via `pnpm --filter @sideline/server backfill-group-role-members [--after <team-uuid>]`): calls `runGroupRoleBackfillPage` directly against the database in a loop until every team is visited, logging the cursor after every page so an interrupted run can resume with `--after <the last logged nextAfter>`.
  - **Termination contract:** `GroupRoleBackfillResult.nextAfter` is `Option.none()` **exactly when** `remainingTeams === 0` — a caller may check either field and always get the same answer. `runGroupRoleBackfillPage`'s cursor is the last team visited *this* page (in the same `(created_at, id)` stable order `TeamsRepository.findTeamIdsAfter`/`countTeamsAfter` walk); a page of zero teams left leaves the cursor as-is rather than rewinding.
- **No remove-extras half exists for this sweep — unlike the roster backfill's paired `reconcileRosterRoleExtras`.** This is add-only by design: `backfillGroupRoleMembers` only ever grants a role, and there is no group-axis equivalent of "Roster-role remove-extras reconcile" in this codebase yet.
- **Why this can't (yet) run itself:** the underlying gap this sweep exists to heal is `applications/server/AGENTS.md:345`'s `Guild/ReconcileMembers` note — a member who joins while the bot is disconnected past the gateway resume window is first observed by reconcile, which deliberately emits nothing for already-active members (to avoid the N×M fan-out of re-deriving every active member's whole group tree on every reconnect). The remedy today is this sweep, run manually. The prerequisite that would let it fire automatically instead is a **persisted per-guild disconnect signal** — something that records "the bot was disconnected from guild X between t0 and t1" — which does not exist: nothing today distinguishes a long outage from a long healthy uptime (a three-week gap between `GUILD_CREATE`s looks the same either way).
...
- **Set it on EVERY write path that alters a member's view — no exceptions.** `EventsRepository.markEventPersonalMessagesDirty(eventId)` runs `UPDATE events SET personal_messages_dirty_at = date_trunc('milliseconds', now()) WHERE id = $1`. It is always best-effort (`markPersonalMessagesDirtyBestEffort` in `src/api/event.ts` wraps it in `Effect.catchCause(... logWarning)`; series/RPC call sites use `.pipe(Effect.ignore)`) so a marker failure never rolls back the primary write. The complete set of call sites that MUST mark dirty:
  ...
  - **Event start (`active`→`started` flip)** — `EventStartCron` calls `markEventPersonalMessagesDirty(event.id)` immediately after the status flip and BEFORE Discord resolution/emit (same ordering rationale as the missed-RSVP increment — see "Idempotent Counter Increment Folded Into the `active`→`started` Status Flip"). A started event is no longer active, so reconcile removes it from members' personal channels. Marking before Discord work means a Discord failure can't lose the mark, since the started event won't reprocess.

  - **Member event preferences PATCH** — `src/api/team.ts`'s `updateMyEventPreferences` calls `markTeamUpcomingEventsPersonalMessagesDirty(teamId)` ONLY when `showAttendeeList` actually changed; the other two toggles mark nothing (see "Member event preferences" below).
...
- **Self-healing stale sweep.** `EventStartCron` also runs `EventsRepository.markStalePersonalMessagesDirty()` ONCE per cycle (best-effort, `Effect.catchCause` → logWarning, independent of the per-event loop). It re-marks any event that is no longer active/upcoming (`status <> 'active' OR start_at < now()`) but still holds `personal_event_messages` rows (`id IN (SELECT DISTINCT event_id FROM personal_event_messages)`) and is not already dirty, so reconcile deletes the stale personal messages. It is self-terminating — once reconcile deletes an event's `personal_event_messages` rows, the event no longer matches the subquery. This is the safety net for events that fell out of the window without a per-path mark (e.g. a prior cycle's failure); keep such stale-cleanup sweeps on `EventStartCron`, not in a new dedicated cron.
...
- **Both eligibility queries use the SAME `WITH RECURSIVE descendant_groups` CTE** (`PersonalEventChannelsRepository`): `_getMembersNeeding` adds an `EXISTS (... descendant_groups ... group_members)` predicate (skipped when `group_id::uuid IS NULL`), and `_getMembersToDeprovision` uses the `NOT EXISTS` negation to find channel-holders who fell outside the group. `_getMembersNeeding` additionally fans each member out over their desired buckets with `CROSS JOIN LATERAL unnest(${sql.unsafe(desiredBucketsSql('tm'))}) AS b(bucket)` and LEFT JOINs `personal_event_channels` on `pec.bucket = b.bucket`, so it emits one row per still-missing `(member, bucket)` and orders by `tm.id, b.bucket`. The CTE walks `groups.parent_id`, scoped to the same `team_id`. This mirrors the established descendant-CTE pattern (see "Group-role backfill" rule above); reuse it for any future group-scoped membership filter — never flatten the hierarchy in application code.
- **`Guild/GetGuildsNeedingPersonalProvisioning` surfaces a guild needing provision, de-provision, OR rename.** Its `_getGuildsNeedingProvisioning` query is a `UNION` of TWO SELECTs carrying five labelled branches — (a), (b), (c) and (e) are OR'd inside the first SELECT's `WHERE`, (d) is the second `UNION` arm (an inactive member fails the first SELECT's `tm.active = true`): (a) an eligible member still missing a channel **for any bucket in their desired set**, (b) a channel-holder no longer in the configured group, (c) a channel whose `applied_channel_format` drifted from the team's current format (see "Personal-channel format-change drift"), (d) an **inactive** member still holding a personal channel (`tm.active = false AND pec.discord_channel_id IS NOT NULL`) — a deactivated member is unreachable via branch (a) yet must be de-provisioned regardless of any group config — and (e) a member holding a channel for a bucket outside their current desired set (a one↔three mode switch), gated on their desired set being fully provisioned. A guild appears if any branch matches, so Pass 1's provision + de-provision + rename all have work.
- **`Guild/GetPersonalChannelsToDeprovision` always merges group-based AND inactive-member de-provision.** The handler runs `getInactiveMembersToDeprovision(teamId, limit)` (channel-holders where `tm.active = false`) **unconditionally**, and additionally runs `getMembersToDeprovision(teamId, groupId, limit)` (the `NOT EXISTS` group query) ONLY when `discord_personal_events_group_id` is `Some`. It merges these result sets — and the obsolete-bucket source described below — de-duplicating by `` `${team_member_id}:${bucket}` ``, NOT by `team_member_id` alone (a split member can present up to three obsolete channels in one tick). An unrestricted team (no group configured) still de-provisions inactive members — do NOT reintroduce a short-circuit-to-`[]` on `Option.none()` group id.
- **De-provision deletes the channel row and its messages in ONE CTE, scoped to that single bucket (Nastavitelná docházka, plan §6.2/S3).** `deletePersonalChannel(teamId, teamMemberId, bucket)` is a single statement: `WITH gone AS (DELETE FROM personal_event_channels WHERE team_id = … AND team_member_id = … AND bucket = … RETURNING discord_channel_id), purged AS (DELETE FROM personal_event_messages pem USING gone WHERE pem.team_member_id = … AND pem.personal_channel_id = gone.discord_channel_id) SELECT discord_channel_id FROM gone`, returning the freed `discord_channel_id` so the caller knows what to delete on Discord. This used to be two unwrapped statements (`_deleteMemberMessages` then `_deleteChannel`) — a failure between them could orphan message rows forever, and a member-wide `WHERE team_member_id = ?` message delete would, during a combined→split switch, wipe the rows the newly-created split channels just wrote. The CTE is atomic and scopes the message delete to the dying channel's `personal_channel_id` only. The bot deletes the Discord channel first and calls `Guild/DeletePersonalChannel` only after the channel is gone (see `applications/bot/AGENTS.md` → "Pass 1b — De-provision").
- **A member owns one `bucket = 'all'` row, or exactly three (`training`/`tournament`/`other`) — never both once a mode switch converges.** `_getObsoleteBuckets` (exposed as `getObsoleteBucketsToDeprovision`, merged into `Guild/GetPersonalChannelsToDeprovision` alongside the group-based and inactive-member sources) is gated: it returns a member's obsolete channel(s) ONLY once every bucket in their CURRENT desired set (`personal_channels_split`) already has a provisioned `discord_channel_id`. Provision and de-provision run in the same bot tick and provision swallows every failure, so without this gate a stuck provision (rate limit, 403, Discord's 500-channel cap, a crashed pod) would delete a member's only working channel before its replacement exists. With the gate, a stuck provision degrades to "the member keeps the channel they had" and the switch retries on the next tick. The dedup `Set` merging the three de-provision sources in `rpc/guild/index.ts` is keyed on `` `${team_member_id}:${bucket}` ``, not `team_member_id` alone — a split member can have up to three obsolete channels in the same tick.

##### Bucket routing (`src/repositories/personalChannelBucket.ts`)

`personal_event_channels.bucket` is one of `all` / `training` / `tournament` / `other` (migration `1792200001`, `CHECK` constraint `personal_event_channels_bucket_valid`). `all` is the combined default; `tournament` covers BOTH `event_type = 'tournament'` and `event_type = 'match'`.

Every query that maps an event onto a bucket, enumerates a member's desired buckets, or decides whether a channel is obsolete MUST splice a fragment from `src/repositories/personalChannelBucket.ts` with `sql.unsafe(...)` — the same technique as `eventVisibility.ts` and `effectiveRoles.ts`. Never hand-write the `CASE` or the bucket array inline.

| Export | Returns | Used by |
|--------|---------|---------|
| `eventBucketSql(eventsAlias)` | `CASE` mapping `event_type` → bucket; unknown/future types fall into `other`, so a new event type can never make an event invisible | `_listForEvent` |
| `desiredBucketsSql(memberAlias)` | `ARRAY['training','tournament','other']` when `personal_channels_split`, else `ARRAY['all']` | `_getMembersNeeding`, `missingDesiredBucketSql`, `deprovisionableBucketSql` |
| `missingDesiredBucketSql(memberAlias, teamIdExpr, memberIdExpr)` | TRUE when some desired bucket has no provisioned `discord_channel_id` (a missing row and a reserved-but-NULL row are the same condition) | `_getGuildsNeedingProvisioning` branch (a), `deprovisionableBucketSql` |
| `deprovisionableBucketSql(memberAlias, channelAlias)` | TRUE for a provisioned channel outside the desired set AND safe to tear down (the gate below) | `_getObsoleteBuckets`, `_getGuildsNeedingProvisioning` branch (e) |

- **The DB value is singular (`other`); only the rendered channel-name suffix is plural (`-others`).** That plural mapping lives in the bot's `formatPersonalChannelName.ts` and MUST NOT be duplicated here.
- **`_listForEvent` (`Guild/ListPersonalChannelsForEvent`) must return exactly ONE row per member.** It filters `pec.bucket = CASE WHEN tm.personal_channels_split THEN <eventBucketSql> ELSE 'all' END`. The `CASE` yields a single bucket value per member, which is what keeps the result single-rowed even in the transitional state where a member holds an `all` row AND split rows at the same time — reachable for a full tick under the de-provision gate. Two rows for one member would make reconcile post the same event twice. Never relax this to a plain `IN (...)` or drop the `CASE`.
- **The bot stays almost entirely bucket-unaware.** It echoes back whatever `bucket` the server handed it (`GetGuildsNeedingPersonalProvisioning`, `GetPersonalChannelsToDeprovision`, `GetPersonalChannelsToRename`) and uses it for exactly one decision of its own: the channel-name suffix. Keep new bucket logic in this module, not in `applications/bot`.

##### Member event preferences (`team_members.show_attendee_list` / `rsvp_reminder_dms` / `personal_channels_split`)

Three `BOOLEAN NOT NULL` columns on `team_members` (migration `1792200000`; defaults `true` / `true` / `false`), read and written ONLY through `TeamMembersRepository.findEventPreferences` / `updateEventPreferences`, and exposed as `GET`/`PATCH /teams/:teamId/me/event-preferences` (`src/api/team.ts`, handlers `getMyEventPreferences` / `updateMyEventPreferences`). They are `Model.FieldExcept(['insert'])` on the domain model — never set on insert, the DB `DEFAULT` owns new rows.

1. **Both handlers authorize with `requireMembership(members, teamId, currentUser.id, forbidden)` and operate on `membership.id`** — the caller's OWN membership row. There is no permission check and no `memberId` parameter: these endpoints can only ever read or write the caller's own preferences. Never add a variant that takes a target member id.
2. **`personalChannelsAvailable` is ADVISORY and response-only.** It is computed in the same query as `(ts.discord_personal_events_category_id IS NOT NULL)` over a `LEFT JOIN team_settings` — one extra expression, no extra round trip — and exists so the web can hide the channel-mode control. It MUST NOT reject a `personalChannelsSplit` write for a team with no personal-events category: the member's stored intent survives until a category is configured, at which point provisioning honours it. `UpdateMemberEventPreferences` (the PATCH payload) deliberately has no such field.
3. **Only a `showAttendeeList` change marks events dirty.** `updateMyEventPreferences` reads the current preferences first and calls `events.markTeamUpcomingEventsPersonalMessagesDirty(teamId)` ONLY when `current.show_attendee_list !== payload.showAttendeeList`. That is the one toggle that changes a rendered card. `rsvpReminderDms` is read at DM time and `personalChannelsSplit` is picked up by the provisioning poll, so dirtying every upcoming event of the whole team for either of them would be pure waste. Do NOT hoist the mark out of the conditional.
4. **`show_attendee_list` reaches the bot through TWO independent RPCs.** `Event/GetUpcomingEventsForUser` (`src/rpc/event/index.ts`) and `Guild/GetAllUpcomingEventsForUser` (`src/rpc/guild/index.ts`) both return `EventRpcModels.UpcomingEventsForUserResult`, and each resolves the flag through its OWN inline `SqlSchema.findOne` member lookup (`SELECT tm.id, tm.show_attendee_list FROM team_members tm ...`). They are separate copies: a member-level field added to that result MUST be added to both, or the bot's reconcile path and its `/event` path render the same member differently.
...
4. **`findStartable` has NO lower bound on `start_at`, and that is load-bearing for the integration suite.** `src/repositories/EventsRepository.ts:330-337` is `WHERE e.status = 'active' AND e.start_at <= NOW()` — nothing like `AND e.start_at > NOW() - INTERVAL '<n> days'`. That absence is the only reason the permanently-past `FIXED_NOW` (`2025-10-15T10:00:00.000Z`) in `test/integration/services/EventStartCron.deferred.test.ts` can coexist with a real, current database clock: every fixture there is force-set `status = 'started'` before the cron runs, so it never reaches `findStartable` at all, and the two deferred sweeps key off the injected `FIXED_NOW` instead. **If a lower bound is ever added to this query, every case in that file breaks** — adding one requires re-dating that whole file in the same PR.

## The Shared Non-Responder Query Filters to Below-Threshold Default-Role Holders

`EventRsvpsRepository.findNonRespondersByEventId(eventId, teamId, memberGroupId, maxMissedRsvps = 4)` is the single source of truth for "who still needs to RSVP". Both the reminder count surfaces (web `submitRsvp` non-responder list and `Event/GetRsvpBoard`) and the increment query (`incrementMissedForEventNonResponders`) apply the **same three eligibility filters**: `tm.active = true`, an `EXISTS` on the **effective** default-role population (`member_roles` UNION group-inherited via `group_members` → group ancestry → `role_groups` — see `effectiveRoles.ts`; `holdsRsvpEligibleRoleWhere('eff')`, i.e. `eff.was_default OR (eff.name = 'Player' AND eff.is_built_in = true)` — every role the team has EVER handed out as its default, not just the current one), and (for the read path) `tm.missed_rsvps < max_missed_rsvps`. A member who has missed `max_missed_rsvps` consecutive RSVPs drops off the reminder list until they respond and `resetMissedRsvps` clears the streak.
...
2. **Both queries splice `holdsRsvpEligibleRoleWhere` from `effectiveRoles.ts` and must stay identical** between the read query and the increment query — they target the same population, so a divergence would increment a counter for a member who never appears in the reminder list (or vice versa). It is a strict SUPERSET of `holdsDefaultRoleWhere`, which `TeamMembersRepository.getDefaultRoleId` splices to PICK one role on join: `setDefaultRole` writes `was_default` and `is_default` in the same statement, so the current default is always eligible. Keep that containment — if the RSVP queries and the join-time resolver ever disagree on the CURRENT default, a team that configures a custom one silently stops sending reminders to its new members. Do not point the resolver at `was_default`: it picks ONE row by priority, and a superseded ex-default must not compete with the `Player` fallback for that slot. Both build it from the shared `effectiveRolesFrom` fragment (`src/repositories/effectiveRoles.ts`) rather than reimplementing the group walk inline, for the same reason.
3. **`rsvp_reminder_dms` is CARRIED on the row, never filtered inside this query.** `NonResponderRow` selects `tm.rsvp_reminder_dms` and returns opt-outs like anyone else. Exactly ONE consumer drops them: `Event/GetRsvpReminderSummary` in `src/rpc/event/index.ts` applies `Effect.map(Array.filter((nr) => nr.rsvp_reminder_dms))`, because that RPC exists solely to drive the bot's reminder DM (`applications/bot/src/rcp/event/handleRsvpReminder.ts` is its only consumer). The organiser-facing non-responder surfaces (`Event/GetRsvpBoard`, the web `submitRsvp` list, both gated on `event:edit`) and `incrementMissedForEventNonResponders` MUST keep seeing opt-outs — a member who silences their own DMs has not stopped owing an RSVP. Never move this filter into `findNonRespondersByEventId`, and never add a second filtering consumer without deciding explicitly which of the two populations it wants.
4. **RSVP eligibility splices `holdsRsvpEligibleRoleWhere`, NOT `holdsDefaultRoleWhere`.** The two answer different questions and the RSVP queries must never be "simplified" back onto the resolver's predicate. `holdsDefaultRoleWhere` is `is_default OR built-in Player` — CURRENT-default membership. Its `Player` half covered a team's FIRST default change for free (every pre-existing member still held `Player`) and stranded the cohort in between on the SECOND: they hold only the superseded custom role, which is no longer `is_default` and was never `Player`, so they silently stopped appearing here and stopped accruing `missed_rsvps`. `holdsRsvpEligibleRoleWhere` is the same shape with the STICKY `roles.was_default` in place of `is_default` — set by `RolesRepository.setDefaultRole` alongside it and deliberately never cleared by `clearDefaultByRoleTeamQuery`, so the population is cumulative and every cohort a team ever created keeps its reminders. **Keep the built-in `Player` disjunct.** It is redundant against a correctly populated `was_default` (migration `1792600001` backfills it onto every built-in `Player`; `initTeamRoles` seeds it) and that is the point: it is the floor that stops a `roles` row created by any other path — a hand-rolled fixture INSERT, a future seeder that forgets the column — from silently costing a team its reminder DMs, which is the exact failure mode this predicate exists to remove. `EventStartCron.deferred.test.ts`'s `assignPlayerRole` is a live example of such an INSERT. Cohorts already stranded by a pre-migration second change are NOT recoverable (nothing recorded the superseded defaults) and return only if that role is re-set as default. Coverage: `test/integration/api/eventRsvpNonResponders.test.ts`, describe block "a second default change does not strand the previous cohort".

## The Team's Default Role Resolves In Exactly One Place

`TeamMembersRepository.getDefaultRoleId(teamId)` is the ONLY expression that answers "which role does a new member of this team receive": `is_archived = false AND (is_default OR (name = 'Player' AND is_built_in = true))`, ordered `is_default DESC LIMIT 1`. The built-in-`Player` half is a **fallback folded into the query, not into the caller** — a team that never configured a default still seeds `Player` rather than admitting a role-less member, and `ORDER BY is_default DESC` makes a configured default win over that fallback.

Five consumers, and no sixth may hand-roll the predicate: the three join paths (`src/api/invite.ts` accept, `src/api/auth.ts` `autoJoinTeams`, `src/rpc/guild/index.ts` guild registration) and the two role read handlers (`src/api/role.ts` `listRoles`, `getRole`).

Rules:

1. **Never re-derive the fallback outside this query** — not in a handler, not in a test helper, not in the browser. `holdsDefaultRoleWhere` (`src/repositories/effectiveRoles.ts`) is the membership form of the same predicate; its wider sibling `holdsRsvpEligibleRoleWhere` (sticky `roles.was_default`) is what both `EventRsvpsRepository` non-responder queries splice; see "The Shared Non-Responder Query Filters to Below-Threshold Default-Role Holders" above for what breaks when the two drift.
2. **API responses carry the RESOLVED answer, never the raw `roles.is_default` column.** `RoleApi.RoleListResponse.defaultRoleId` / `.defaultRoleGrantsManage` and `RoleApi.RoleDetail.isDefaultForNewMembers` are all computed from `getDefaultRoleId`. Shipping `is_default` per row instead would report `false` for every role of a team that configured nothing — while that team is in fact seeding built-in `Player` on every join, and the UI would render an empty control over a working system.
3. **`RolesRepository.setDefaultRole(roleId)` takes NO `teamId`, deliberately.** Both statements derive the team as `(SELECT team_id FROM roles WHERE id = ${roleId})`. Accepting the path param instead would let an admin of team A clear team A's default while marking a role of team B. `src/api/role.ts` guards the boundary too; the repository signature is the defence that survives a future caller forgetting it. Do not "simplify" it by adding the parameter back.
4. **The first statement inside its transaction is `SELECT id FROM roles WHERE team_id = (…) FOR UPDATE`.** The transaction alone does NOT serialize two concurrent setters: under READ COMMITTED, TX2's `WHERE is_default = true` re-evaluates only after TX1 commits and still does not see the row TX1 set (it was `false` in TX2's snapshot), so both mark a default and both hit `23505` on `idx_roles_team_default`. Any future write that must hold a "exactly one row per team" invariant needs the same row lock.
5. **Clear-then-set is two UPDATEs, never one `SET is_default = (id = ${roleId})`.** A single multi-row UPDATE transiently holds two `is_default` rows for the team and trips the partial unique index — Postgres checks it per row as each row is written, not at statement end (see `packages/migrations/AGENTS.md` → "A Bulk `UPDATE` That Changes A Uniquely-Indexed Value…").
6. **`initializeTeamRoles` seeds `Player` with `is_default = true` and MUST only run on a team that has no default yet.** Its `ON CONFLICT (team_id, name) DO NOTHING` catches a re-seed of the same names, NOT a conflict on `idx_roles_team_default` — calling it on an established team that configured a different default raises `23505`. Its only caller is `src/utils/provisionNewTeam.ts`, on a team created moments earlier.
7. **`setDefaultRole` still carries two known holes — read "Moving A Per-Team Default Flag: The Archived-Row Hole And The Zero-Row Rollback" below before copying it.** Its `markDefaultQuery` has no `is_archived` guard, and it commits even when the mark matches zero rows. `MembershipPlansRepository.setDefaultMembershipPlan` is the corrected shape of the same transaction.

## Moving A Per-Team Default Flag: The Archived-Row Hole And The Zero-Row Rollback

Two tables carry an "exactly one default row per team" flag behind a **partial** unique index that excludes soft-deleted rows: `roles` (`idx_roles_team_default ON roles(team_id) WHERE is_default AND NOT is_archived`) and `membership_plans` (`idx_membership_plans_team_default ON membership_plans (team_id) WHERE is_default AND archived_at IS NULL`). Both move the flag by clearing the team's current default and then marking the new one, inside one `sql.withTransaction`. **Copy `MembershipPlansRepository.setDefaultMembershipPlan`, never `RolesRepository.setDefaultRole`** — the roles version is the older shape and still carries both holes below, unfixed. Do not "align" `membership_plans` back onto it.

Because the index is partial it enforces nothing about soft-deleted rows, so the database cannot catch either mistake for you. The mark statement's own `WHERE` is the only place the guard can live.

1. **The mark UPDATE MUST carry the not-soft-deleted predicate** — `AND archived_at IS NULL` on `membership_plans`, `AND NOT is_archived` on `roles`. Without it, pointing the endpoint at an ARCHIVED row's id clears the team's live default and then successfully flags the archived row: the partial index never sees that row, nothing raises, and the team is left with **zero active defaults**. Every read that resolves "the team's default" then returns nothing, and on `membership_plans` the `AND NOT is_default` archive guard now protects no row, so the team's last active plan becomes archivable. `RolesRepository.setDefaultRole`'s `markDefaultQuery` is `UPDATE roles SET is_default = true, was_default = true WHERE id = $1` — no archived guard. That is the bug, not the pattern.
2. **The mark UPDATE MUST report its affected-row count, and zero rows MUST roll the transaction back.** Clearing and marking in one transaction does not help here: no statement failed, so the transaction commits the cleared default with nothing replacing it — the same zero-defaults end state as (1), reachable by any request that names an id the mark cannot match. `setDefaultMembershipPlan` runs the mark through `SqlSchema.findAll` with `RETURNING id` and fails with the module-private `MarkDefaultNoOp` when `rows.length === 0`; `sql.withTransaction` rolls back on **any** typed failure, not only `SqlError` (verified against `effect@4.0.0-beta.40`). That failure is caught immediately outside the transaction — `Effect.catchTag('MarkDefaultNoOp', () => Effect.succeed(0))` — so it never reaches the caller and the handler still reads a `0` row count as its 404. `setDefaultRole` returns `void` and commits; that is the second unfixed hole.
3. **Clear-then-mark is two UPDATEs, and the transaction opens with `SELECT … FOR UPDATE` over the team's rows.** Both are mechanical consequences of the partial unique index and READ COMMITTED, explained under "The Team's Default Role Resolves In Exactly One Place" rules 4 and 5 above: a single `SET is_default = (id = $1)` transiently holds two flagged rows and trips the index, and without the row lock two concurrent switches both `23505`.
4. **The clear UPDATE is deliberately NOT filtered on the soft-delete column.** `clearDefaultByPlanTeamQuery` clears `is_default` on every flagged row of the team including archived ones, so a stale flag left on an archived row by an older code path is swept up rather than surviving the switch.

## Membership Selection Deadline Lives On `teams`, Not `team_settings`

Slice 2 of "Setup memberships" added `teams.membership_selection_deadline` (migration `1792900000_membership_selection`). It is deliberately on `teams`, never `team_settings`, even though every other per-team configurable value in this codebase lives on `team_settings`.

A `teams` row always exists, so setting the deadline is a plain `UPDATE ... WHERE id = $1` (`MembershipPlansRepository.setSelectionDeadline`). `team_settings` is a one-to-one *extension*, created lazily on first write — and creating that row is not side-effect-free: the RSVP-reminder, claim-request, and coaching-status crons each `INNER JOIN team_settings`, and `rsvp_reminders_enabled` DEFAULTs to `true`. A team that had never opened its settings page would suddenly start receiving Discord RSVP reminders purely as the side effect of a captain setting a membership deadline — a completely unrelated feature silently switching on. Putting the deadline on `teams` (which always has a row) avoids that upsert-with-side-effects trap entirely.

`team_members.membership_plan_id` (same migration) is `NULL` by default and is **never backfilled**. `NULL` means "on the team's default plan" — a *computed* fallback, resolved web-side from the active plan list the client already has for the list page, not a value the server writes in. The same fallback applies when a member's chosen plan is later archived: an archived `membership_plan_id` is not cleared or reassigned, the web treats "archived" the same as "none" and falls back to the default. This is why there is no shared `effectiveMembershipPlan` helper anywhere in the server — there is exactly one call site (`listMembershipPlans`) and it hands back the raw id, never a resolved one.
...
Every team owns a customizable catalogue of event types (migration `1792400000_create_event_types.ts`), seeded six-per-team (`training`/`match`/`tournament`/`meeting`/`social`/`other`). See root `AGENTS.md` → "Event Types" for the full trigger-ownership model (`events.event_type` vs `event_type_id`, why deletes archive, why `position` never resolves a kind). This section covers rules specific to this application.

1. **Route on `kind`, never on `name`.** `name` is nullable free text a team admin can set to anything (`EventTypeName`, 1–50 chars); it is presentation only. Every behavioural branch — bucket routing, training-type/claim gating, workout generation, reminder-lead-time overrides — reads `kind`, which is immutable once a row exists. `api/event-type.ts`'s `UpdateEventTypeRequest` has no `kind` field for exactly this reason: there is no update path that could change it.
2. **`personalChannelBucket.ts` is deliberately untouched by this feature.** `eventBucketSql(eventsAlias)` reads `${alias}.event_type` (the trigger-kept-authoritative kind column), not `event_type_id` or `name` — a custom-renamed "Cup game" type with `kind = 'match'` still buckets into `tournament` exactly like the built-in "Match" type did. `deprovisionableBucketSql` must remain the SAME fragment spliced into both `_getObsoleteBuckets` and `_getGuildsNeedingProvisioning` branch (e); reject any diff to `personalChannelBucket.ts` in an event-types change — if a review touches this file "while they're in there", that is a sign the diff strayed off `kind`.
3. **Only the six render-feeding `EventsRepository` queries carry the `event_types` LEFT JOIN**: `findByTeamId`, `findByIdWithDetails`, `findByChannelId`, `findUpcomingForDashboard`, `findUpcomingByGuild`, `findByUserId` — these need `name`/`color` to render a badge. `findStartable`, `findLoggableTrainingsByGuild`, `_findClaimInfo`, and the claim UPDATE are kind-routed behaviour, not rendering, and were deliberately left alone. Do not reflexively add the JOIN to a query just because it touches `events` — ask whether the caller renders a type badge or only branches on `kind`.
4. **`reorderEventTypes(teamId, ids)` scopes its `UPDATE ... FROM unnest($2::uuid[]) WITH ORDINALITY` by `team_id` in the `WHERE`** — that is the authorization boundary for a client-supplied id array; a foreign id in the array is silently a no-op row, never cross-team-written. The handler (`api/event-type.ts`) additionally rejects the request at 400 (`EventTypeReorderInvalid`) before it reaches the repository when the id list has a duplicate, a foreign/unknown id, or a length that doesn't match the team's current active-type count — the SQL-level scope is defence-in-depth, not the only check.
5. **`deleteEventType` checks `countActiveByTeamId(teamId) <= 1` and fails 409 `EventTypeLastRemaining` BEFORE archiving**, never after — archiving the last active type would make event creation impossible for the team. Archiving is a plain `archived_at = now()` UPDATE; there is no un-archive endpoint (a renamed row also can never return to `name = NULL`, i.e. "reset to default" does not exist).
6. **Handlers must construct `new EventTypeApi.EventTypeInfo({…})` explicitly** — `scripts/check-rpc-encoding.mjs` fails otherwise, and returning a repository row directly type-checks (structurally compatible) but dies at encode. See `toEventTypeInfo` in `api/event-type.ts` for the one conversion point.
7. **`listEventTypes` is membership-only (no `team:manage`)** — deliberately, so the event create/edit pickers on the web (and the bot's autocomplete) can load the catalogue with the caller's own membership check, without a second `team:manage`-gated round trip. Create/update/delete/reorder all require `team:manage`; there is no separate `event-type:*` permission, so — unlike training types and activity types — Captains cannot reach event-type management, only Admins. This is a documented, reversible trade-off (a 4-line grant migration), not an oversight.

## Event Attendance (`event_attendance`, `EventAttendanceRepository`, `api/event-attendance.ts`)

Slice 3a of "Setup memberships" (migration `1793100000_create_event_attendance.ts`). **`event_attendance` is the billing source of truth.** A later slice reads ONLY rows where `confirmed_at IS NOT NULL AND present` — never a live `event_rsvps` response, never the unconfirmed pre-tick this endpoint also shows. Two decisions from the design phase are load-bearing precisely because a fuller version of each was specced and rejected; do not reintroduce either half.

1. **`PUT /teams/:teamId/events/:eventId/attendance` is a full replace, never a stamp-and-forget.** Every candidate the caller was shown is re-sent in one payload; every written row gets `confirmed_at = now()` in the same `INSERT ... ON CONFLICT DO UPDATE`, with the event's own guards (`event_type = 'training'`, `status <> 'cancelled'`, `start_at <= now()`) folded into the INSERT's `SELECT ... FROM events e WHERE e.id = ... AND ...` rather than a preceding check — a check-then-act would let a concurrent cancellation land between the two. Zero rows returned is the merged signal for "the event doesn't qualify anymore" and maps to `409 AttendanceNotConfirmable`. "This event is confirmed" therefore means "it has any confirmed row", unambiguously, because confirmation is all-or-nothing.
2. **The activity-log integration is ONE predicate, not a stamp on confirm.** `EventRsvpsRepository.findYesRsvpMemberIds` (the one thing `TrainingAutoLogCron` reads) gained `AND NOT EXISTS (SELECT 1 FROM event_attendance ea WHERE ea.event_id = ... AND ea.team_member_id = ... AND ea.confirmed_at IS NOT NULL AND NOT ea.present)`. A design that instead had the confirm handler call `EventsRepository.markTrainingAutoLogged` or delete the cron's `source = 'auto'` activity-log row was rejected: stamping `auto_logged_at` on confirm strands logs forever (an all-day training's RSVP window is still open at confirm time, and a member who RSVPs or arrives afterwards would never be logged by anything again), and deleting an auto row is unsafe because the per-day dedup on `activity_logs` means one auto-log can stand for two different trainings on the same day — deleting it for one can destroy the other's legitimate log. The one predicate above is evaluated **at INSERT time** by Postgres itself, which is what closes the confirm-vs-cron race: `TrainingAutoLogCron` reads its member list once via `findYesRsvpMemberIds` and acts on that snapshot, so a flag set later could never retroactively un-select a row already read, whereas a predicate baked into the read itself always sees the latest committed `event_attendance` state, no matter which order the RSVP, the confirm, and the cron tick land in. Do not touch `TrainingAutoLogCron.ts`, `api/player-rating.ts`, or `ActivityLogsRepository._delete`'s `AutoSourceForbidden` guard for this feature — none of the three needs to change, and weakening the guard was one of the rejected designs.
3. **The candidate population's recursive group walk is GUARDED; `EventRsvpsRepository`'s own copy of the same shape is NOT, and that is a known, separate issue — do not "fix" it here.** `EventAttendanceRepository`'s `descendant_groups` CTE follows `GroupsRepository.countMembersForGroup`'s form: `is_archived = false` in both the anchor and recursive terms, `g.team_id = d.team_id` in the recursive term, and `depth < 32`. `EventRsvpsRepository.findNonResponders`/`findYesAttendeesWithLimit` splice the same shape of walk with none of those three guards. Copying the unguarded form into a billing query would make an archived subgroup's members billable while invisible to every other group-scoped surface, and would spin forever on a cycle in `groups.parent_id` (nothing DB-side prevents one). Fixing `EventRsvpsRepository`'s copy is out of scope for this slice — it would change who gets RSVP reminders, a different, already-shipped behaviour.
4. **`GET` is intentionally more permissive than `PUT`.** The read gate is `(event:edit AND (team:manage OR owner-group membership))` OR `finance:view` — the Treasurer holds no event permissions but bills from this data and needs to answer "why was I charged". `canConfirm` in the response is the write gate alone, never widened by `finance:view`. For a non-training event, `GET` returns `200` with `canConfirm: false` and empty `entries` rather than `403`/`404`, because the web loader calls this endpoint for every event on a team's calendar and a 403 there would log a warning on the overwhelming majority of page views.

## Series Times Are Team-Local Wall Clock

`event_series.start_time` and `event_series.end_time` are, **per row**, either a `HH:MM[:SS]` **wall-clock** value in the owning team's `team_settings.timezone`, or a legacy UTC time-of-day — which one is recorded in `event_series.times_are_team_local` (`TRUE` = wall clock, `FALSE` = UTC), not assumed unconditionally. This was a deliberate two-release split (`.work-plans/series-time-conversion.md`): the column was added, defaulted `FALSE`, in `1791700000_add_series_times_team_local_flag.ts`; the conversion of existing rows to `TRUE` — and the write paths that start declaring `TRUE` — was completed by the following release's `1792100000_series_time_is_team_local.ts`, so that a rolling, blue-green deploy never had the previous server image misreading a row the new image had just converted. The split is now complete on both releases. Same underlying wall-clock convention as `team_settings.rules_quiz_time` and `team_settings.rsvp_reminder_time` once a row is `TRUE`. A recurring "Tuesday 18:00" wall clock has no single UTC time-of-day equal to it all year, so treating a `TRUE` row as UTC (or a `FALSE` row as wall clock) is an hour wrong for half the year in every DST zone.

Rules:

1. **Never build an occurrence instant as `` `${dateStr}T${time}Z` ``** (or any other string that pins the series time to UTC) **except through `seriesTimeDialect.resolveSeriesOccurrenceInstant`, whose `FALSE` branch is that exact legacy expression by design** (see the two-release split above). That carve-out did NOT expire when `1792100000` shipped, and never will: a client that omits the `timesAreTeamLocal` payload flag still creates `FALSE` rows after the conversion has run, and the column's `DEFAULT` is still `FALSE` (`1792100000` shipped without flipping it — see `docs/database.md`'s row for that migration), so the `FALSE` branch must never be deleted. Everywhere else, call `resolveSeriesOccurrenceInstant(dateStr, time, timezone, timesAreTeamLocal)` from `src/utils/seriesTimeDialect.ts` — it dispatches per row rather than assuming every row means the same thing, delegating its `TRUE` branch to `resolveOccurrenceInstant(dateStr, time, timezone)` in `src/utils/seriesOccurrence.ts`. That resolver accepts both `HH:MM` and `HH:MM:SS` (a PG `TIME` column can come back with seconds), never throws, and falls back to `'Europe/Prague'` for an unparseable zone. Reference caller: `EventHorizonCron.ts`.
2. **Every query that selects a series time must select the team's zone AND `times_are_team_local` alongside it** — the zone as `COALESCE(ts.timezone, 'Europe/Prague') AS team_timezone`, reusing the `team_settings` join the query already has (see `EventSeriesRepository.findActiveForGeneration`), and `es.times_are_team_local` itself. A series time without its zone is not interpretable; a series time without its dialect flag is not interpretable EITHER — the zone alone does not tell you whether the value is wall-clock in that zone or UTC (see the "Series Times Are Team-Local Wall Clock" note above). Omitting the flag here is how the next instance of this bug gets written: a caller that only has the zone has no way to avoid assuming one dialect for every row.
3. **SQL that re-derives an occurrence instant must use the team's zone on BOTH sides of the round trip** — `((start_at AT TIME ZONE ${tz})::date + ${time}::time) AT TIME ZONE ${tz}`, never a literal `AT TIME ZONE 'UTC'` **except through `seriesTimeDialect.seriesSqlZone(timezone, timesAreTeamLocal)`, which returns exactly `'UTC'` for a `FALSE`-dialect row by design** (see the two-release split above) — `event-series.ts`'s call into `EventsRepository.updateFutureUnmodified` binds `tz` from `seriesSqlZone`, so every `FALSE` row this release resolves through a bound `'UTC'` parameter, not the team's zone, on purpose. Reference: `EventsRepository.updateFutureUnmodified`, whose `timezone` field is required precisely so no caller can omit it. Without this carve-out a reviewer or a future agent "fixes" `seriesSqlZone`'s `FALSE` branch to always pass the team zone and reintroduces the pre-#650 bug for rows that are genuinely UTC-dialect.
4. **`team_settings.timezone` is guarded by `team_settings_timezone_check` (`1792000005_team_settings_timezone_check.ts` — `now() AT TIME ZONE timezone IS NOT NULL`), but still treat it as untrusted.** That CHECK is the WEAKEST of the three validators this column passes through, and all three disagree (verified on `postgres:17` and the repo's `effect` build): `EST` passes the CHECK and `DateTime.makeZoned` but is not a `pg_timezone_names` row; `UTC+3` passes the CHECK but `makeZoned` returns `None`; lowercase `europe/prague` passes the CHECK and `makeZoned` but fails the case-sensitive `pg_timezone_names` join. The HTTP validator `isValidIanaTimezone` (`packages/domain/src/api/TeamSettingsApi.ts`) is a `new Intl.DateTimeFormat` probe, which also accepts `EST` and `europe/prague` — so a value stored through the API can still be one the SQL join falls back on. Therefore keep BOTH fallbacks: in TypeScript, `DateTime.makeZoned` returns `None` for an unknown zone — fall back to `'Europe/Prague'`, never throw on a cron or render path. In SQL, use the `pg_timezone_names` join described in `packages/migrations/AGENTS.md` → "Reading A Per-Team Setting Inside A Migration `UPDATE`". A write the CHECK rejects fails with SQLSTATE `22023` (`time zone "..." not recognized`), NOT `23514` — never detect it by matching a check-violation code.
5. **A team-timezone change must re-anchor already-materialized future occurrences — and the date-recovery expression is deliberately NOT shared with the migration's.** `src/api/team-settings.ts` re-derives `start_at`/`end_at` for events matching `series_id IS NOT NULL AND NOT series_modified AND status = 'active' AND start_at >= now()`, reading the calendar date in the OLD zone and resolving it in the NEW one. Keep those four guards identical to `1792100000`'s Statement B, **plus a fifth: `AND es.times_are_team_local`** — a `FALSE` (UTC-dialect) series' `start_time` is an absolute UTC time-of-day, so a team-timezone change is a no-op for it, exactly as it was before #650, and re-deriving it here would wrongly reinterpret a UTC value as wall clock. Statement B carries a fifth predicate too, but a *different* one — `es.id = ANY(convertedIds)`, restricting it to the series Statement A itself just converted, not `AND es.times_are_team_local`. The governing principle (`.work-plans/series-time-conversion.md` §B.1) is: **recover the occurrence date in the dialect the event was materialized in.** Statement B, by construction, only ever sees UTC-dialect events (built as `` `${dateStr}T${time}Z` ``), so it recovers the date as `(start_at AT TIME ZONE 'UTC')::date`. `team-settings.ts` only ever sees already-team-local events (its own `AND es.times_are_team_local` guard), built as `(dateStr + time) @ tz`, so it correctly recovers the date as `(start_at AT TIME ZONE oldTz)::date`. These are the same rule applied to two disjoint populations, not the same expression — **do not align them**: making either site copy the other's date expression would silently re-anchor its population on the wrong calendar day. The four *shared* guards (`series_id`, `NOT series_modified`, `status = 'active'`, `start_at >= now()`) exist to avoid clobbering per-occurrence captain edits, cancelled events, and the past, and must stay identical between the two sites even as the date expressions differ.
6. **A write path must carry the dialect explicitly — never hardcode it, never translate between dialects.** The invariant "every `FALSE` row is genuinely UTC-semantics" (NOT "every row is `FALSE`") mattered for `1792100000`'s `WHERE NOT es.times_are_team_local` conversion guard, and matters the same way for any future conversion of this shape. Create (`src/api/event-series.ts` → `EventSeriesRepository.insertEventSeries`) therefore writes `payload.timesAreTeamLocal` into `times_are_team_local` verbatim and names the column in the `INSERT` — never substitute a hardcoded `FALSE`, and never rely on the column's `DEFAULT FALSE` here (the default is unconditional and does not know which dialect a given write intends). Update never rewrites the stored dialect: it calls `assertDialectMatches(payload.timesAreTeamLocal, existing.times_are_team_local)` from `src/utils/seriesTimeDialect.ts`, failing with `EventSeriesApi.EventSeriesTimeDialectMismatch` (HTTP 400), and calls it ONLY when the payload actually supplies a time — gate on `Option.isSome(payload.startTime) || Option.exists(payload.endTime, Option.isSome)`, evaluated BEFORE either field is resolved with `Option.getOrElse(..., () => existing.*)`. A patch that omits both time fields asserts no dialect and MUST pass through untouched, because `timesAreTeamLocal` is not `Option`-wrapped and therefore always encodes on the wire (a location-only edit from any client always sends some value for it).
...
| `EventStartCron` | `src/services/EventStartCron.ts` | every minute | Mark active events as `started` when `start_at <= NOW()`, increment `team_members.missed_rsvps` for non-responders (see "Idempotent Counter Increment Folded Into the `active`→`started` Status Flip"), emit `event_started` sync events. **Two clock sources, deliberately.** The two deferred all-day sweeps take an **injected instant** — `makeEventStartCronEffect(now)` passes one `now: Date` to both `findAllDayEventsPastLastLocalDay(now)` and `findAllDayEventsNeedingStartedPost(now)`; the flip path (`findEventsToStart`, `startEvent`) reads the Postgres clock. Do NOT unify them: `findStartable`'s `start_at <= NOW()` and `startEvent`'s `SET missed_rsvp_counted_at = … now() …` arming stamps must agree with **each other inside one statement**, which only the DB clock guarantees — an injected JS instant would let an event be selected against one clock and stamped against another. See "A Cron's Test Fixtures And The Cron Must Share ONE Instant". |
...
## Fio API Client — The Token Is In The URL Path (`src/services/FioApiClient.ts`)

The token travels as `Redacted.Redacted<string>` from `FioSecretCrypto.decrypt` (which shares `src/services/secretBox.ts`'s AES-256-GCM primitives with `EmailSecretCrypto` but keeps its own `FIO_TOKEN_ENCRYPTION_KEY` and its own error tags) all the way to the URL, and `FioApiClient` is the only module that may call `Redacted.value()` on it — apart from `api/bank-sync.ts`, which unwraps the treasurer's submitted token once to encrypt it. Five constraints are counter-intuitive enough that a reasonable-looking edit breaks them without failing a test.

1. **Provide `HttpClient.TracerDisabledWhen` by wrapping the client VALUE with `HttpClient.transform`, never on the layer that constructs the client.** `HttpClient` reads `fiber.getRef(TracerDisabledWhen)(request)` at **execute** time, from the *caller's* fiber — a `Layer.provide` under the construction layer is not in that fiber's services and is silently ignored, leaving `span.attribute('url.full', …)` set unconditionally. Fio puts the 64-character token in the URL **path** and `OTEL_EXPORTER_OTLP_ENDPOINT` is a required env var, so the failure mode is the live bank credential in plaintext in SigNoz. The predicate must be a string-prefix test (`isFioUrl`), not `new URL(req.url).hostname` — a relative URL makes the `URL` constructor throw, and that throw is a defect raised inside `Effect.withFiber`, not a typed failure.
2. **Nothing obtained from `HttpClient` may escape the client, and that includes the body decode.** `TransportError` is a `reason` class nested inside `HttpClientError`, not a top-level error tag, so `Effect.catchTag('TransportError', …)` never fires; `catchTag('HttpClientError', …)` plus a final `Effect.catchCause` (re-raising interruption-only causes) is the only containment that holds. `HttpClientError.message` derives from `reason.methodAndUrl` — the tokenised URL — and its constructor lifts `reason.cause` onto itself, so `Cause.pretty` prints the token twice. `StatusCodeError`, `DecodeError` and `EmptyBodyError` carry it too, so the decode runs **inside** the boundary. No typed `Fio*` error carries the original as `cause`; they carry an endpoint name and a team id and nothing else.
3. **The one-request-per-token-per-30-seconds limit is enforced in SQL (`fio_token_throttle`), not in process memory.** A `Semaphore`/`Ref` lives in one Node process; two replicas — or the overlap of any rolling deploy — then both call inside 30 s and neither converges. The upsert *reserves* a slot before sleeping and `RETURNING`s a **duration**, never a DB timestamp: the caller sleeps on the replica's clock, so a timestamp would be wrong by exactly the clock skew. It MUST run as its own autocommitted statement — **no caller may wrap a Fio fetch in `sql.withTransaction`**, or the `ON CONFLICT` row lock is held across both the sleep and the HTTP call and a second replica blocks on it, which is the failure the SQL throttle exists to prevent. The same UPSERT exists twice — `FioApiClient.makeReal`'s copy is the one wired to production callers, `BankSyncConfigRepository.reserveThrottleSlot` exists only for the tests that pin the autocommit contract — and the `Math.ceil` round-up-never-down must stay identical in both. The `maxWaitSeconds` budget guard on the conflict `WHERE` clause is the one **deliberate** divergence: it lives in `FioApiClient` only, because it needs to surface a `FioRateLimited`-shaped "busy beyond budget" verdict the repository has no caller for. Do not "fix the drift" by mirroring it; add it to the repository copy only when a real caller needs that verdict.
4. **A dead token answers with a bodyless HTTP 500 — there is no 401 and no 403 — and `500` is the ONLY status that means that.** Every Fio error body is empty (`content-length: 0`), so `decodeContained` dispatches on the status code alone and never parses an error body; the tag it picks is the only information anyone downstream will ever have. The map is closed and exhaustive:

   | Status | Tag | Retried? |
   |--------|-----|----------|
   | `409` | `FioRateLimited` | **Yes** — the only retried status (`Schedule.exponential`, 2 takes), and only when `retryRateLimited` is left at its `true` default |
   | `413` | `FioTooManyMovements` | No |
   | `422` | `FioHistoryLocked` | No |
   | `404` | `FioBadRequest` | No |
   | `500` | `FioServerError` — "Fio judged this token dead" | **Never** — a blanket retry-on-5xx hammers Fio forever on a revoked token |
   | any other non-`200` | `FioUnreachable` | No |
...
1. **Never name a base-14 font.** `src/services/BankStatementPdf.ts` registers the vendored `src/assets/fonts/NotoSans-Regular.ttf` / `NotoSans-Bold.ttf` and calls `doc.font(...)` before **every** `.text()` — a font set once does not survive `.addPage()` on all pdfkit paths.
2. **Resolve the file with `new URL('../assets/fonts/…', import.meta.url)`** so the same relative path works under `tsx src/run.ts` and under `build/esm`. Never a system font path: the production image is `node:25-slim` and ships no fonts at all, so there is no fallback to mask a wrong path in prod — while a dev machine that happens to have fonts installed masks it completely.
3. **`tsc` does not copy `.ttf` (or any non-`.ts` asset).** A new file under `src/assets/` needs the `build` script's `node scripts/copy-assets.mjs` step, and `postbuild`'s `node scripts/assert-dist.mjs` must assert it reached `build/esm/` (it also enforces a minimum byte size, which catches a truncated or empty copy).
...
- **`BIGINT` columns read into an `SqlSchema` `Result` schema** — node-postgres registers no `int8` type parser, so `@effect/sql-pg` hands the value back as a JS **string**. Decode it with `Fee.AmountMinor` (or another `Schema.Union([Schema.Number…, Schema.NumberFromString…])`), or cast the column `::int` in the `SELECT`. Plain `Schema.Number` compiles, then fails decode on **every** row at runtime, and `catchSqlErrors` turns that `SchemaError` into a `LogicError` **defect** — so the symptom is a generic user-facing error (`bot_finance_error_generic`, an untyped 500), never a type error. `Request` schemas are unaffected: they encode. References: `FinanceOverviewRepository.myStatusQuery` decodes `due_minor`/`paid_minor` (from view `fee_assignment_status_v` over `fee_assignments.amount_minor`/`paid_minor`) with `Fee.AmountMinor`; the sibling `overviewByTeamQuery` may keep `Schema.Number` only because every money column there is cast `::int`; `BankSyncConfigRepository`'s `StatementPeriodResult` uses the union.
...
- **Cast a bind parameter explicitly whenever the surrounding SQL gives Postgres nothing to infer its type from.** A parameter compared against a typed column (`col = ${p}`) is inferred and needs nothing; a parameter used only in `${p} IS NULL`, in arithmetic against an `interval` literal (`${p} * interval '1 second'`), or in `${p} || ${q}` is not. Postgres fails the statement at **Parse** time with `could not determine data type of parameter $N`, before any row is touched, so the query fails on *every* call rather than on an edge case — and `catchSqlErrors` turns it into a `LogicError` **defect**, which surfaces as an untyped 500 or a crashed cron, never as a message naming the parameter. Write `${p}::int`, `${p}::text`, `${p}::timestamptz`. Repeat the cast at every occurrence of the same value; one cast does not type the other placeholders. Reference: `src/services/FioApiClient.ts`'s throttle-reservation UPSERT (`${maxWaitSeconds}::int` in both the `IS NULL` test and the interval multiplication).
...
7. **`RolesRepository.findRoleById(roleId)` has NO team filter — every handler that resolves a role by id MUST guard `role.team_id !== teamId` itself.** This is the one table where rule 4's `findByIdScoped` does not exist and is not being added: `roles` is read by id from six handlers in `src/api/role.ts` (`getRole`, `updateRole`, `deleteRole`, `assignRole`, `unassignRole`, `setDefaultRole`), plus `GroupApi.assignGroupRole` (rule 6). Each one places `Effect.tap(({ role }) => role.team_id !== teamId ? Effect.fail(new RoleApi.RoleNotFound()) : Effect.void)` immediately after the `Option.match` that unwraps the lookup and BEFORE any other check on the row. A seventh role endpoint that omits it lets an admin of team A read, rename, delete, assign or default-mark a role of team B. `RoleNotFound` (404), never `Forbidden` — a foreign role's existence is not this team's business (rule 2).
...
5. Emit `member_removed` channel-sync events for every roster and every group (including ancestor groups via `getAncestorsIncludingArchived` — deliberately archived-blind, see rule 3).
...
The cascade guards the "team must keep ≥1 active manager" invariant with `SELECT pg_advisory_xact_lock(hashtext(${teamId}))` as the FIRST statement inside `sql.withTransaction(...)`. Without it, two managers leaving simultaneously could both pass `hasOtherActiveManager` (each still sees the other as active) and both deactivate, orphaning the team with zero managers. The lock serializes all cascades for the same team so the count check and the deactivation are effectively one atomic step. When you add any transaction whose correctness depends on a check over team-wide state that a concurrent transaction could invalidate, acquire the same per-team `pg_advisory_xact_lock(hashtext(teamId))` before the check.

`src/api/group.ts`'s `moveGroup` is the second reference case, and it carries two lessons the cascade does not:

1. **The lock key defines the invariant's scope, so every edge the check walks must stay inside it.** `moveGroup`'s guard walks `groups.parent_id`; a parent in another team would sit under a different `hashtext(teamId)` and two concurrent moves could still close a cycle across the two trees. `requireSameTeamParent` therefore runs on BOTH `parent_id` writers — `createGroup`'s insert as well as `moveGroup`'s update — because a cross-team edge planted at creation is one a later same-team move extends into exactly that cycle. Guarding only the racy writer is not enough.
2. **On an interactive endpoint, precede the lock with `SET LOCAL lock_timeout`.** node-pg cannot cancel an in-flight query, so a fiber interrupted while parked on the lock still pins its pool connection (default max 10) until the lock is granted; there is no `statement_timeout` configured anywhere in this repo. `moveGroup` is currently the ONLY holder of this key that sets the timeout — the other four (`src/utils/deactivateMemberCascade.ts`, `reconcileRosterRoleExtras.ts`, `backfillGroupRoleMembers.ts`, `backfillRosterRoleMembers.ts`) set none, and that is NOT evidence they are all background sweeps that can block forever: `deactivateMemberAndCascade` is reached from the interactive `deactivateMember` endpoint (`src/api/roster.ts`), a pre-existing gap. Any new request-path transaction that takes this key gets the `SET LOCAL lock_timeout` line.
...
Rules:
1. Always include every business precondition in the `WHERE` clause — never read-then-update across two statements. Under READ COMMITTED this is not a style preference: an UPDATE that blocks on a concurrent writer's row lock re-evaluates its OWN `WHERE` against the row version that writer committed, so a precondition written into the `WHERE` is re-checked after the wait, while one checked by a preceding `SELECT` is not and the UPDATE applies anyway. `MembershipPlansRepository.archiveMembershipPlan` depends on exactly this — `AND archived_at IS NULL AND NOT is_default` lives in the UPDATE, so a concurrent `setDefaultMembershipPlan` that promotes the plan while the archive waits makes the archive match zero rows; a read-then-check would have archived the team's brand-new default
...
3. Used by: `EventsRepository.claimTraining` / `unclaimTraining` (claim race), `EventsRepository.markEventStarted` (start race), `EventRsvpsRepository` upserts, `DiscordChannelMappingRepository.saveClaimThreadIfAbsent` (claim-thread create race; `WHERE claim_thread_id IS NULL` — `None` means another request won, so re-read instead of mapping to an error), `MembershipPlansRepository.archiveMembershipPlan` (archive vs. concurrent default-switch; affected-row count `0` → 404)
...
5. **Locks a trigger takes on your behalf still count, and the driving SELECT must be ordered by the column the TRIGGER will lock on.** `BankTransactionMatcher.unmatch` voids a transaction's payments `ORDER BY fee_assignment_id ASC, id ASC` — deliberately not by payment id — because each `void_` fires `payments_finance_recompute`, which locks that payment's `fee_assignments` row. Payment ids are random UUIDs, so voiding in payment-id order acquires the assignment locks in an arbitrary order and two concurrent `/unmatch` calls spanning the same two assignments deadlock (`40P01`) on a money write. The transitive locks are invisible at the call site; check the triggers on every table you write. `member_credit_deposits` joined that list: `credit_deposits_bank_recompute` (migration `packages/migrations/src/before/1792800001_auto_credit_bank_transfers.ts`) locks the deposit's `bank_transactions` row, so every writer of a deposit must already hold the member's `member_credit_accounts` lock (see rule 6) before the deposit INSERT/UPDATE.

6. **`member_credit_accounts` before `fee_assignments`, and every caller that can touch a credit account is in the lock order.** Four paths take a `member_credit_accounts` row lock (`FOR UPDATE OF a`, team-scoped through `team_members`): `MemberCreditsRepository.settle` (account lock, then `fee_assignments`, then the debit/credit writes — settle-all and pay-in-advance are one code path), `MemberCreditsRepository.voidDeposit` (account lock before the deposit read), `PaymentsRepository.void_` (account lock before the `payments` row, so it can restore credit when a `'credit'`-method payment is voided — re-entrant with `BankTransactionMatcher.unmatch`, which already holds the same lock when it calls into `void_`), and `BankTransactionMatcher.writeAutoCredit` (account lock, then `bank_transactions`, then `fee_assignments`). All four lock a single account row, never a bare `FOR UPDATE` that would pull `team_members` into the order.

7. **When a second write path needs an existing path's allocation under a DIFFERENT lock order, share the PURE algorithm module — never call the existing repository method.** `BankTransactionMatcher.writeAutoCredit` allocates a bank transfer across a member's outstanding assignments exactly as `MemberCreditsRepository.settle` does, and must NOT delegate to it: `settle` acquires `member_credit_accounts` → `fee_assignments` with no `bank_transactions` leg, so the bank-linked `payments` rows inserted under it would fire `payments_finance_recompute` and take `bank_transactions` AFTER `fee_assignments` — deadlocking (`40P01`) against `BankTransactionMatcher.writeAutoMatch`. `writeAutoCredit` re-implements only the acquisition (`member_credit_accounts` → `bank_transactions` → `fee_assignments`) and shares `SettlementPlan.planSettlement` (`packages/domain/src/models/SettlementPlan.ts`), so the arithmetic and the ordering stay identical to `settle` and to the client's settlement preview. It passes a credit pool of **`0`**: the transfer allocates its own money only. `settle` is the one path that SPENDS credit; `writeAutoCredit` only ADDS it — draining a pre-existing balance here would make `/unmatch` refund credit the transaction never created. `writeAutoCredit`'s candidate query MUST keep the same predicate as `settle`'s `lockAssignments` (active, unarchived, `amount_minor > paid_minor`, matching currency, `ORDER BY fa.id ASC`, `FOR UPDATE OF fa`) so both money paths see the same candidate set.

8. **Unmatching a transaction must undo its credit deposits as well as its payments, and the account-lock hoist is keyed off BOTH sets.** `BankTransactionMatcher.unmatch` reads its `payments` ids AND its active `member_credit_deposits` ids (both plain `SELECT`s, `ORDER BY id ASC`) before hoisting the account locks, and the hoist's `UNION` covers both. A pure top-up — the member owed nothing, so the whole transfer became credit — has ZERO payments and one deposit, so the old `paymentIds.length === 0` skip would have bypassed the hoist entirely. The deposit void is a single conditional `UPDATE` guarded by `a.balance_minor >= d.amount_minor`, never SELECT-then-UPDATE: credit is fungible, so an already-spent balance must become the typed `BankSyncApi.UnmatchCreditSpent` (409) rather than a `balance_minor >= 0` CHECK violation that aborts the whole transaction as an untyped `SqlError`.

Reference: `CarpoolsRepository.reserveSeat` / `removeCar` / `addCar`, `PlayerRatingsRepository.applyGameUpdates`, and `MemberCreditsRepository.settle` / `voidDeposit` (`src/repositories/`).
...
4. **A mock for a repository method whose result is defined by production SQL must model ONLY the property under test — never re-implement the query.** When a unit test needs a repository whose real behaviour is a recursive CTE, a `UNION`, or an archived/active filter, the mock must implement the smallest rule that makes the test's assertion observable, and must carry a comment saying (a) what it deliberately does NOT model and (b) which integration test covers the real semantics. Reference: the `groupRoles` map in `test/rpc/RegisterMember.test.ts` is a flat `group -> roles` lookup with no ancestor walk and no `is_archived` handling, because that file only proves the group bind happens BEFORE the role diff; the real walk is asserted only against a live database (`test/integration/repositories/TeamMembersRepository.groupRoles.test.ts`, `middleGroupArchivedChain.test.ts`, `test/integration/rpc/registerMemberGroupInviteRoleSync.test.ts`). A mock that grows into a second implementation of the fragment drifts from it silently and turns green while production is broken — mock-vs-SQL drift has already produced a bug in this exact area. **Any assertion about a query's real semantics belongs in `test/integration/`, never in a mocked unit test.**
5. **Mock objects must build the canonical domain model via its constructor, not as a plain camelCase literal.** `new WeeklyChallenge.WeeklyChallengeView({ challenge: new WeeklyChallenge.WeeklyChallenge({ ... }), completedMemberIds: [], isActive: false })` — NOT `{ challenge: { id: '...', weekStartDate: '...' }, ... }`. The HTTP handler encodes the response through the schema; a string-shaped literal fails encoding and tempts a "fix" that bypasses schema encoding entirely. Build mocks via the same constructors the production code uses.
6. **Every repository mock layer is cast (`as any` / `as never`, rule 3), so RENAMING or RESHAPING a repository method produces ZERO compile errors.** `TeamMembersRepository` alone is mocked at 97 `Layer.succeed(TeamMembersRepository, { … } as any)` sites across `applications/server`; not one of them type-checks against the real service. A method the mocks still expose under its old name (or with its old result shape) fails only at runtime, in a test that was passing, with a message pointing at the consumer rather than the mock. **Any change to a repository method's name, parameter list or result shape therefore requires running the FULL server unit suite (`cd applications/server && pnpm test`), never a hand-picked file list** — `grep -rl '<oldMethodName>' applications/server/test` finds the mocks that must be updated, and only the full run proves none were missed. Reference: renaming `getPlayerRoleId` → `getDefaultRoleId` touched 71 server test files that all compiled clean beforehand.

### A Cron's Test Fixtures And The Cron Must Share ONE Instant

An integration test for a cron MUST evaluate the cron against the **same instant** its fixtures were built from. `test/integration/services/EventStartCron.deferred.test.ts` used to build fixtures from the Postgres clock (`SELECT now() ± N minutes`, squeezed back into the local day by a `localTimeOffsetClampedToDay` helper) and then run a cron that sampled `new Date()` a moment later. Those are two different instants, and any team-local date boundary landing between them flips a case's expected outcome: an unclamped `now + 60 minutes` sampled at 23:30 local yielded `00:30`, which `findAllDayEventsNeedingStartedPost`'s `(now AT TIME ZONE tz)::time >= COALESCE(ts.all_day_post_time, TIME '08:00')` reads as ALREADY PAST — the "must not post yet" cases fired and the suite flaked nightly.

`fix/event-start-cron-injectable-clock` makes the instant a parameter. `makeEventStartCronEffect(now: Date)` (`src/services/EventStartCron.ts`) threads ONE `now` into BOTH deferred sweeps — `findAllDayEventsPastLastLocalDay(now)` and `findAllDayEventsNeedingStartedPost(now)` — replacing the two independent `new Date()` calls that used to sit there; `export const eventStartCronEffect = Effect.suspend(() => makeEventStartCronEffect(new Date()))` is the production entry point. `localTimeOffsetClampedToDay` no longer exists in this tree — do NOT reintroduce it or any clamp-based variant.

Rules:

1. **Make the cron's instant an injected parameter, and never sample a second one inside the cron.** Export `make<Name>CronEffect(now: Date)` for the tests plus `export const <name>CronEffect = Effect.suspend(() => make<Name>CronEffect(new Date()))` for production. The `Effect.suspend` is load-bearing: without it the instant freezes at module-load time instead of being sampled per run (`test/services/EventStartCron.test.ts` U2 asserts exactly this, with `vi.useFakeTimers({ toFake: ['Date'] })`). Taking `now` as a parameter rather than reading a clock inside makes "one instant per cycle" structural — there is no second place to sample. Inject ONLY where a test needs determinism; see the `EventStartCron` row of the Cron Jobs table for why `findEventsToStart` / `startEvent` deliberately stay on the Postgres clock.
2. **Pin ONE permanently-past literal in the test and derive every fixture timestamp from it as a SQL bind parameter.** `const FIXED_NOW = new Date('2025-10-15T10:00:00.000Z')` (= 12:00 Europe/Prague, CEST) is passed to `makeEventStartCronEffect` AND is the origin of every fixture date: `localMidnight(tz, dayOffset)` binds `FIXED_NOW.toISOString()` and casts it in SQL (`(${FIXED_NOW.toISOString()})::timestamptz AT TIME ZONE ${tz}`). **Never `SELECT now()` in a fixture for a cron whose instant is injectable** — that reintroduces the second instant this design removed. Read the `FIXED_NOW` header block (`test/integration/services/EventStartCron.deferred.test.ts:54-91`) before adding a case to that file; it enumerates exactly what still reads the real Postgres clock.
3. **The defect was two INSTANTS, not two clocks.** The fixture samples at T0, the cron at T1, and a date boundary can fall in between. This is true regardless of whether T1 came from `new Date()`, an Effect `Clock`, or `SELECT now()` — swapping one clock source for another fixes nothing; sharing one instant does. The fix is **test-determinism only, with zero production behaviour change**: production still passes a fresh `new Date()` per cycle, and a long cycle now evaluates both deferred sweeps against the cycle-start instant (worst case a sweep slips one 60-second cycle, strictly better than two independent samples).
4. **Never write a one-sided `if (nowLocal >= boundary) expect(...)` guard** — that form silently asserts nothing for the whole complementary window. Assert both sides of the boundary and leave only the guard band unasserted. This rule is mechanism-independent and still binding; with a pinned instant it is now trivially satisfiable rather than merely aspirational. It is the rule that produced case 8b's guard band, and case 8b now asserts unconditionally (`expect(emitted).toBe(1)`) because `FIXED_NOW` at 12:00 Prague is unconditionally past the 08:00 fallback.
5. **Never let a boundary case fall through to a `COALESCE(<column>, <literal>)` default — set the column explicitly.** Every I-series case calls `setAllDayPostTime(team.id, '00:00:00' | '02:20:00')` for this reason. And note the silent half: a bare `UPDATE team_settings SET <col> = … WHERE team_id = …` affects **ZERO rows** when the team has no `team_settings` row, without failing or warning — so a forgotten `setTeamTimezone` (which is what creates the row, via `TeamSettingsRepository.upsert`) silently reverts the case to the `COALESCE` default instead of failing. An earlier draft of the I-series was unlandable for exactly this: the 08:00 `all_day_post_time` default made a midnight-boundary case fail for an unrelated reason. Only cases 8/8b may omit both calls — the missing `team_settings` row IS their subject.
6. **Do NOT reach for `effect/testing/TestClock` in a DB-backed integration test.** Two independently reproduced deadlocks: `test/integration/services/BankSyncPoller.test.ts:536-546` and `test/integration/services/FioApiClient.test.ts:312-326` — a `SqlSchema` call chained ahead of an `Effect.sleep` inside one forked fiber leaves that sleep permanently unresolved no matter how far the virtual clock is advanced (both tests escape via `TestClock.withLive`). Separately, `@effect/vitest`'s `it.effect` auto-provides a `TestClock` starting at **epoch 0**, so making a cron read the Effect `Clock` silently moves every existing `it.effect` in that file to 1970. Both are why the injected-parameter design was chosen over `Clock`. `test/integration/services/EventStartCron.deferred.test.ts` uses no `TestClock` anywhere.
...
The sibling failure mode — a fixture computed FROM `now` that wraps across a local-midnight or DST boundary — is the preceding section, "A Cron's Test Fixtures And The Cron Must Share ONE Instant". Read both before writing any date fixture.
