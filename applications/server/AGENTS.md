...
2. **Guarded today:** the ancestor walk in `src/repositories/effectiveRoles.ts`, `GroupsRepository.findAncestors`, `GroupsRepository.findActiveAncestors`, `GroupsRepository.findDescendantMembersWithDiscordIdQuery`, `GroupsRepository.countMembersForGroup`, `GroupsRepository.findDescendantMembers`, `GroupsRepository.findActiveGroupsWithAncestorsForMemberQuery`, `GroupsRepository.findByTeamId`'s `group_tree` walk, `EventAttendanceRepository.findByEventId`, and all three `descendant_groups` walks in `EventRsvpsRepository.ts`. **Not yet guarded:** the four `descendant_groups` walks in `PersonalEventChannelsRepository.ts`, `src/rpc/event/index.ts` (both the rows and the `total` count), `src/rpc/guild/index.ts`, and `EventsRepository.findScopedTrainingTypeIds`' lateral `ancestors` walk. Add the guard to any of those the next time you touch one — do not leave a walk you edited unguarded.
3. **The worst remaining gap is `PersonalEventChannelsRepository._getGuildsNeedingProvisioning` (and its sibling branch).** Its `Request` is `{ limit }` — no team scope — and it scans `FROM teams t` with a correlated recursive CTE per candidate. ONE cyclic `parent_id` in ANY team therefore hangs personal-channel provisioning for EVERY guild on the instance, not just that team's. Fix the four walks in that file as one unit: provision, deprovision and both poll branches must stay semantically identical, or the poll stops waking for work that the other half still wants to do.
