...
2. **Guarded today:** the ancestor walk in `src/repositories/effectiveRoles.ts`, `GroupsRepository.findAncestors`, `GroupsRepository.findActiveAncestors`, `GroupsRepository.findDescendantMembersWithDiscordIdQuery`, `GroupsRepository.countMembersForGroup`, `GroupsRepository.findDescendantMembers`, and `GroupsRepository.findActiveGroupsWithAncestorsForMemberQuery`. **Not yet guarded:** `GroupsRepository.findByTeamId`'s `group_tree` walk (the group-list endpoint — the ONE walk left unguarded in that file), `EventsRepository.findScopedTrainingTypeIds`' lateral `ancestors` walk, and the `descendant_groups` walks in `EventRsvpsRepository.ts`, `PersonalEventChannelsRepository.ts`, `src/rpc/event/index.ts`, `src/rpc/guild/index.ts`. Add the guard to any of those the next time you touch one — do not leave a walk you edited unguarded.
