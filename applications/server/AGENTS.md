...
5. **`GuildsRpcLive`'s top-level `Effect.Do.pipe` is at 19 of a maximum 20 arguments** (`Pipeable.d.ts:40`, the last typed overload is `ab`…`tu`). A 21st argument silently drops to the untyped rest overload and degrades every `deps` destructure to `unknown` — the failure mode `reconcileMemberDiscordRoles.ts:83-86` already documents defensively. Resolve a newly-needed repository inline with `<Repository>.asEffect()` inside the function that needs it (as `applyInviteGroup` does for `ChannelSyncEventsRepository`), or split the chain. Do not append a 20th/21st top-level bind.
6. **A third producer of group `member_added` exists, and it is sighting-driven, not invite-driven** (`utils/emitMemberGroupChannelRoles.ts`, wired into `observeGuildMembership`). Where `applyInviteGroup` requires a resolved invite context and `setupNewMember`'s channel-mapping-derived add requires the member already holding the Discord role, this one runs on every `GUILD_MEMBER_ADD` observation (`payload.source === Some('member_add')`) and diffs the member's `findActiveGroupsWithAncestorsForMember` set against `payload.roles`, independent of any invite — this is what fixes a member who joins Discord manually or past the invite's 15-minute acceptance window (`InviteAcceptancesRepository.ts:343`). Rules that apply specifically to it:
   ...
   - **Its call site MUST keep the `Effect.catchCause` + `logWarning` wrap.** `emitMemberGroupChannelRoles` declares `Effect.Effect<…, never, …>`, but every repository call inside it pipes `catchSqlErrors`, so a transient database failure is a DEFECT, not a typed error — see root `AGENTS.md` → "Effect-TS Patterns" → "Error Handling" → "Rules" rule 8. Unwrapped, that defect short-circuits the `Effect.tap` it sits in and skips both `reconcileMemberDiscordRoles` and the welcome message sequenced after it, i.e. an additive heal would break pre-existing join behaviour. Same convention as `reapplyGroupGrants` at `rpc/channel/index.ts:220-221` and `:245-246`.
   - **`MAX_GROUP_CHANNEL_EMISSIONS_PER_MEMBER = 25` is a PERMANENTLY LOSSY cap on this path, unlike every other emission budget in this codebase.** `MAX_ROLE_SYNC_EMISSIONS_PER_GUILD_RECONCILE` (`utils/reconcileMemberDiscordRoles.ts:18`) only defers work — the next reconcile re-derives the diff. This producer fires exactly once per member, on their join, so anything past the cap is never re-derived and never emitted. That is why `findActiveGroupsWithAncestorsForMember` carries `ORDER BY min(depth), name, id` (nearest-first, deterministic): the groups kept when the cap bites must be the member's own groups and closest ancestors, never an arbitrary slice of an unordered result. Never remove that `ORDER BY`, and never raise the cap without first giving this path a re-derivation trigger.
...
Two facts worth keeping in mind when touching this handler: the role diff is bot-version-gated — `payload.source: None` (a pre-PR-8 bot that doesn't send `source`) skips `observeGuildMembership` entirely, so an old bot never triggers a diff at all. And nothing in `registerMemberWithReconcile` is transactional: the group insert, the `channel_sync_events` insert, the `role_sync_events` inserts, and `markDiscordJoined` are four independent commits (the file's only `sql.withTransaction` is elsewhere, unrelated to this path). Fixing the ordering buys correctness of what gets written when everything succeeds — it does not buy atomicity if one of the four fails partway through.
...
> **DORMANT as of `fix/discord-roles-sync`.** A Sideline role is a PERMISSIONS construct — it grants `role_permissions` and nothing else — and is never mirrored into a Discord guild role. `RoleSyncEventsRepository`'s emit functions are all no-ops and `findUnprocessed` drains nothing, so everything described below still computes its diff but enqueues nothing and the bot's `rcp/role/*` worker never sees an event. Discord roles come from the three subsystems that model actual membership, none of which route through this file: **groups** and **rosters** (`channel_sync_events` → `discord_channel_mappings`, bot `rcp/channel/*`) and **achievements** (`role_provision_events`, bot `rcp/roleProvision/*`). The rules below are retained because the diff code still ships and is still read; deleting it, the bot worker, the `role_sync_events` table and the `syncMemberDiscordRoles` endpoint is a follow-up ticket. **Do not "fix" the no-op by reinstating a write** — a request for "a Discord role for these members" is asking for a group or a roster.
...
- **10011 (Unknown Role) healing clears the role column, it never deletes the mapping row, and only when a channel still exists.** `channelUtils.ts`'s `clearStaleRoleOnUnknownRole` calls the new RPC `Channel/ClearMappingRole` → `DiscordChannelMappingRepository.clearGroupRole`, which is gated `WHERE ... AND discord_channel_id IS NOT NULL`. This deliberately differs from the role-axis precedent `clearStaleMappingOnUnknownRole` (`applications/bot/src/rcp/role/handleAssigned.ts:93-95`), which `Role/DeleteMapping`s the whole `discord_role_mappings` row on the same 10011 signal — safe there because that table holds nothing but the role. `discord_channel_mappings` is a shared channel+role row, so deleting it here would also destroy a still-valid channel link. Two reasons the column-clear is gated on `discord_channel_id IS NOT NULL`, both required:
...
2. **Both queries splice `holdsDefaultRoleWhere` from `effectiveRoles.ts` and must stay identical** between the read query and the increment query — they target the same population, so a divergence would increment a counter for a member who never appears in the reminder list (or vice versa). That fragment is shared with `TeamMembersRepository.getDefaultRoleId` too: if the RSVP queries and the join-time resolver ever disagree on who holds "the default role", a team that configures a custom default silently stops sending reminders to its new members. Both build it from the shared `effectiveRolesFrom` fragment (`src/repositories/effectiveRoles.ts`) rather than reimplementing the group walk inline, for the same reason.
...
4. **Ceiling: `holdsDefaultRoleWhere`'s legacy half is hardcoded to `Player`, not "whatever the previous default was".** It covers a team's FIRST default change (built-in `Player` → a custom role) for free, because every pre-existing member still holds `Player`. It does NOT cover a SECOND change: switch the default again later and the cohort that joined during the now-superseded custom default (never `Player`, and no longer `is_default`) silently stops appearing in this query and stops accruing `missed_rsvps`. Known ceiling (see the `ponytail:` comment on `holdsDefaultRoleWhere` in `effectiveRoles.ts`), not fixed by this change. Upgrade path: a persistent `roles.is_rsvp_eligible` column set (and never cleared) on every role that has ever been the default, or retaining the full history of past defaults instead of collapsing to one `is_default` boolean.
