...
5. **A team-timezone change must re-anchor already-materialized future occurrences.** `src/api/team-settings.ts` re-derives `start_at`/`end_at` for events matching `series_id IS NOT NULL AND NOT series_modified AND status = 'active' AND start_at >= now()`, reading the calendar date in the OLD zone and resolving it in the NEW one. Keep those four guards identical to the migration's Statement B, **plus a fifth: `AND es.times_are_team_local`** — a `FALSE` (UTC-dialect) series' `start_time` is an absolute UTC time-of-day, so a team-timezone change is a no-op for it, exactly as it was before #650, and re-deriving it here would wrongly reinterpret a UTC value as wall clock. Statement B itself needs no such predicate: `1792100000`'s conversion migration runs Statement A (converting every `FALSE` row to `TRUE`-semantics) before Statement B, so by the time Statement B runs, every row it touches is already genuinely `TRUE`. The four shared guards exist to avoid clobbering per-occurrence captain edits, cancelled events, and the past.

   **Do not "align" this call's date expression with the migration's, and do not copy the migration's expression here.** The four shared guards above are identical between the two sites on purpose — same population semantics, same carve-outs. The date expression each uses to recover an occurrence's calendar date deliberately differs, because each sees a disjoint dialect population: `team-settings.ts`'s re-anchor only ever runs against already-`TRUE` (team-local-dialect) series, so it reads the date in the OLD team zone (`(start_at AT TIME ZONE oldTz)::date`); the migration's Statement B only ever runs against series it JUST converted from `FALSE` (UTC-dialect), so it reads the date with `AT TIME ZONE 'UTC'` instead. Both are applications of the same rule — "recover the date in the dialect the event was materialized under, then re-resolve in the corrected zone" — to two disjoint populations, not two expressions that happen to differ by oversight. Swapping one site's expression for the other's would resolve the wrong population's date: see `1792100000_series_time_is_team_local.ts`'s doc comment, "Dialect-recovery principle", for the concrete near-midnight failure this produces.
6. **A write path must carry the dialect explicitly — never hardcode it, never translate between dialects.** The invariant any conversion migration depends on is "every `FALSE` row is genuinely UTC-semantics", NOT "every row is `FALSE`": `1792100000`'s `WHERE NOT es.times_are_team_local` conversion guard was only sound while that held, and any future conversion of the same shape would depend on it identically. Create (`src/api/event-series.ts` → `EventSeriesRepository.insertEventSeries`) therefore writes `payload.timesAreTeamLocal` into `times_are_team_local` verbatim and names the column in the `INSERT` — never substitute a hardcoded `FALSE` on the reasoning that most rows are converted by now, and never rely on the column's `DEFAULT FALSE` here. Update never rewrites the stored dialect: it calls `assertDialectMatches(payload.timesAreTeamLocal, existing.times_are_team_local)` from `src/utils/seriesTimeDialect.ts`, failing with `EventSeriesApi.EventSeriesTimeDialectMismatch` (HTTP 400), and calls it ONLY when the payload actually supplies a time — gate on `Option.isSome(payload.startTime) || Option.exists(payload.endTime, Option.isSome)`, evaluated BEFORE either field is resolved with `Option.getOrElse(..., () => existing.*)`. A patch that omits both time fields asserts no dialect and MUST pass through untouched, because `timesAreTeamLocal` is not `Option`-wrapped and therefore always encodes on the wire (a location-only edit from any client always sends some value for it).
