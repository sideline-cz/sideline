...
4. **RSVP eligibility splices `holdsRsvpEligibleRoleWhere`, NOT `holdsDefaultRoleWhere`.** The two are different questions and the RSVP queries must never be "simplified" back onto the resolver's predicate. `holdsDefaultRoleWhere` is `is_default OR built-in Player` — current-default membership, with a `Player` half that covered a team's FIRST default change for free (every pre-existing member still held `Player`) and stranded the cohort in between on the SECOND: they hold only the superseded custom role, which is no longer `is_default` and was never `Player`, so they silently stopped appearing here and stopped accruing `missed_rsvps`. `holdsRsvpEligibleRoleWhere` is the sticky `roles.was_default` column instead — set by `RolesRepository.setDefaultRole` alongside `is_default` and deliberately never cleared by `clearDefaultByRoleTeamQuery`, so the population is cumulative and every cohort a team ever created keeps its reminders. Migration `1792600001_add_roles_was_default` backfills it from exactly the old union (`is_default OR built-in Player`), so the deploy is behaviour-preserving; cohorts already stranded by a pre-migration second change are NOT recoverable (nothing recorded the superseded defaults) and come back only if that role is re-set as default. Coverage: `test/integration/api/eventRsvpNonResponders.test.ts`, describe block "a second default change does not strand the previous cohort".
