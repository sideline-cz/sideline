...
- [#519](https://github.com/maxa-ondrej/sideline/pull/519) [`c1b1ade`](https://github.com/maxa-ondrej/sideline/commit/c1b1ade02e958bb671390c1e949176ed387f88f3) Thanks [@maxa-ondrej](https://github.com/maxa-ondrej)! - Add a terminal `catchCause` backstop to the `/event list` command's deferred-reply fork. It caught `RpcClientError` + REST but not defects, so a server-side `LogicError.die` could leave the deferred reply unresolved ("Sideline is thinking…"). Resolves it with the existing `bot_event_list_error`, mirroring the event-create backstop.

- [#518](https://github.com/maxa-ondrej/sideline/pull/518) [`15d8ca3`](https://github.com/maxa-ondrej/sideline/commit/15d8ca361181365614a04cf23a2630806886be27) Thanks [@maxa-ondrej](https://github.com/maxa-ondrej)! - Add a terminal `catchCause` backstop to the `/finance status` command's deferred-reply fork. It caught `RpcClientError` + REST but not defects, so a server-side `LogicError.die` could leave the deferred reply unresolved ("Sideline is thinking…"). Resolves it with the existing `bot_finance_error_generic`, mirroring the event-create backstop.

- [#515](https://github.com/maxa-ondrej/sideline/pull/515) [`42c9de1`](https://github.com/maxa-ondrej/sideline/commit/42c9de1529acbc2ecf1b1de13acf4a784e62730f) Thanks [@maxa-ondrej](https://github.com/maxa-ondrej)! - Add a terminal `catchCause` backstop to the `/makanicko leaderboard` command's deferred-reply fork. It caught its `Activity*` errors + `RpcClientError` + REST but not defects, so a server-side `LogicError.die` could leave the deferred reply unresolved ("Sideline is thinking…"). Resolves it with the existing `bot_makanicko_leaderboard_error`, mirroring the event-create backstop.

- [#514](https://github.com/maxa-ondrej/sideline/pull/514) [`ea6fb94`](https://github.com/maxa-ondrej/sideline/commit/ea6fb945bc655db9143315058eaa0ce1a97bc1e9) Thanks [@maxa-ondrej](https://github.com/maxa-ondrej)! - Add a terminal `catchCause` backstop to the `/makanicko log` command's deferred-reply fork. It caught its `Activity*` errors + `RpcClientError` + REST but not defects, so a server-side `LogicError.die` could leave the deferred reply unresolved ("Sideline is thinking…"). Resolves it with the existing `bot_makanicko_log_error`, mirroring the event-create backstop.

- [#513](https://github.com/maxa-ondrej/sideline/pull/513) [`7ccfb30`](https://github.com/maxa-ondrej/sideline/commit/7ccfb3081d849eb453bd53793de6a76e73fc8678) Thanks [@maxa-ondrej](https://github.com/maxa-ondrej)! - Add a terminal `catchCause` backstop to the `/makanicko stats` command's deferred-reply fork. It caught its `Activity*` errors + `RpcClientError` + REST but not defects, so a server-side `LogicError.die` could leave the deferred reply unresolved ("Sideline is thinking…"). Resolves it with the existing `bot_makanicko_stats_error`, mirroring the event-create backstop.

- [#521](https://github.com/maxa-ondrej/sideline/pull/521) [`ec2d280`](https://github.com/maxa-ondrej/sideline/commit/ec2d28050b0ee18f907f93f72aee8566d1daf98f) Thanks [@maxa-ondrej](https://github.com/maxa-ondrej)! - Add a terminal `catchCause` backstop to the `/poll` command's deferred-reply fork. Its tail combined `RpcClientError` + REST, so only an untagged defect (`CreatePoll`/`SavePollMessageId` die, or a `buildPollEmbed` throw) could leave the deferred reply unresolved ("Sideline is thinking…"). Resolves it with the existing `bot_poll_err_generic`, mirroring the event-create backstop.

- [#511](https://github.com/maxa-ondrej/sideline/pull/511) [`61d995a`](https://github.com/maxa-ondrej/sideline/commit/61d995aec4639c98b437b22f69f4e2414c6e3dfe) Thanks [@maxa-ondrej](https://github.com/maxa-ondrej)! - Add a terminal `catchDefect` backstop to the four remaining deferred-reply poll interaction forks (`PollOpenButton`, `PollVoteButton`, `PollAddModalSubmit`, `PollCloseButton`). They already caught `RpcClientError` + REST, but an untagged defect (a renderer throw or a server-side `LogicError.die`) would die in the forked fiber and leave the deferred reply blank ("Sideline is thinking…"). Resolves it with the existing `bot_poll_err_generic`, mirroring the poll-voters / poll-remove defect backstops already in the file.

- [#512](https://github.com/maxa-ondrej/sideline/pull/512) [`e7ae3f8`](https://github.com/maxa-ondrej/sideline/commit/e7ae3f8ba8d41f2ad50af3b1d7bca1ceac497ec5) Thanks [@maxa-ondrej](https://github.com/maxa-ondrej)! - Add a terminal `catchCause` backstop to the roster-approval interaction fork. It caught `RpcClientError` + REST but not defects, so a server-side `LogicError.die` (or renderer throw) surfaced as a defect could leave the deferred reply unresolved ("Sideline is thinking…"). Resolves it with the existing `bot_roster_ephemeral_error`, mirroring the event-create backstop.
...
- [#520](https://github.com/maxa-ondrej/sideline/pull/520) [`5be38c1`](https://github.com/maxa-ondrej/sideline/commit/5be38c11700a37cd368e5b3a8261479a15ab9a9f) Thanks [@maxa-ondrej](https://github.com/maxa-ondrej)! - Add a terminal `catchCause` backstop to the `/sudo` command's deferred-reply fork. Its tagged errors were well handled, but an untagged defect (e.g. a server-side `LogicError.die`) could still leave the deferred reply unresolved ("Sideline is thinking…"). Resolves it with the existing `bot_sudo_err_generic`, mirroring the event-create backstop.

- [#510](https://github.com/maxa-ondrej/sideline/pull/510) [`a30fb82`](https://github.com/maxa-ondrej/sideline/commit/a30fb829fe4d0f4cc4fbdffa0ce1ebc25981566a) Thanks [@maxa-ondrej](https://github.com/maxa-ondrej)! - Add a terminal `catchCause` backstop to the `SudoLeaveButton` deferred-reply fork. Its inner `ensureSudoRole`/`deleteGuildMemberRole` chain only handled `ErrorResponse` and the outer catch only `RpcClientError`, so a transient `HttpClientError`/`RatelimitedResponse` from the role deletion — or an untagged defect — left the deferred reply unresolved ("Sideline is thinking…"). The backstop always resolves it with the existing `bot_sudo_err_generic` message, mirroring the event-create backstop.

- [#517](https://github.com/maxa-ondrej/sideline/pull/517) [`7ea8870`](https://github.com/maxa-ondrej/sideline/commit/7ea8870a58173afb42715e8307d64fa809628040) Thanks [@maxa-ondrej](https://github.com/maxa-ondrej)! - Add a terminal `catchCause` backstop to the `/training generate` command's deferred-reply fork. It caught `RpcClientError` + REST but not defects, so a server-side `LogicError.die` could leave the deferred reply unresolved ("Sideline is thinking…"). Resolves it with the existing `bot_training_generate_error`, mirroring the event-create backstop.

- [#516](https://github.com/maxa-ondrej/sideline/pull/516) [`558b366`](https://github.com/maxa-ondrej/sideline/commit/558b3668473847c1bf17e26c1581a5b36e0e8c41) Thanks [@maxa-ondrej](https://github.com/maxa-ondrej)! - Add a terminal `catchCause` backstop to the `/training result` command's deferred-reply fork. It caught `RpcClientError` + REST but not defects, so a server-side `LogicError.die` could leave the deferred reply unresolved ("Sideline is thinking…"). Resolves it with the existing `bot_training_result_error`, mirroring the event-create backstop.

- [#522](https://github.com/maxa-ondrej/sideline/pull/522) [`af56048`](https://github.com/maxa-ondrej/sideline/commit/af5604851477f723d14b6e31190a7599e8e56914) Thanks [@maxa-ondrej](https://github.com/maxa-ondrej)! - Add a terminal `catchCause` backstop to the three deferred-reply-resolving upcoming-RSVP interaction forks. They caught `RpcClientError` + REST but not defects (and drive several extra RPCs via `postRsvpDiscordUpdates` / `renderUpcomingPagePayload` that could die), so a server-side defect could leave the deferred reply unresolved ("Sideline is thinking…"). Resolves it with the existing `bot_event_list_error`, mirroring the event-create backstop.

- Updated dependencies [[`687cd1a`](https://github.com/maxa-ondrej/sideline/commit/687cd1a05920e49ceae5abc3198e674e6e207aa9), [`970d8c8`](https://github.com/maxa-ondrej/sideline/commit/970d8c814c3e9c1795e463bd3198f1ceba7bda72), [`cd6c321`](https://github.com/maxa-ondrej/sideline/commit/cd6c3211ad35572378f683b58c6ac9fba5d5ec7a)]:
  - @sideline/i18n@0.18.5
...
- Updated dependencies [[`df6359f`](https://github.com/maxa-ondrej/sideline/commit/df6359fc427fbe0d9d421edcf0b4d153b7818455)]:
  - @sideline/domain@0.37.3
  - @sideline/i18n@0.18.3
