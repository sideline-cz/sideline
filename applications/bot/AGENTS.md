...
**A team's built-in Sideline roles (Admin / Captain / Player / Treasurer) never appear in this queue at all.** The server refuses to write them — see `applications/server/AGENTS.md` → "Role-sync diffs: assignment is unfiltered, removal is gated", rule 4 — so the bot needs no name check of its own here, and must not grow one: `ensureMapping` (`src/rest/roles/ensureMapping.ts`, adopt-or-create) would mint a guild role named "Player" from a single leaked `role_assigned` reaching `src/rcp/role/handleAssigned.ts`. A guild that already carries such a role from before that guard keeps it: the server's diffs still select a previously-mapped built-in role as a `role_unassigned` candidate, but `findSyncableRoleIds` drops the row before it is written, so no event ever reaches this queue and the bot neither re-assigns nor strips it. Do NOT add a cleanup pass that deletes those roles.
