...
The bank-sync subsystem ingests Fio movements (`applications/server/src/services/BankSyncPoller.ts`, hourly), matches them against `fee_assignments` (`services/BankTransactionMatcher.ts`), and exposes the queue, CSV and PDF at `src/api/bank-sync.ts`. Three invariants span the whole monorepo and are not visible from the call sites that violate them.
...
| `BankSyncConfigRepository.clearPollBackoff` | **Yes, on probe success only** | `UPDATE … SET next_attempt_at = NULL, updated_at = now()` and nothing else. This is 100% of the useful write — un-sticking the exponential poll backoff — and touches no column the status ladder reads. |
