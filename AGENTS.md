...
**Last Updated**: 2026-05-17 (server AGENTS.md: added `PaymentReminderCron` to the cron table; added "Bot-Ack Idempotency for Discord-Side-Effect Crons" section documenting the `<resource>_sync_events` outbox + `<resource>_sent` delivery-log + bot-driven ack-after-Discord-success pattern, with `payment_reminder_sync_events` / `payment_reminders_sent` / `Finance/MarkReminderSent` as the canonical example; added "iCal Feed Generation" section codifying mandatory `escapeICalText` usage on every dynamic value in `SUMMARY` / `DESCRIPTION` / `LOCATION` / `CALNAME`, the replacement order rule, and the 180-day history cap on payment VEVENTs. Bot AGENTS.md: added "Finance Sync (payment reminders → user DMs)" section under sync types, including the rule that `Finance/MarkReminderSent` must run AFTER `createMessage` succeeds and BEFORE `Finance/MarkPaymentReminderProcessed`; added "Tagged-Union Dispatch With `Match.exhaustive`" section listing every dispatcher (`PaymentReminderKind`, `UnprocessedPaymentReminderEvent`, channel / event sync union) and forbidding `Match.orElse` for closed unions or per-branch casts.)
