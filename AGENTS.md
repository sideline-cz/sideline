...
**Last Updated**: 2026-05-17 (web AGENTS.md: documented pure-helper modules under `src/lib/<feature>/` with co-located tests, the loader pattern for parallel fetch with per-arm `tapError → catch` graceful degradation, and Pattern C — per-row lazy fetch via `useQuery` + `useRun`, all referencing the My Payments page; added rule 6 forbidding scoped translation-key duplicates of existing generic keys, referencing reuse of `finance_kpi_*`. Server AGENTS.md: documented the caller-scoped-read auth pattern — `my*` endpoints use `requireMembership` only and hardcode `memberId = membership.id` in the repository call, referencing `myStatus` / `myPaymentHistory`)
