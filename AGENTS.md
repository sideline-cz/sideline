...
4. **Never hard-code the version string.** `APP_VERSION` must always resolve from `package.json`. NOTE: with Changesets removed, `package.json` versions are now frozen — nothing auto-bumps them on release. `APP_VERSION` therefore reflects the (static) `package.json` version, which may lag the actual release tag (`vX.Y.Z`) until version stamping is wired to the tag. Wiring the displayed version to the release tag is a known follow-up; do not invent a mechanism here.
