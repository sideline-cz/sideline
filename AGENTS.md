...
### Personal Event Channels — The De-Provision Gate And Stored-Channel Addressing

A team member's private event channels are provisioned by the bot (`applications/bot/src/rcp/personalEvents/`), addressed by the server (`applications/server/src/repositories/PersonalEventChannelsRepository.ts`), and switched on and off by the member on the web profile page (`applications/web/src/components/organisms/EventPreferencesCard.tsx`). A member owns either ONE `bucket = 'all'` channel (the default) or THREE — `training` / `tournament` / `other`, with `event_type = 'match'` folded into `tournament`. Two invariants span all three applications and are invisible from the call sites that would break them.

**1. Nothing is de-provisioned until the member's desired bucket set is FULLY provisioned.** Provision (Pass 1a) and de-provision (Pass 1b) run in the SAME bot tick, and provision swallows every failure into a log line. A mode switch that tore the old channel down first would therefore, on any stuck provision — Discord rate limit, missing permission, the guild-wide 500-channel cap (`30013`), a crashed pod — leave the member with no channel and no message history at all, permanently, because the switch is already recorded. The gate is ONE SQL fragment, `deprovisionableBucketSql` in `applications/server/src/repositories/personalChannelBucket.ts`, and both the de-provision query (`_getObsoleteBuckets`) and the poll that wakes it (`_getGuildsNeedingProvisioning` branch (e)) splice the SAME fragment, so the poll can never fire on a tick where de-provision would refuse to act. With the gate, a stuck provision degrades to "the member keeps the channel they had" and retries next tick. Group-based and inactive-member de-provision are deliberately NOT gated — those members lose every channel. Never hand-copy the predicate into a third query, and never reorder the passes so de-provision runs first.

**2. A stored personal message is addressed by its STORED `personal_channel_id`, never by the member's current channel.** `events.event_type` is mutable and `team_members.personal_channels_split` is user-settable, so the bucket an already-posted card belongs to can change under it. `Guild/ListPersonalChannelsForEvent` reports where the card SHOULD live now (exactly one row per member, via a `CASE`); `personal_event_messages.personal_channel_id` records where it actually IS. Every `updateMessage` / `deleteMessage` in `applications/bot/src/rcp/personalEvents/handleReconcile.ts` targets the stored id. When the two disagree, reconcile CREATES in the new channel first and only then deletes the old message, with that delete retried: create-then-delete degrades to a duplicate on a transient failure, while delete-then-create loses the card outright, because the create path swallows Discord failures and `ProcessorService` clears the dirty flag either way. The move check MUST run BEFORE the `payload_hash` no-op skip — a move usually leaves the payload byte-identical, so the skip would strand a live, fully interactive card in a channel nothing ever revisits again.

Full mechanics: `applications/server/AGENTS.md` → "Personal event tables and reserve-first provisioning" (plus "Bucket routing" and "Member event preferences"); `applications/bot/AGENTS.md` → "Personal Events Sync".
...
**Last Updated**: 2026-09-22 (Configurable attendance `feat/configurable-attendance`. Root AGENTS.md: new section "Personal Event Channels — The De-Provision Gate And Stored-Channel Addressing" (the `deprovisionableBucketSql` gate shared by the de-provision query and the poll that wakes it; stored-`personal_channel_id` addressing with create-then-delete on a bucket move, ordered before the `payload_hash` skip). server AGENTS.md: `personal_event_channels` is now keyed `(team_id, team_member_id, bucket)` — reserve `ON CONFLICT` 3-tuple, single-CTE `deletePersonalChannel`, `_getGuildsNeedingProvisioning` five branches, `` `${team_member_id}:${bucket}` `` de-provision dedup; new sections "Bucket routing (`src/repositories/personalChannelBucket.ts`)" (fragment table, `_listForEvent`'s one-row-per-member `CASE`) and "Member event preferences" (own-membership-only endpoints, advisory `personalChannelsAvailable`, dirty mark only on `showAttendeeList`, the two independent `UpcomingEventsForUserResult` lookups); non-responder section gained rule 3 (`rsvp_reminder_dms` is carried, filtered only by `Event/GetRsvpReminderSummary`). bot AGENTS.md: per-`(member, bucket)` channels, `bucket` on the Pass 1a/1b/1c payloads, `formatPersonalChannelName`'s 4th argument, per-channel reorder grouping (constraints 4 and 8), new reconcile rules 6 and 7 (stored-channel addressing, move-before-hash-skip), the four-call-site `show_attendee_list` gating table, server-side reminder-DM opt-out, Discord `30013` guild-cap logging. web AGENTS.md: `useCardForm` rules 8 (seeds once → separate load-failure component) and 9 (`useCardForm`/`SaveRow` are generic, import in place); loader rule 5 (fan-out over N of the same non-critical resource wraps every arm, `null` = failure). migrations AGENTS.md: new section "Widening a UNIQUE Constraint (Add the Wider Index BEFORE Dropping the Narrower One)".)

**Last Updated**: 2026-09-22 (Series-time conversion migration `fix/recurring-series-edit-400`. Two corrections to what the tree said about non-deferrable unique indexes, both verified against a real `postgres:17`: Postgres checks such an index **per row, as each row is written**, never at statement end, so a bulk `UPDATE` that rewrites the indexed value can abort with `duplicate key value violates unique constraint` on another row's still-live value even when the FINAL state holds no duplicate — which rows collide is decided purely by the plan's row order, so the same statement passes on a small dev database and fails in production. `packages/migrations/AGENTS.md` gains "A Bulk `UPDATE` That Changes A Uniquely-Indexed Value Must Drop And Recreate That Index": the mandatory four-statement bracket (`SET LOCAL lock_timeout = '5s'` -> `DROP INDEX IF EXISTS` -> the `UPDATE` -> a verbatim copy of the original `CREATE UNIQUE INDEX`), why `lock_timeout` comes first before ANY DDL in a `MigrateBefore` migration (boot runs during a blue-green window in which the previous container still queries the table, so a waiting `ACCESS EXCLUSIVE` request both hangs boot and queues every later query on that table behind it — the same idiom `api/group.ts` uses on an interactive endpoint), why neither `CREATE INDEX CONCURRENTLY` nor `DEFERRABLE` is available as an alternative, and two mandatory testing rules: force the worst-case access path with `SET LOCAL enable_indexscan = off` / `enable_bitmapscan = off` inside the SAME `sql.withTransaction` as the migration run (at test cardinality the planner otherwise satisfies the join through the very index under test, visits rows in ascending key order, and the collision case stays green with the whole bracket deleted), and pin `pg_indexes.indexdef` against the original definition so deleting the trailing `CREATE UNIQUE INDEX` cannot pass either. `applications/server/AGENTS.md` -> "Renumbering a contiguous-position column after deleting rows": rules 1-2 asserted the opposite mechanism (a statement-end check that "makes even the unguarded form safe") and are corrected — the `ROW_NUMBER() OVER (ORDER BY position)` CTE is safe because its ascending output drives the UPDATE, and a `DESC` window, an unordered row source (`VALUES` list, plain table join) or per-row UPDATEs each reintroduce the collision. Reference implementation `1792100000_series_time_is_team_local.ts`, coverage `applications/server/test/integration/migrations/seriesTimeIsTeamLocal.test.ts`. No `.claude/` change — no agent or skill referenced either rule.)

**Last Updated**: 2026-09-22 (Unmapped role never provisioned by reconcile `fix/reconcileMemberDiscordRoles-unmapped-role`. `applications/server/AGENTS.md`: new "Role-sync diffs: assignment is unfiltered, removal is gated" — all three `role_sync_events` diff sites (`syncMemberDiscordRoles.ts`, `syncGroupRoleMembers.ts`, `reconcileMemberDiscordRoles.ts`) build `role_assigned` candidates from the member's desired effective roles WITHOUT intersecting `discord_role_mappings`, because the bot's `handleMemberAdded` bootstraps the mapping via `ensureMapping`; only `role_unassigned` stays gated on the mapping AND `member_role_grants` (the CC-8 anti-stripping guard).)

**Last Updated**: 2026-09-22 (RSVP note clearing `fix/web-ui-clear-rsvp-note`. server AGENTS.md → "Wire-value projection & effective-value guards": new rule 3 — a COALESCE-based partial upsert can never clear the column, so clearing needs a separate explicit signal that EVERY call site must pass. `upsertRsvp`'s `clearMessage = false` default let the HTTP `submitRsvp` handler omit the 5th argument and silently made the `upsertClearing` branch unreachable, so the web UI could never remove an RSVP note. The rule records the per-surface derivation table — HTTP `submitRsvp` has no flag, so a blank/whitespace `message` means clear and `null` means keep, while the RPC `Event/SubmitRsvp` request carries an explicit `clearMessage` field — and requires the cleared message to reach BOTH `upsertRsvp` and `isRsvpMessageRequiredAndMissing` as `Option.none()` so the `coming_later` guard sees what the write will store. No other AGENTS.md or `.claude/` change — no new workflow, agent, or skill.)

**Last Updated**: 2026-09-22 (Bank-sync `activating` window made two-sided `fix/bank-sync-activating-window`. `services/bankSyncStatus.ts` rule 4 was one-sided (`tokenCreatedAt > now - 5min`), but `fio_token_created_at` is a user-declared calendar date snapped to 12:00 UTC by `applications/web/src/lib/datetime.ts`'s `dateOnlyToUtcNoon` — never a real instant — so it also matched every moment BEFORE the anchor: a token dated today reported `activating` from 00:00 through 12:05 UTC, and a future-dated token reported it until that date passed. Now `now - 5min < tokenCreatedAt <= now`. server AGENTS.md: new "A Noon-Anchored Date-Only Column Is Not An Instant" — names the four noon-anchored columns (`bank_sync_config.fio_token_created_at`, `payments.paid_at`, `fees.due_at`/`fee_assignments.due_at`, `expenses.spent_at`) and requires any comparison of one against a sub-24h window to be two-sided, never treats such a value as evidence of an observed instant, and requires tests to assert BOTH sides of the window. web AGENTS.md: "Date Inputs — `DatePicker`" gained rule 4 — `toDate` is the DAY-granularity upper clamp (`endMonth` + react-day-picker `disabled={{ after }}`), `toYear` is year-granularity only and leaves every day of the final year selectable; old rules 4-5 renumbered to 5-6. `components/ui/date-picker.tsx` gained the optional `toDate` prop and `FioBankCard.tsx` passes `toDate={new Date()}`.)

**Last Updated**: 2026-09-21 (Archived roles kept granting permissions `fix/archived-roles-grant-permissions`. `deleteRole` is a SOFT delete — `RolesRepository.archiveRoleById` runs `UPDATE roles SET is_archived = true` and deletes no `member_roles` / `role_groups` / `role_permissions` rows — but `applications/server/src/repositories/effectiveRoles.ts`, the one fragment every "which roles/permissions does this member hold?" query splices, filtered archived GROUPS and never archived ROLES, so a deleted role kept granting its permissions through `effectivePermissionsAgg`, including to `findMembershipQuery` — the query behind every `requirePermission` guard. The fix is `AND r.is_archived = false` on BOTH `JOIN roles r` clauses inside `effectiveRolesFrom` (the direct `member_roles` half and the group-inherited `role_groups` half); a filter on only one of them still leaks a role whose `source` is `'both'`. `TeamMembersRepository.findEffectiveRolesForMembersQuery`'s per-caller `JOIN roles r ON r.id = er.role_id AND r.is_archived = false` is deleted with it: it was the only effective-roles query that revoked an archived role, which is exactly why it disagreed with its per-member sibling `findEffectiveRoleIdsForMemberQuery`. server AGENTS.md → "Effective Roles Are Derived In Exactly One Place (`effectiveRoles.ts`)": new rule 4 states the archived-role contract and the put-the-filter-in-the-fragment-never-in-a-caller requirement; the previous rules 4 and 5 are renumbered 5 and 6. Integration coverage: `applications/server/test/integration/repositories/archivedRoleGrantsNothing.test.ts` — direct, group-inherited, and the `source: 'both'` shape.)
