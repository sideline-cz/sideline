...
3. Launches all of the following concurrently (concurrency: 10):
   ...
   - `CoachingStatusCron`
   - `BankSyncPoller` — hourly Fio bank-movement poll + inline auto-match (not one of the `*Cron.ts` files above; see [4. Background Cron Jobs](#4-background-cron-jobs))
   - `BankTokenExpiryCron` — daily Fio token-expiry scan (see [4. Background Cron Jobs](#4-background-cron-jobs))
...
| `FIO_TOKEN_ENCRYPTION_KEY` | No | — | AES-256-GCM encryption key used by `FioSecretCrypto` to encrypt/decrypt Fio bank API tokens stored in `bank_sync_config.fio_token_encrypted`. A deliberately separate key from `EMAIL_IMAP_ENCRYPTION_KEY` — the two secrets never rotate together. Same "optional, fails on use, not on boot" pattern: a missing key never fails server startup, only a `PUT /teams/:teamId/bank-sync` call that includes a token (500 `FioSecretKeyMissing`), and an already-configured team whose most recent poll failed for this reason reports `status: 'misconfigured'` rather than blaming the treasurer's token. Generate with: `openssl rand -base64 32` |
...
| `AI_CHAT_ENABLED` | No | `false` | Kill switch for the read-only in-app AI assistant (`GET/POST /teams/:teamId/ai/*`). Same permissive, case-insensitive parsing as `DISCORD_JOIN_ENFORCEMENT_ENABLED` above and the same never-fail-boot guarantee. When disabled (the default) or when no LLM is configured (`LLM_API_URL` unset), `capabilities.enabled` is `false` and `chat` short-circuits to a 200 degraded response before consuming any rate-limit budget or calling the LLM. **Rate limiting is per replica**: `ChatRateLimiter` (`applications/server/src/services/ChatRateLimiter.ts`) is an in-process, in-memory fixed-window limiter (20 chat turns / 10 minutes and 120 / day per user) with no shared state across replicas, so the effective fleet-wide limit is `replicas × 20` per 10 minutes and `replicas × 120` per day — accepted because the real cost bound is per-request (bounded tool-call iterations and `max_tokens`), not the rate limiter. |
| `PROFILE_GATE_ENABLED` | No | `true` | Global incident lever for the profile-completeness gate (RSVP / training claim / carpool seat — `EventRsvpProfileIncomplete`/`RsvpProfileIncomplete`/`ClaimProfileIncomplete`/`CarpoolProfileIncomplete`). Unlike `DISCORD_JOIN_ENFORCEMENT_ENABLED` and `AI_CHAT_ENABLED` above, this flag **defaults ON**: the real off switch is the per-team `team_settings.require_complete_profile` column (default `false`), so no team is affected until its captain opts in — this variable exists purely to kill the gate for every team at once during an incident. Same permissive, case-insensitive parsing (`true`/`1`/`yes`/`on` vs `false`/`0`/`no`/`off`), but an empty/unset value and any unrecognised value both resolve to **enabled** (not disabled) — the inverse of the other two flags' safe direction — and an unrecognised value logs a startup warning without failing boot. |
...
Source files: `applications/server/src/services/*Cron.ts`, plus `BankSyncPoller.ts` (below) which follows the same `Schedule.cron`-based shape without the `*Cron.ts` naming convention.
...
| `EventStartCron` | `* * * * *` (every minute) | Finds `active` events whose `start_at` time has passed, transitions each to `started` status, and emits an `event_sync_events` row of type `event_started` for the bot to process (deletes the owners-thread training claim message; nothing is posted for the event starting). Also marks the event's `personal_messages_dirty_at` so the reconcile worker removes it from members' personal channels, and runs a once-per-cycle self-healing sweep that re-marks any non-active/past event still holding `personal_event_messages` rows. |
...
| `TrainingAutoLogCron` | `*/5 * * * *` (every 5 minutes) | Finds ended training events that haven't been auto-logged yet. For each event, inserts an `activity_logs` row for every member with an attending RSVP ("yes" or "coming later" — "not sure" does not count). Ignores duplicate-key violations (idempotent). |
...
| `PaymentReminderCron` | `* * * * *` (every minute) | Finds fee assignments that have crossed a reminder cadence threshold (T−3 days, T+0, T+3, T+10, T+21 days) and have not already been queued for that cadence (no unprocessed `payment_reminder_sync_events` row for the same `(assignment_id, kind)` pair). For each candidate, inserts a row into `payment_reminder_sync_events`. Since the Fio bank-sync feature, a sixth candidate branch (`assigned`) fires once immediately at fee-assignment creation — not date-gated — for any team with an enabled `bank_sync_config`, carrying the payment QR alongside a neutral "new fee assigned" message. The bot's Finance Sync worker drains the outbox and sends a Discord DM to the member; on successful delivery it calls `Finance/MarkReminderSent` to record the send in `payment_reminders_sent`. Instrumented with the `payment-reminder` metric label. The server must be running for reminders to fire; the bot need not be running for the cron to enqueue them, but the DMs are only delivered while the bot is connected. |
| `BankSyncPoller` | `0 * * * *` (hourly, `applications/server/src/services/BankSyncPoller.ts`) | For every team with an enabled, token-configured `bank_sync_config`, pulls new movements from the Fio API over a rolling window (`from = max(today-89, min(today-14, last_success_date-1))` to `today`), ingests them into `bank_transactions` (idempotent on `(team_id, provider, fio_movement_id)`), and runs the auto-matcher inline over the newly-ingested rows. Guarded by a per-team distributed poll lease (`bank_sync_config.poll_leased_until`/`poll_leased_by`, 6-minute lease against a 4-minute per-team call timeout) so only one replica polls a given team at a time, and by a per-Fio-token 30-second cross-replica throttle (`fio_token_throttle`). Not named `*Cron.ts` (it mirrors `ImapPoller.ts`'s naming instead) but registered the same way — a long-running supervised effect launched alongside the other crons in `run.ts`. See [Bank Sync (Fio) in `database.md`](database.md#19-bank-sync-fio) for the ingestion/matching schema. |
| `BankTokenExpiryCron` | `0 4 * * *` (daily at 04:00 UTC) | Scans `bank_sync_config` for enabled configs whose Fio token sits at exactly T−14, T−7, or T−1 calendar day(s) from `fio_token_created_at + 180d`, and emits one row into `bank_token_expiry_events` per `(team, threshold)` match, skipping any pair that already has a pending (unprocessed) row. Writes `bank_token_expiry_sent` itself immediately after a successful emit (there is no bot-side "sent" ack for this outbox, unlike `PaymentReminderCron`'s). The bot's Finance Sync worker (Pass 2) drains the outbox and DMs the treasurer. |
...
| `CoachingStatusCron` | `* * * * *` (every minute) | Finds claimed training events whose `coaching_status_sent_at` is NULL and whose `start_at` is on the current calendar day. For each, resolves the target Discord channel via the event's owner-group channel mapping (`DiscordChannelMappingRepository`); if unresolvable, warns and still marks the event sent. Emits a `coaching_status` sync event so the bot posts a "today's coach is X" announcement to that channel. Sets `coaching_status_sent_at = now()` on the event. Instrumented with the `coaching-status` metric label. |
