...
| **Main Flow** | 1. The actor opens the event detail page (web) or sees the event embed in Discord. 2. The actor clicks a response button: Yes, No, or Coming later. 3. **Yes/No:** both the web app and the Discord bot immediately call the RSVP endpoint on click (no separate confirmation step). **Web:** calls `PUT /teams/:teamId/events/:eventId/rsvp` with the selected response and the actor's existing message (if any). **Discord bot:** calls `Event/SubmitRsvp` via RPC; the bot replies with an ephemeral confirmation. **Coming later:** since a non-blank comment is mandatory, neither surface submits on click — the web app reveals a required note field (Save disabled until it is filled in) and the Discord bot opens a modal requiring the comment before submitting. 4. The server records the response and returns updated counts; `coming_later` counts as full attendance (same as `yes`) for roster auto-provisioning, team generation, training auto-log, player ratings, and the min-players headcount. 5. The bot updates the RSVP count display on the Discord embed. 6. **Web (note):** After a response is recorded, the actor can type or edit a note and click "Save note", which triggers a second `PUT /teams/:teamId/events/:eventId/rsvp` call carrying the updated message; for "Coming later" this note is mandatory and Save stays disabled while it is blank. **Discord bot (message):** The ephemeral confirmation includes a `[💬 Add a message]` button. Clicking it first calls `Event/GetRsvpMessage` to fetch the actor's currently stored note (falling back to no prefill if that call fails, since a modal response cannot be deferred), then opens a modal — pre-filled with that note when one exists — where the actor can type a message (mandatory and non-empty for "Coming later", optional otherwise); submitting saves the message via a second `Event/SubmitRsvp` call. If a message already exists the buttons shown are `[💬 Edit message]` and, for Yes/No only, `[🗑️ Clear message]` (never offered for "Coming later", since clearing would leave the mandatory comment blank). |
