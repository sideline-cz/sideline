...
A member can alternatively be onboarded **entirely from within Discord**, without ever visiting the web app. Simply joining a Discord guild the bot is installed in already creates a full Sideline user and team membership from the Discord identity alone, independent of any invite link (`Guild/RegisterMember`, Alternate Flow C of UC-02) — no OAuth login is required for this path. What was missing at that point was any prompt to finish a profile, and any enforcement; `/complete` (UC-34) fills the profile in without leaving Discord, and a captain can now opt a team into requiring a complete profile before a member can RSVP, claim a training, or take a carpool seat (UC-41). This gate is off by default, so a bare Discord join stays fully functional immediately unless a captain turns it on.
...
Events represent scheduled team activities (training, match, tournament, meeting, social, other). Events can be created as one-off occurrences or as part of a recurring series. A series definition specifies recurrence frequency and days of the week; individual events are generated from series by the background scheduler. RSVP responses are yes/no/coming later/not sure, with a free-text message that is optional for yes/no but required for "coming later" (a player will attend but arrive late) and "not sure" (an undecided answer must say what it depends on); "coming later" counts as full attendance everywhere "yes" does (roster auto-provisioning, team generation, training auto-log, player ratings, min-players headcount) — "not sure" does not.
...
flowchart LR
    ...
    subgraph SLASH["Slash Commands"]
        ...
        UC_COMPLETE["\/complete\nTakes no options; opens a single modal for name, birth date,\ngender (dropdown), plus optional jersey number\nephemeral · sets is_profile_complete = true\nalso reachable via the 'Finish my profile' button\n(Guild/CompleteMemberProfile)"]
    end

    subgraph BUTTONS["Button Interactions on Embeds"]
        UC_BTN_RSVP["RSVP Button (Yes / No / Coming later / Not sure)\nYes/No save immediately on click (no modal)\nComing later and Not sure each open a required-comment modal instead\nEphemeral confirm with Add/Edit/Clear message buttons\nupdates embed counts in real time"]
...
| **Precondition** | The event exists and has not been cancelled. The RSVP deadline has not passed. The actor is a member of the team (or, if the event has a member group, is a member of that group). If the team has opted into the profile-completeness gate (UC-41), the actor's profile (name, birth date, gender) must be complete. |
| **Main Flow** | 1. The actor opens the event detail page (web) or sees the event embed in Discord. 2. The actor clicks a response button: Yes, Coming later, Not sure ("maybe"), or No. 3. **Yes/No:** both the web app and the Discord bot immediately call the RSVP endpoint on click (no separate confirmation step). **Web:** calls `PUT /teams/:teamId/events/:eventId/rsvp` with the selected response and the actor's existing message (if any) — except when leaving "Coming later" or "Not sure", which always sends an empty message to clear its mandatory note. **Discord bot:** calls `Event/SubmitRsvp` via RPC; the bot replies with an ephemeral confirmation. **Coming later / Not sure:** since a non-blank comment is mandatory for both (`EventRsvp.rsvpResponseRequiresMessage`), neither surface submits on click — the web app reveals a required note field (Save disabled until it is filled in) and the Discord bot opens a modal requiring the comment before submitting. 4. The server records the response and returns updated counts; `yes` and `coming_later` count as full attendance for roster auto-provisioning, team generation, training auto-log, player ratings, and the min-players headcount — "Not sure" does not (that is the only remaining behavioural difference between the two). Switching away from "Coming later" or "Not sure" clears its stored note server-side unless the new submission supplies a replacement, so a mandatory reason never survives onto another response; because the destination response may itself require a note, that clear is computed before the mandatory-message check, so switching between "Coming later" and "Not sure" with no new note is rejected rather than silently blanked. 5. The bot updates the RSVP count display on the Discord embed. 6. **Web (note):** After a response is recorded, the actor can type or edit a note and click "Save note", which triggers a second `PUT /teams/:teamId/events/:eventId/rsvp` call carrying the updated message; for "Coming later" and "Not sure" this note is mandatory and Save stays disabled while it is blank. **Discord bot (message):** The ephemeral confirmation includes a `[💬 Add a message]` button. Clicking it first calls `Event/GetRsvpMessage` to fetch the actor's currently stored note (falling back to no prefill if that call fails, since a modal response cannot be deferred), then opens a modal — pre-filled with that note when one exists — where the actor can type a message (mandatory and non-empty for "Coming later" and "Not sure", optional otherwise); submitting saves the message via a second `Event/SubmitRsvp` call. If a message already exists the buttons shown are `[💬 Edit message]` and, for Yes/No only, `[🗑️ Clear message]` (never offered for "Coming later" or "Not sure", since clearing would leave the mandatory comment blank). |
...
| **Alternate Flow A** | If the RSVP deadline has passed the API returns `400 RsvpDeadlinePassed` and the UI informs the user. |
| **Alternate Flow B** | If the response is "Coming later" or "Not sure" and the effective message (submitted, or previously stored if the submission omits it) is blank, the API returns `400 EventRsvpMessageRequired` and the UI shows an inline error instead of recording the response. |
| **Alternate Flow C** | If the gate is on for this team and the actor's profile is incomplete, the web API returns `403 EventRsvpProfileIncomplete` and the bot's RPC call fails with `RsvpProfileIncomplete` — checked before the event and deadline are resolved, so it takes priority over Alternate Flow A even for a non-member or a closed event. See UC-41. |
...
| **Notes** | Reminders are silenced automatically once the assignment is paid or waived: the cron's candidate query filters on `computed_status NOT IN ('paid', 'waived')`. Reminders are also silenced once the underlying fee is archived — the treasurer archiving a fee stops collection on every money path, including this one. The QR's own text fields (the payment message, the transliterated recipient name) are rendered in uppercase, diacritic-free ASCII — a SPAYD convention, not a translation bug — while the surrounding embed copy keeps full Czech diacritics. |
...
|---|---|
| **Actor** | Captain / Admin (any member holding `member:edit` permission) |
| **Precondition** | The actor is navigated to the training result section of an event detail page. The event is a training event with status `active` or `started`, and its `start_at` is within the past 2 days. At least two members with an attending RSVP ("yes" or "coming later") exist. |
| **Main Flow** | 1. The actor opens the training result section on the event detail page. The web app calls `GET /teams/:teamId/events/:eventId/training-games` to load any previously logged rounds. 2. The actor assigns RSVP-yes (including "coming later") members to Team A and Team B, selects the outcome (`teamA`, `teamB`, or `draw`), and clicks **Save**. 3. The web app calls `POST /teams/:teamId/events/:eventId/training-games` with the payload. 4. The server verifies `member:edit` permission, confirms the event is loggable (training, active/started, within 2 days), validates that all selected members have an attending RSVP, inserts a `training_games` row (auto-incrementing `round`), inserts `training_game_participants` rows, updates Elo ratings using the same engine as `applyGameResult`, and returns the `TrainingGameResult`. 5. The web app appends the new round to the read-only logged games list and refreshes the displayed ratings. |
...
| **Alternate Flow B** | If a member selected for a team did not submit an attending RSVP, the server returns `422 PlayerRatingInvalidGameResult` with `reason = "notRsvpYes"`. |
| **Alternate Flow C** | If `teamA` or `teamB` is empty, the server returns `422 PlayerRatingInvalidGameResult` with `reason = "emptyTeam"`. |
| **Notes** | Game results are immutable once logged in this iteration — there is no edit or delete endpoint. Attendance auto-log (best-effort, deduped per UTC day) runs on save for all RSVP-yes (including "coming later") members. |
...
| **Precondition** | The actor is an authenticated team member with the `member:edit` permission. The target event is a training event that has at least as many RSVP-yes members (including "coming later", which counts as attending) as the desired team count (minimum 2 teams × 1 member each). |
...
| **Main Flow** | 1. The user invokes `/complete` (Czech: `/dokoncit`) — the command itself now takes no options. The user can also reach the same modal by tapping the **Finish my profile** button, which appears on a blocked-action ephemeral (see UC-41), on the welcome message field (when the gate is on and the profile is incomplete), or on the pinned card of the read-only `#start-here` channel. 2. The bot opens one modal asking for **Name** (required), **Date of birth** (required, accepts ISO `YYYY-MM-DD` or Czech `D. M. YYYY`), **Jersey number** (optional, 0–99), and — now inside the same modal, as a dropdown rather than a command option — **Gender** (required, Male/Female/Other). 3. The user submits the modal. The bot sends a deferred ephemeral response, then re-validates all four fields client-side (gender against `User.Gender`; birth date first normalised from the Czech format to ISO, then checked with `Auth.BirthDateString` — the same rule the web onboarding form uses; a well-formed but under-`MIN_AGE` date is tagged distinctly so the user sees an age-specific message rather than a generic "invalid date" one) before calling `Guild/CompleteMemberProfile` with the guild ID, Discord user ID, and the four values. 4. The server resolves the guild to a team and the caller to a membership, re-validates the same fields server-side, and — inside a single transaction — completes the profile via `UsersRepository.completeProfile` (`users.name`/`birth_date`/`gender`, `is_profile_complete = true`) and, if a jersey number was supplied, sets the caller's `team_members.jersey_number`. 5. On success the bot also revokes the caller's "Sideline Unverified" Discord role, if held (best-effort; a failed revoke is not reported to the user and self-heals on the member's next guild join or next `/complete`). 6. The bot edits the deferred message with a confirmation echoing the saved values. |
| **Postcondition** | The member's global profile is complete: `name`, `birth_date`, `gender`, and `is_profile_complete = true` on `users` (and optionally `jersey_number` on `team_members`) are saved — the same postcondition as completing web onboarding (UC-01), reached via a Discord-native path instead. If the profile-completeness gate was blocking the member (UC-41), they can now RSVP, claim a training, or take a carpool seat, and their "Sideline Unverified" role and `#start-here` channel access are gone. |
| **Alternate Flow** | If the guild is not linked to a team (`CompleteProfileGuildNotFound`) or the caller is not a member (`CompleteProfileNotMember`), the bot replies with a distinct message for each case. If the gender dropdown was left unselected, or the birth date fails validation as a well-formed-but-too-young date, or fails validation for any other reason, the bot replies with a specific inline error for that field and never reaches the RPC. If server-side re-validation rejects the input (`CompleteProfileInvalidInput`), or the RPC call fails outright, the bot replies with a generic "try again" message; all replies are ephemeral. |
...
| **Alternate Flow** | A freshly created Fio token needs about 5 minutes before Fio accepts calls with it — the connection reports status `activating` during that window and the server retries on its own. Historical movements older than 90 days require the treasurer to open a 10-minute unlock window in Internetbanking (the padlock icon on the API tab) before starting a backfill (`POST /teams/:teamId/bank-sync/backfill`); outside that window the backfill reports `history_locked`. If `account_number`/`bank_code` do not form a valid CZ IBAN, the server rejects the save with `InvalidBankAccount` (400). If the token is valid but reads a *different* account than the one just entered — most often a Fio account prefix the treasurer forgot to fill in, since Fio never returns the prefix in its own answer — `Test connection` answers `account_mismatch` instead of `ok`, and `BankSyncPoller` refuses to ingest ANY statement for the team (no movements, no statement period recorded, no matching) until the account fields are corrected and a poll succeeds; the settings page and the bank transactions page both surface this with a dedicated banner naming both IBANs. |
...
| **Alternate Flow** | The actor can undo a manual or automatic match via `POST /teams/:teamId/bank-transactions/:txId/unmatch` (requires a reason); the linked payment is voided (not deleted, for audit purposes) and the transaction returns to the queue with auto-matching suppressed for that movement until a human matches it again. If the transaction was auto-credited (see below), unmatching also voids the linked credit deposit — refused with `UnmatchCreditSpent` (409) if that credit has already been spent, the same refusal a plain credit-deposit void gives. Several rows can be bulk-marked as `other_income`/`not_relevant` in one call (`POST /teams/:teamId/bank-transactions/bulk`). |
| **Alternate Flow (auto-credit opt-in)** | If the treasurer has turned on **auto-credit** (`bank_sync_config.auto_credit_enabled`, opt-in, default off — a separate switch from `auto_match_enabled`), a transfer whose VS resolves to exactly one member skips the queue even when its amount does not exactly match one open fee: the matcher allocates it across that member's open fees oldest-due-first and adds any remainder to their credit balance (`member_credit_deposits`, `source: 'auto'`), the same allocator behind `POST /teams/:teamId/members/:memberId/settlements` ("settle a member's balance in one action"). A missing, unknown, or ambiguous VS still queues regardless of this setting — money landing on the wrong person is strictly worse than a manual review row. |
...
### UC-41: Profile-Completeness Gate Blocks an Unfinished Profile

| Field | Detail |
|---|---|
| **Actor** | Captain/Admin (configures the gate); Player / Discord User (is blocked by it) |
| **Precondition** | A captain has enabled `requireCompleteProfile` for the team (`PATCH /teams/:teamId/settings`, or the equivalent "Require a complete profile" checkbox in team settings) — off by default for every team. The server-wide `PROFILE_GATE_ENABLED` env var (default enabled) has not been used to disable the gate fleet-wide during an incident. |
| **Main Flow** | 1. A member whose profile is incomplete (missing `name`/`birth_date`/`gender` — most commonly, one who joined the guild directly rather than through the web onboarding flow; see UC-02 Alternate Flow C) attempts to submit an RSVP, claim a training, reserve a carpool seat, or add a car to a carpool. 2. The server checks `team_settings.require_complete_profile` and the member's `users.is_profile_complete` before any other precondition for that action, and fails with a dedicated error: `EventRsvpProfileIncomplete`/`RsvpProfileIncomplete` (RSVP), `ClaimProfileIncomplete` (training claim), or `CarpoolProfileIncomplete` (carpool). 3. On Discord, the bot catches the specific error and replaces its ephemeral response with a message explaining the profile is unfinished, plus a **Finish my profile** button that opens the same modal as `/complete` (UC-34). On the web, the API returns `403` and the UI surfaces the equivalent message. 4. The member completes their profile (UC-34) and repeats the original action, which now succeeds. |
| **Postcondition** | The blocked action is not performed until the member's profile is complete. No data is lost — the member simply retries the same tap once their profile is done; there is no automatic resume of the original action. |
...
| Field | Detail |
|---|---|
| **Actor** | Any active team member |
| **Precondition** | The team has a bank account connected (`bank_sync_config`) and the member has a variable symbol assigned (see UC-37). |
| **Main Flow** | 1. The member opens **Team → My Payments**. 2. Sideline calls `GET /teams/:teamId/my-topup`, which returns the club's computed IBAN, the caller's own variable symbol, and a rendered SPAYD QR code (`data:image/png;...`) — with no amount and no due date, unlike a per-fee QR (UC-13/UC-16). 3. The member scans the code (or pays by hand using the printed IBAN/VS) for any amount, at any time. 4. `BankSyncPoller` later ingests the transfer and matches it by variable symbol like any other movement (UC-38). |
| **Postcondition** | If the team has `auto_credit_enabled` on and the VS resolves unambiguously, the transfer is applied automatically: it pays the member's open fees oldest-due-first and any remainder becomes credit. Otherwise the movement lands in the treasurer's matching queue like any other transfer, to be resolved by hand. |
| **Alternate Flow** | If the club has not connected a bank account, or the member has no variable symbol yet, the affected field(s) come back empty (`Option.none`) rather than an error, and the page shows a "not set up yet" message instead of a broken code. A QR-rendering failure is treated the same way — the account number and variable symbol printed alongside the code are still enough to pay by hand. |
