...
| **Main Flow** | 1. The actor navigates to **Team settings → Email forwarding**. 2. The page loads via `GET /teams/:teamId/email-forwarding`. 3. The actor enables forwarding, selects a target channel and a coach channel, and optionally adds monitored recipient addresses. 4. On save, the web app calls `PUT /teams/:teamId/email-forwarding`. The server upserts `email_forwarding_config` and returns the updated view. 5. The actor copies the inbound webhook URL (composed from the server base URL + the `inbound_token`). 6. The actor configures their email provider to forward to the webhook URL. Optionally the actor rotates the token via `POST /teams/:teamId/email-forwarding/regenerate-token` if the old URL is compromised. |
