...
| **Main Flow** | 1. The actor navigates to **Team settings → Email forwarding**. 2. The page loads via `GET /teams/:teamId/email-forwarding`. 3. The actor enables forwarding, selects a target channel and a coach channel, and optionally adds monitored sender addresses (an allow-list matched against each email's `from`). 4. On save, the web app calls `PUT /teams/:teamId/email-forwarding`. The server upserts `email_forwarding_config` and returns the updated view. 5. The actor copies the inbound webhook URL (composed from the server base URL + the `inbound_token`). 6. The actor configures their email provider to forward to the webhook URL. Optionally the actor rotates the token via `POST /teams/:teamId/email-forwarding/regenerate-token` if the old URL is compromised. |
| **Postcondition** | The `email_forwarding_config` row is upserted. Future inbound emails matching the configured address filter are accepted by the webhook. |
...
|---|---|
| **Actor** | Captain / Admin (any member holding `team:manage`) |
| **Precondition** | An email has been received, AI-summarized, and an approval-request embed has been posted to the team's configured coach channel by the bot. The email is in `pending_approval` status. |
| **Main Flow (Discord path)** | 1. The bot posts an approval embed to the coach channel. The embed shows the AI summary, sender, subject, and received time. Three buttons appear: **Approve**, **Reject**, and optionally **Review & edit in Sideline**. 2. The coach clicks **Approve**. The `EmailApproveButton` handler calls `Email/RecordApproval` RPC. The server transitions the email to `approved`, creates a `post_summary` outbox row in `email_post_sync_events`, and records the coach's Discord snowflake in `approved_by`. 3. The bot disables the Approve/Reject buttons and sends an ephemeral confirmation to the coach. 4. The Email Sync worker polls `email_post_sync_events`, picks up the `post_summary` row, and posts the summary embed to the target channel. |
| **Alternate Flow (web path)** | The coach opens the **Email detail page** (`/teams/:teamId/emails/:emailId`) via the **Review & edit** link. They edit the AI-generated summary text via `PUT /teams/:teamId/emails/:emailId/summary`, then click **Approve** or **Reject** on the page. These call `POST /teams/:teamId/emails/:emailId/approve` or `/reject` respectively, triggering the same server-side logic. |
| **Rejection path** | If the coach clicks **Reject**, the server creates a `post_original` outbox row. The bot posts the original email body (truncated at 3500 characters) to the target channel instead of the summary. |
