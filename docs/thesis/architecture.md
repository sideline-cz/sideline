...
| `EventStartCron` | `* * * * *` (every minute) | Transitions `active` events to `started` status when their `start_at` time passes, emits `event_started` sync events for the bot to delete the owners-thread training claim message (nothing else is posted), and marks personal-channel messages dirty (plus a self-healing sweep for stragglers) so the started event is removed from members' personal channels |
...
| `CoachingStatusCron` | `* * * * *` (every minute) | Emits `coaching_status` sync events on the day of a claimed training (where `coaching_status_sent_at IS NULL`), causing the bot to post a "today's coach is X" announcement to the event's owner-group channel |
...
| `BankSyncPoller` | `0 * * * *` (hourly) | For every team with an enabled, token-configured Fio connection, pulls new bank movements over a rolling window, ingests them into `bank_transactions`, and runs the auto-matcher inline to create `payments` rows for unambiguous matches. Guarded by a per-team distributed poll lease and a per-token cross-replica 30-second throttle. Requires `FIO_TOKEN_ENCRYPTION_KEY` to decrypt stored tokens at poll time. Named `BankSyncPoller.ts` rather than `*Cron.ts`, mirroring `ImapPoller`'s naming, but registered as a long-running effect in `run.ts` the same way as the crons above. |
| `BankTokenExpiryCron` | `0 4 * * *` (daily at 04:00 UTC) | Scans for enabled Fio connections whose token sits at exactly T−14, T−7, or T−1 days from its 180-day expiry, and emits one outbox row per team/threshold for the bot to deliver as a Discord DM to the treasurer. Idempotent per `(team, threshold, token generation)` so a replacement token re-arms all three thresholds. |
...
- **`AppErrorBoundary`:** A React class error boundary wrapping the entire component tree (outside `ThemeProvider`). On a render throw it first checks whether an automatic one-shot reload is still permitted (via `canAutoReloadOnce()`). If so, it renders a minimal `AppReloadingScreen` spinner placeholder and — in `componentDidCatch` — calls `requestAutoReloadOnce('crash-auto')` to silently reload the page once before the user ever sees an error. Only if the auto-reload budget is already spent (or sessionStorage is unavailable) does it fall back to rendering `AppCrashFallback` — the **Reload / Reset app** screen — and logs the error via the Effect runtime if it is initialised, or via `crashBeacon.ts` (direct `navigator.sendBeacon`) if the runtime is not yet up.
- **`reloadGuard`:** Session-storage counters that cap automatic page reloads so a persistent crash never loops forever. Three independent counters are maintained: `sideline-reload-count` (cap: 2) for manual/pre-mount guarded reloads; `sideline-auto-reload-count` (cap: 1) for the crash-boundary's one-shot auto-reload; and `sideline-sw-reload-count` (cap: 1) for SW-update `controllerchange` reloads. `clearReloadGuard()` (called on healthy mount) resets both the main counter and the auto-reload counter so a later unrelated crash in the same session is again allowed its single automatic reload.
