...
The database is organised into eleven functional domains: authentication and user identity, teams and membership, roles and permissions, groups (hierarchical sub-divisions of a team), training types, events and RSVP tracking, Discord bot integration, activity logging and achievements, weekly activity summaries, calendar token management, and in-app notifications. The following diagrams capture the relationships within and between these domains.
