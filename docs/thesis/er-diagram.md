...
The database is organised into fifteen functional domains: authentication and user identity, teams and membership (including onboarding tokens), roles and permissions, groups (hierarchical sub-divisions of a team), training types, events and RSVP tracking, Discord bot integration, activity logging and achievements, weekly activity summaries, calendar token management, in-app notifications, finance (fee management, payment tracking, payment reminders, and expense tracking), weekly challenges, and dashboard layouts. The following diagrams capture the relationships within and between these domains.
