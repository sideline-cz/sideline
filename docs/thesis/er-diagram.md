...
The database is organised into seventeen functional domains: authentication and user identity, teams and membership (including onboarding tokens), roles and permissions, groups (hierarchical sub-divisions of a team), training types, events and RSVP tracking, Discord bot integration, activity logging and achievements, weekly activity summaries, calendar token management, in-app notifications, finance (fee management, payment tracking, payment reminders, and expense tracking), weekly challenges, dashboard layouts, carpools, email forwarding (per-team inbound email configuration, stored messages, post outbox, and attachment storage), and event roster attendance (linking events to rosters with an auto-approve or owner-approval workflow). The following diagrams capture the relationships within and between these domains.
