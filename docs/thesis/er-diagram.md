...
The database is organised into sixteen functional domains: authentication and user identity, teams and membership (including onboarding tokens), roles and permissions, groups (hierarchical sub-divisions of a team), training types, events and RSVP tracking, Discord bot integration, activity logging and achievements, weekly activity summaries, calendar token management, in-app notifications, finance (fee management, payment tracking, payment reminders, and expense tracking), weekly challenges, dashboard layouts, carpools, and email forwarding (per-team inbound email configuration, stored messages, post outbox, and attachment storage). The following diagrams capture the relationships within and between these domains.
...
erDiagram
    ...
    events ||--o{ event_join_requests : "tracks"
...
erDiagram
    ...
    team_members ||--o{ event_rsvps : "submits"
    events ||--o{ event_join_requests : "tracks"
    team_members ||--o{ event_join_requests : "submits"
    team_members o|--o{ event_join_requests : "decides"
```
...
erDiagram
    ...
    event_sync_events {
        ...
        UUID join_request_id "nullable"
        TEXT join_request_message_id "nullable"
        TEXT requester_display_name "nullable"
        TEXT request_message "nullable"
        TEXT decided_by_display_name "nullable"
        TIMESTAMPTZ processed_at
        TEXT error
        TIMESTAMPTZ created_at
    }
...
| `event_join_requests` | Join requests submitted by team members for tournament events. Status is `pending`, `accepted`, or `declined`. Accepted rows represent confirmed tournament attendance. |
