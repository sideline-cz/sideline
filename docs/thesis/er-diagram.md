...
The database is organised into thirteen functional domains: authentication and user identity, teams and membership (including onboarding tokens), roles and permissions, groups (hierarchical sub-divisions of a team), training types, events and RSVP tracking, Discord bot integration, activity logging and achievements, weekly activity summaries, calendar token management, in-app notifications, and finance (fee management, payment tracking, payment reminders, and expense tracking). The following diagrams capture the relationships within and between these domains.
