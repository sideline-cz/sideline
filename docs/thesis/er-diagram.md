...
The Finance subsystem tracks fee definitions, per-member assignments, and payment records. `paid_minor` on `fee_assignments` is kept current by a PostgreSQL trigger; the `fee_assignment_status_v` view computes the displayed status.
