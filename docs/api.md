...
| `discordEventsChannelId` | `Snowflake \| null` | Yes | **Deprecated** (remove-global-events-board Release A): the shared "global" events board no longer exists, so this field is no longer read by the bot or shown in the web UI. Kept on the wire for compatibility until Release B removes it. |
...
| `discordEventsChannelId` | `Snowflake \| null` | No | — | **Deprecated** (remove-global-events-board Release A): accepted and stored for backward compatibility, but no longer read by the bot; the web UI no longer sends it |
...
| `channelFormat` | `string` | The team's current `discordChannelFormat` template string (e.g. `{emoji}│{name}`). Clients can use this to preview how a channel name will appear in Discord. |
...
| `name` | `string` | No | Logical channel name (clean, without emoji or format prefix). The Discord channel name is composed from this plus `emoji` via the team format template. |
| `emoji` | `string \| null` | Yes | Optional emoji stored alongside the logical name; `null` for channels without an emoji or for Discord-only channels |
...
| Field | Type | Required | Description |
|---|---|---|---|
| `name` | `string` | Yes | Non-empty logical channel name (must be unique among active channels for this team). The actual Discord channel name is composed server-side from this name and the team's `discordChannelFormat` template (via `applyDiscordFormat`). |
| `emoji` | `string \| null` | Yes | Optional emoji used to compose the Discord channel name via the format template; stored separately from the logical name (null for none) |
| `category` | `string \| null` | Yes | Sideline-side category label (null for no category) |

**Response:** `201 Created` — `ChannelDetail`

`ChannelDetail` extends `ChannelInfo` (all fields above, including `emoji`) with:
...
Archives a managed channel. Sets `archived = true`; emits a `channel_archived` / `managed` sync event. If an archive category is configured in team settings the bot moves the Discord channel there; otherwise the bot deletes the Discord channel (delete-fallback). The `discord_channel_id` is **preserved** on the Sideline record after archiving — this allows the restore endpoint to re-activate the channel without reprovisioning it.
...
#### `POST /teams/:teamId/discord-channels/:discordChannelId/restore`

Restores a single archived Discord channel — moves it out of the archive category back to uncategorized (`parent_id = null`). Emits a `channel_restored` / `discord` sync event (or `channel_restored` / `managed` for managed channels) that the bot handles by calling `updateChannel { parent_id: null }`. The `discord_channel_id` link on the managed-channel row is preserved through archive and restore, so this endpoint re-activates an already-linked channel without reprovisioning it.
...
|---|---|---|
| `teamId` | `TeamId` (string) | Team ID |
| `discordChannelId` | `Snowflake` (string) | Discord channel snowflake to restore |
...
| `ChannelForbidden` | 403 | Missing `group:manage` permission |
| `ChannelNotFound` | 404 | The Discord channel is not found in this team's guild |
| `ChannelNotRestorable` | 409 | The channel is not currently in the archive category (already active), is a category channel, or cannot be found |

---

#### `POST /teams/:teamId/discord-channels/bulk-restore`

Restores multiple archived Discord channels in a single call. Each channel is processed independently; failures for individual channels do not abort the remaining ones. Duplicate IDs in the payload are de-duped before processing.
...
**Request Body:** `BulkArchiveDiscordChannelsRequest` (same shape as bulk-archive)

| Field | Type | Required | Description |
|---|---|---|---|
| `discordChannelIds` | `Snowflake[]` | Yes | IDs of the Discord channels to restore |

**Response:** `200 OK` — `ChannelBulkRestoreResult`

| Field | Type | Description |
|---|---|---|
| `restored` | `Snowflake[]` | IDs of channels successfully queued for restore |
| `skipped` | `{ discordChannelId: Snowflake, reason: string }[]` | Channels skipped without error; see skip reasons below |
| `failed` | `{ discordChannelId: Snowflake }[]` | Channels that encountered an unexpected error during processing |
...
| Reason | When |
|---|---|
| `already_active` | Channel is not in the archive category (already active) |
| `not_archived` | Channel row exists but is not marked archived |
| `is_category` | Channel is a category (type 4) |
| `not_found` | Channel ID is not present in this guild's channel mirror |
...
`POST {API_PREFIX}/email/inbound/:token` — receives emails from external providers. Mounted under `API_PREFIX` (empty in local dev → `/email/inbound/:token`; `/api/email/inbound/:token` behind the reverse proxy in preview/production, which only forwards `/api/*` to the server). The web UI builds this URL from `SERVER_URL`, which already includes the prefix. Authenticated by a per-team `inbound_token` (path parameter) and a global HMAC-SHA-256 signature (`X-Signature` header; secret = `EMAIL_WEBHOOK_SIGNING_SECRET`). Returns `202 Accepted` on success. Size limits: 36 MB total body, 10 MB per attachment, 25 MB total attachments. If `monitored_addresses` is configured, it is an allow-list of permitted senders — emails whose `from` does not match one of the addresses are silently dropped (`200 OK`). It is a raw route, not part of the authenticated API group.
...
Issues one real, throttled Fio `/periods` probe (a 2-day window) to validate the stored token, without ingesting movements. This is a SINGLE-attempt verdict, not a re-check of the seven-rank `BankSyncConfigView.status` ladder above — a probe that comes back green clears the poller's exponential backoff (`next_attempt_at`) so a stuck retry schedule doesn't outlive a fixed token, but it never calls `recordSuccess`: it does not reset `last_error_code`/`last_success_at`/`consecutive_failure_count` or clear `coverage_warning`, because the probe's 2-day window is not evidence the poller's 14-day import actually works. The two can legitimately disagree. The backoff clear is gated on the account cross-check: an `account_mismatch` verdict writes **nothing at all**, because that backoff is the poller's deliberate state and clearing it would only send the poller back to a config it will refuse again.

**Auth:** Bearer token (AuthMiddleware) · **Required Permission:** `finance:manage_fees`

**Response:** `200 OK` — `BankSyncTestResult` — `{ ok: boolean, status: BankSyncTestStatus, message: string | null, accountIban: string | null }`. `ok` is a derived alias, always `status === 'ok'`. `message` is now always `null` (`@deprecated` — user-facing copy is client-side, keyed off `status`). `accountIban` is Fio's own `info.iban` from the probe's decoded statement, `Some` when `status` is `'ok'` or `'account_mismatch'` and Fio supplied it.

`BankSyncTestStatus` values: `'ok'`, `'account_mismatch'` (Fio accepted the token and answered, but its `info.iban` differs — whitespace- and case-insensitively — from the IBAN computed from `accountPrefix`/`accountNumber`/`bankCode`; `ok` is `false`, and the hourly poller refuses to ingest anything in this state. No comparison is made, and the verdict stays `'ok'`, when either IBAN is absent), `'invalid'` (Fio's bodyless HTTP 500 — a dead/expired/revoked token, the only documented dead-token signal), `'rate_limited'` (per-token throttle budget exceeded — answered immediately, without waiting out the throttle or burning a poller slot), `'history_locked'`, `'unreachable'` (a transport failure, or any other unexpected non-200/500, that is not itself a Fio verdict on the token — the poller records this distinctly as `last_error_code: 'unreachable'`, which does **not** count toward the `invalid` escalation the way `'fio_error'` does), `'misconfigured'` (server-side, e.g. `FIO_TOKEN_ENCRYPTION_KEY` unset — nothing the treasurer can fix), `'no_token'` (no token saved yet).
...
| `Event/GetChannelsWithStoredMessages` | — → `{ discord_channel_id, guild_id }[]` | Returns all channels that have a stored event embed message; was used by the bot's `recoverDeletedMessages` startup task, which was removed along with the shared events board in the remove-global-events-board Release A |
...
`RosterChannelCreatedEvent` (the domain type decoded from `roster` / `channel_created` outbox rows) carries a `target_category_id` field (`Snowflake | null`) sourced from `channel_sync_events.target_category_id`. When non-null the bot passes `parent_id: target_category_id` to the Discord channel-create call. If the category ID is stale or deleted, the channel-create call fails with a permanent Discord error (any non-429 4xx — e.g. `10003` Unknown Channel, or `50035` Invalid Form Body referencing `parent_id`). The bot then skips the parent and retries the create at the guild root (the root create is itself retried on transient failures). Transient errors (5xx) instead retry with the original `parent_id` intact. Only applies when a new channel is being created; when linking an existing channel (`existing_channel_id` is set) `target_category_id` is ignored.
...
| `Channel/GetExpectedRoleHolders` | `team_id`, `discord_role_id` → `RosterMemberDiscord[]` | Returns the union of active members across all rosters that share the given `discord_role_id` for the team. Called by `handleRosterRoleReconcile.ts` to determine who should hold the role before computing extras to remove |
...
| `Poll/RemoveOptions` | `guild_id`, `discord_user_id`, `poll_id`, `option_ids: PollOptionId[]` → `PollView` | Removes one or more options from an open poll. Requires `poll:manage`. Votes for removed options are deleted via FK cascade. Remaining options are renumbered to contiguous 0-based positions so their regional-indicator letters (🇦🇧🇨…) stay correct. A poll must always retain at least 2 options. Errors: `PollGuildNotFound`, `PollNotMember`, `PollForbidden`, `PollNotFound`, `PollClosed`, `PollOptionNotFound`, `PollTooFewOptions`. |
