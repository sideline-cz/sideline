...
| `Guild/IdentifyEventsChannel` | `guild_id`, `channel_id`, `discord_user_id` → `{ kind: 'global' \| 'personal' \| 'none', team_id: TeamId \| null, team_member_id: string \| null }` | Classifies a channel for the `/refresh-events` command: returns `global` if the channel is the team's configured global events channel, `personal` if it is the calling Discord user's own personal events channel (with `team_id` and `team_member_id` populated), or `none` if neither. Implemented via `PersonalEventChannelsRepository.findOwnedPersonalChannel` for the personal case |
