...
Both row 1's button highlight and row 2's message-management row (Edit/Clear/Add message buttons) are built from the same value — `entry.my_response_actual`, falling back to `entry.my_response` for rolling-deploy safety. Against a server on this release the two fields are identical (no projection); `my_response_actual` only matters for the window where a bot on this release talks to an older server that still projects `coming_later` down to `my_response: 'maybe'` — using the true value there keeps a real `coming_later` RSVP from being confused with `maybe` (e.g. wrongly highlighting the "Not sure" button, or rendering the "Clear message" button — never shown for `coming_later` or `maybe`, since clearing a mandatory comment is illegal).
