...
| `training_claim_request` | `handleTrainingClaimRequest.ts` | Posts a claim-board message to the event's owner-group channel (resolved from `discord_target_channel_id` in the sync event). The embed shows event details, an orange "Unclaimed" colour, and a "Claim" primary button. After posting, saves the resulting message ID via `Event/SaveClaimDiscordMessageId`. Then creates a Discord thread from the message (name: `"{title} · YYYY-MM-DD"`, 7-day auto-archive); the thread ID is persisted via `Event/SaveClaimThreadId`. Thread creation is best-effort — a failure is logged and swallowed without failing the handler. An idempotency check via `Event/GetClaimInfo` skips thread creation if `claim_thread_id` is already set. |
...
| `Event/SaveClaimThreadId` | Persist the Discord thread ID created from the claim-board message; called by `handleTrainingClaimRequest` after thread creation |
| `Event/GetClaimInfo` | Fetch current claim state (`EventClaimInfo`) for a training; returns `None` if the event does not exist. The returned `EventClaimInfo` now includes a `claim_thread_id` field (Discord snowflake of the claim thread, or `None` if not yet created) |
