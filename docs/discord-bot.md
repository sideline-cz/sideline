...
**"Coming later" is not instant-submit:** the third response button (labeled 🕒 **Coming later**, was ❓ **Maybe**) never uses this custom ID. Because `coming_later` requires a non-blank comment, its button carries custom ID `u-add-msg:{teamId}:{eventId}:coming_later` instead — clicking it opens the same message modal used by the "Add a message" button, pre-targeting the `coming_later` response, and the RSVP is only submitted once the modal is filled in and submitted.
...
| `Event/GetUpcomingEventsForUser` | Fetch paginated upcoming events with the invoking user's RSVP status; used by `/event list` and pagination/RSVP buttons on the per-user embed. Each entry carries both the legacy-projected `my_response` (`yes`/`no`/`maybe`) and the true `my_response_actual` (which can be `coming_later`), so the embed builder can render the correct message-management buttons |
...
| `Event/GetYesAttendeesForEmbed` | Fetch attending-member display names for the embed (`yes` and `coming_later`/legacy `maybe`); accepts an optional `member_group_id` to filter results to the event's member group and its descendants via `WITH RECURSIVE descendant_groups` (used by event sync and start announcements) |
