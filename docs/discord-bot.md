...
Appears on each per-user upcoming event message (produced by `/event list` and the overview show button). Clicking **Yes** or **No** saves the RSVP immediately and refreshes that message to show the updated response and counts.

**Custom ID pattern:** `upcoming-rsvp:{eventId}:{teamId}:{response}` where `response` is `yes` or `no`.

**"Coming later" is not instant-submit:** the third response button (labeled 🕒 **Coming later**, was ❓ **Maybe**) never uses this custom ID. Because `coming_later` requires a non-blank comment, its button carries custom ID `u-add-msg:{teamId}:{eventId}:coming_later:v` instead — clicking it opens the same message modal used by the "Add a message" button, pre-targeting the `coming_later` response, and the RSVP is only submitted once the modal is filled in and submitted. The trailing `:v` marker exists only to keep this custom ID distinct from row 2's edit-message button (`u-add-msg:{teamId}:{eventId}:coming_later`, no marker), which always renders once the member's response is `coming_later` (the comment is mandatory, so the edit button is unconditional); without it the two buttons on the same message collide, Discord rejects the whole message with error 50035, and the card becomes unrenderable — see the "Personal Events Reconcile Worker" section below for what that did to personal-channel cards before it was fixed.
...
The embed's message-management row (Edit/Clear/Add message buttons) is built from the *unprojected* response — `entry.my_response_actual`, falling back to the projected `my_response` for rolling-deploy safety — not the legacy-projected `my_response` used for the response buttons themselves, so a real `coming_later` RSVP is never confused with a legacy `maybe` when deciding whether to render the "Clear message" button (never rendered for `coming_later`, since clearing the message would leave the mandatory comment blank).

**Errors:** same as RSVP Button, plus `GuildNotFound` (shows "not a member" message) and `RsvpMessageRequired` (shown when a `coming_later` submission has no non-blank message).
...
2. For each member: calls `Guild/ReservePersonalChannel` (lease-based `INSERT ... ON CONFLICT DO UPDATE`) to claim the slot. A fresh reservation or one whose `updated_at` is more than 15 minutes stale (e.g. a crashed worker left it unprovisioned) is (re-)claimed and returns `reserved = true`; a recent reservation held by another in-flight worker or an already-provisioned row returns `reserved = false` and the member is skipped. This lease lets a member abandoned mid-provisioning be retried on a later tick instead of being permanently skipped.
...
6. On success, calls `Guild/SavePersonalChannelId` (now includes `channel_format`) to write the Discord snowflake and the applied format back. If category overflow is detected (Discord HTTP 400 / JSON error code 50035 with a nested `CHANNEL_PARENT_MAX_CHANNELS` sub-error on `parent_id` — category has reached the 50-channel limit), allocates a new overflow category row via `Guild/AllocatePersonalOverflowCategory`, fetches the base category name, calls `createGuildChannel(GUILD_CATEGORY)` to create the new Discord category (named identically to the base category — no numeric suffix), saves the Discord snowflake via `Guild/SavePersonalOverflowCategoryId`, and retries the channel creation in the new category.
...
| `Event/GetUpcomingEventsForUser` | Fetch paginated (`limit`/`offset`) upcoming events with the invoking user's RSVP status; used by `/event list`. Each entry carries both the legacy-projected `my_response` (`yes`/`no`/`maybe`) and the true `my_response_actual` (which can be `coming_later`), so the embed builder can render the correct message-management buttons. `UpcomingRsvpButton`/`UpcomingClearMessageButton`/`UpcomingRsvpModal` use the unpaginated `Guild/GetAllUpcomingEventsForUser` instead, not this RPC |
| `Event/GetTrainingTypesByGuild` | Fetch training type choices for autocomplete |
| `Event/SubmitRsvp` | Record a member's RSVP response (`yes`, `no`, `maybe`, or `coming_later` — the latter requires a non-blank `message` or fails with `RsvpMessageRequired`); payload includes `clearMessage: boolean`; returns `SubmitRsvpResult` with late-RSVP flag, optional notification channel, and `message: Option<string>` |
| `Event/GetRsvpCounts` | Fetch yes/no/maybe counts for an event (`coming_later` responses are folded into the `maybe` count) |
| `Event/GetRsvpAttendees` | Fetch paginated attendee list (each entry's response is projected to the legacy `yes`/`no`/`maybe` vocabulary) |
...
| `Event/GetYesAttendeesForEmbed` | Fetch attending-member display names for the embed (`yes` and `coming_later`/legacy `maybe`); accepts an optional `member_group_id` to filter results to the event's member group and its descendants via `WITH RECURSIVE descendant_groups` (used to rebuild personal-channel and upcoming-RSVP embeds; no longer called by `handleStarted.ts`, which posts nothing) |
