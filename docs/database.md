...
**Notes**: `end_date` was made nullable in migration `1741600000` when the rolling horizon model was introduced. `days_of_week` replaced the single `day_of_week INTEGER` column in migration `1742400000`, allowing a series to recur on multiple days per week. `location_url` is an optional public `https://` URL attached to the location text; requires `location` to be non-empty (added in migration `1746100000`). `start_time`/`end_time` are `HH:MM[:SS]`, and their dialect is **per-row, not unconditional**: `times_are_team_local = TRUE` means wall-clock in the team's own `team_settings.timezone`; `FALSE` (the legacy, pre-fix meaning, and the DB column default a still-running previous-release server or a client that omits the `timesAreTeamLocal` payload flag both land on) means a UTC time-of-day — but it is not every row's value even as of the `1791700000` release: a create can also explicitly declare `TRUE` (via the `timesAreTeamLocal` payload flag on `POST /teams/:teamId/event-series`), and `testing` in particular already carries `TRUE` rows written before this migration by a previewed, unreleased server. `applications/server/src/utils/seriesTimeDialect.ts`'s `resolveSeriesOccurrenceInstant(dateStr, time, timezone, timesAreTeamLocal)` dispatches on the flag per occurrence — its `TRUE` branch delegates to `resolveOccurrenceInstant` (matching how `rsvp_reminder_time`/`rules_quiz_time` already work), its `FALSE` branch is the legacy `` `${dateStr}T${time}Z` `` expression, unchanged. This is a deliberate two-release split (`.work-plans/timezone-migration-deploy-window.md`): `times_are_team_local` was added, defaulted `FALSE`, by migration `1791700000_add_series_times_team_local_flag`, converting nothing; the follow-up migration `1792100000_series_time_is_team_local` converts existing rows to `TRUE`-semantics (anchored at each series' own `start_date`) and re-anchors already-materialized future, non-hand-edited `events` rows to match. The column is never dropped since it is the only thing that makes that one-time conversion `UPDATE` safely re-runnable by hand.
