...
`membership_selection_deadline` added in migration `1792900000_membership_selection` (Slice 2 of "Setup memberships" — players select which plan they want, with an optional deadline). `NULL` means selection stays open indefinitely. Deliberately placed on `teams`, not `team_settings`: a `teams` row always exists, so `setSelectionDeadline` is a plain `UPDATE`, whereas `team_settings` is a one-to-one extension that would need an upsert — and creating that row is not side-effect-free, since the RSVP-reminder, claim-request, and coaching-status crons `INNER JOIN team_settings` and `rsvp_reminders_enabled` DEFAULTs to `true`, so a team that had never opened its settings would suddenly start getting Discord reminders purely as a side effect of the row now existing. Set via `PUT /teams/:teamId/membership-selection-deadline` (requires `finance:manage_fees`).
...
| `show_attendee_list` | BOOLEAN | NOT NULL | `true` |
| `rsvp_reminder_dms` | BOOLEAN | NOT NULL | `true` |
| `personal_channels_split` | BOOLEAN | NOT NULL | `false` |
| `membership_plan_id` | UUID | FK → `membership_plans(id)` ON DELETE SET NULL | `NULL` |
...
**Notes**: The original `role TEXT` column was replaced by the `member_roles` junction table across migrations `1740614400` and `1740700800`. `active` was added in migration `1740355200`. `jersey_number` was moved from `users` in migration `1740990000`; it can also be set from Discord via the `/complete` slash command (`Guild/CompleteMemberProfile` RPC → `TeamMembersRepository.setJerseyNumber`), alongside completing the profile. `missed_rsvps` added in migration `1790300000_add_team_members_missed_rsvps` — a consecutive count of events for which an invited default-role member did not respond. Incremented by `EventStartCron` when an active event transitions to `started` and the member (who holds the team's default role — configured `is_default` role or built-in `Player`, see `holdsDefaultRoleWhere` in `roles` above — and belongs to the event's member group) has no `event_rsvps` row for that event. Reset to `0` by any RSVP submission (yes, no, maybe, or coming later). `variable_symbol` added in migration `1792000000_add_team_member_variable_symbol` — the bank payment variable symbol used to auto-match Fio transactions to this member's fee assignments and to build their SPAYD payment QR codes (see [12. Finance](#12-finance) and the Bank Sync section below). Set via `PATCH /teams/:teamId/members/:memberId` or the bulk `POST /teams/:teamId/members/variable-symbols/assign` endpoint — never written directly by the matcher. `show_attendee_list`, `rsvp_reminder_dms`, and `personal_channels_split` added in migration `1792200000_add_team_member_event_preferences` (Nastavitelná docházka / configurable attendance) — per-member preferences the member controls themselves from the **My events in Discord** card on their web profile page, via `GET`/`PATCH /teams/:teamId/me/event-preferences`. `show_attendee_list` (default `true`) hides the "Going" attendee-name list on the member's own event card (both the personal-channel embed and `/event list` ephemeral cards) when `false`; the ✅/❌/🕒 RSVP counts and the **Attendees** button are unaffected — it hides names, not turnout — and it never affects what any other member sees. `rsvp_reminder_dms` (default `true`) opts the member out of the RSVP reminder DM (`Event/GetRsvpReminderSummary`'s `nonResponders` list is filtered to `rsvp_reminder_dms = true` before the bot sends); it does not affect the organiser-facing non-responders view or `missed_rsvps` accounting, both of which read the same members table unfiltered. `personal_channels_split` (default `false`) switches the member's personal Discord events channel from one combined channel to three, bucketed by event type — see the `personal_event_channels.bucket` notes below. `membership_plan_id` added in migration `1792900000_membership_selection` (Slice 2 of "Setup memberships"). `NULL` means "on the team's default plan" — a COMPUTED fallback resolved web-side from the active plan list, never backfilled: a captain changing which plan is default must move every `NULL` member along with it. Same reasoning applies when the chosen plan is later archived — both cases fall back to the team default the same way. `ON DELETE SET NULL`, not `RESTRICT` — and not for the reason one might assume: deleting a team whose own member points at its own plan succeeds under *both* actions, because non-deferrable FK triggers fire at the END of the enclosing statement, by which point the `team_members.team_id -> teams` cascade has already removed the referencing row (measured, not reasoned). The actual reasons are that nothing stops a `team_members` row from naming *another* team's plan, and under `RESTRICT` one such row makes that plan's team permanently undeletable with an opaque `23503`; and that `RESTRICT` guards nothing real here, since no code path hard-deletes a plan (`deleteMembershipPlan` archives). A regression to `RESTRICT` is caught by the `delete_rule` assertion in the migration test, not by a delete-a-team test, which stays green either way. Set via the member's own `PUT /teams/:teamId/me/membership-plan` (membership-gated only — any member picks their own plan, no captain permission needed); rejected with `409 MembershipSelectionClosed` once `teams.membership_selection_deadline` has passed.
...
| `timezone` | TEXT | NOT NULL, CHECK (`now() AT TIME ZONE timezone IS NOT NULL`) | `'Europe/Prague'` |
...
| `require_complete_profile` | BOOLEAN | NOT NULL | `false` |
| `created_at` | TIMESTAMPTZ | NOT NULL | `now()` |
| `updated_at` | TIMESTAMPTZ | NOT NULL | `now()` |

**Notes**: Created in migration `1741600000`. RSVP reminder columns (`min_players_threshold` and `rsvp_reminder_hours`) added in `1742500000`. Discord channel columns added in `1742100000`. The six per-event-type columns from that migration (`discord_channel_training`, `discord_channel_match`, `discord_channel_tournament`, `discord_channel_meeting`, `discord_channel_social`, `discord_channel_other`) were removed from code in a later release (event routing had already moved to `discord_events_channel_id` alone); the columns themselves were dropped in migration `1790300015_drop_team_settings_channel_targeting` after the code-only release fully rolled out. `create_discord_channel_on_roster` added in `1743500000`. `discord_archive_category_id` added in `1743600000`. `discord_channel_cleanup_on_group_delete` and `discord_channel_cleanup_on_roster_deactivate` added in `1743600000`. `discord_role_format` and `discord_channel_format` added in `1743700000`. `discord_channel_late_rsvp` added in `1743900000`. `rsvp_reminder_days_before`, `rsvp_reminder_time`, `reminders_channel_id`, and `timezone` added in `1745800000` (`rsvp_reminder_v2`); `rsvp_reminder_hours` was dropped in the same migration with data migrated to `rsvp_reminder_days_before`. `weekly_summary_channel_id` added in `1780000000` (`weekly_summary`). `claim_request_days_before` added in migration `1789300000_improve_coach_assigning`; controls how many days before a training event the `TrainingClaimRequestCron` posts the claim-request message (default 3); must be ≥ 0 (0 means the message is posted on the day of the training). `rsvp_reminder_days_before` specifies how many days before the event the reminder fires (0 disables reminders). `rsvp_reminder_time` is the time-of-day (in the team's `timezone`) at which the reminder fires; the cron matches within a 5-minute window. `reminders_channel_id` is the Discord channel where RSVP reminders and event-start announcements are posted; when `NULL` the bot falls back to the event owner-group's channel. `timezone` is a valid IANA timezone string (default `'Europe/Prague'`); guarded by the `team_settings_timezone_check` CHECK constraint (`now() AT TIME ZONE timezone IS NOT NULL`) added in migration `1792000005_team_settings_timezone_check`, which first sanitizes any row whose `timezone` isn't in `pg_timezone_names` back to `'Europe/Prague'` so the `ADD CONSTRAINT` itself can't fail on bad data. The CHECK asserts only that Postgres can resolve the zone — abbreviations and POSIX offsets (`EST`, `UTC+3`) pass it but are absent from `pg_timezone_names`, so readers must still use the `pg_timezone_names` join. `discord_channel_late_rsvp` is the channel where the bot posts a notification when a member submits or changes their RSVP after the reminder was sent. `create_discord_channel_on_group` and `create_discord_channel_on_roster` control whether the bot automatically creates Discord channels when a group or roster is created. `discord_archive_category_id` is the Discord category channel ID to move channels into when `archive` mode is active. `discord_roster_category_id` is the Discord category channel ID under which new roster channels are created; when set, reactivated roster channels are also re-created in this category; when `NULL` the bot places the channel at guild root. Added in migration `1790100000_add_roster_category`. `discord_personal_events_category_id` is the Discord category channel ID under which the bot creates private per-member event channels; when `NULL` the personal-channel feature is disabled for this team. Both columns added in the personal-events rework migration. `discord_events_channel_id` used to be the Discord channel where the bot posted a shared "global" event embed visible to all members; the shared-board feature was removed from code in the remove-global-events-board Release A (the bot no longer reads this column, and the web team-settings UI no longer exposes a field for it) — events now live only in members' personal event channels and the web app. The column itself, and the wire field that carries it, are kept for one more release (tolerant decode on both ends) and are dropped in the Release B follow-up migration. `discord_personal_events_group_id` is an optional UUID FK to `groups(id)` (ON DELETE SET NULL); when set, only members of that group and its descendant groups receive a personal channel — all others are routed to the global events channel and any existing personal channels for excluded members are de-provisioned. Added in migration `1790300010_add_personal_events_group_and_name_format`. `discord_personal_events_channel_format` is the template string used to name each member's personal events channel; must contain `{name}` (member display name) or `{discord_id}` (Discord user snowflake); defaults to `'events-{discord_id}'`. Added in the same migration. `discord_channel_cleanup_on_group_delete` and `discord_channel_cleanup_on_roster_deactivate` each accept one of three values: `'nothing'` (keep the channel, delete only the role and mapping), `'delete'` (delete the channel and role), or `'archive'` (move the channel to `discord_archive_category_id` and delete the role — for a group, this is a no-op if `discord_archive_category_id` is unset; for a roster it falls back to `'delete'` instead). `discord_role_format` and `discord_channel_format` are template strings used to format Discord role and channel names respectively; they must contain `{name}` and may contain `{emoji}`. `weekly_summary_channel_id` is the Discord channel where the bot posts the weekly summary embed each Sunday at 20:00 local team time; when `NULL` the cron skips the team. `max_missed_rsvps` added in migration `1790300001_add_team_settings_max_missed_rsvps`; controls the threshold used to suppress RSVP reminder DMs, the public reminder embed's non-responder list, and the captain-facing `GET …/rsvps/non-responders` response — members whose `team_members.missed_rsvps` equals or exceeds this value are excluded from all three (must be between 1 and 50; default 4). `require_complete_profile` added in migration `1792300001_add_team_settings_require_complete_profile`; the captain's per-team opt-in for the profile-completeness gate that blocks a member with an incomplete profile (missing name/birth date/gender) from RSVPing (`Event/SubmitRsvp`, `submitRsvp`), claiming a training (`Event/ClaimTraining`), or reserving/adding a carpool car (`Carpool/ReserveSeat`, `Carpool/AddCar`) — read via a `LEFT JOIN team_settings` on each of those write paths' existing membership SELECT, so the gate adds no extra query. Un-blocking actions (`Carpool/LeaveCarpool`, `Carpool/RemoveCar`, `Event/UnclaimTraining`) and `Carpool/AssignSeat` are deliberately never gated. `DEFAULT false` so no existing team is affected on deploy; also short-circuited server-wide by the `PROFILE_GATE_ENABLED` env var (default enabled — see `docs/deployment.md`), which exists purely as an incident kill switch since the real off switch is this per-team column.
...
| `was_default` | BOOLEAN | NOT NULL | `false` |
| `created_at` | TIMESTAMPTZ | NOT NULL | `now()` |

**Unique**: `idx_roles_team_name` on `(team_id, name)` (unique index); `idx_roles_team_default` on `(team_id) WHERE is_default AND NOT is_archived` (partial unique index — at most one non-archived default role per team, DB-enforced)

**Notes**: `is_archived` added in migration `1741000002`. Built-in roles cannot be deleted but can be archived. Custom roles are soft-deleted (`is_archived = true`, `member_roles`/`role_groups`/`role_permissions` rows untouched); the effective-roles derivation (see `role_groups` below) filters `is_archived` on every read, so an archived role stops granting permissions immediately. `is_default` added in migration `1792500001_add_roles_is_default` — marks the role newly-joining members are assigned (invite accept, `autoJoinTeams`, Discord guild registration), settable via `PUT /teams/:teamId/default-role`. The migration backfills `is_default = true` onto every team's built-in `Player` role so existing teams keep today's behaviour. The **resolved** default a team currently hands out is `is_default = true` OR (no row has `is_default = true` AND the role is the built-in `Player`) — see `TeamMembersRepository.getDefaultRoleId` and the shared `holdsDefaultRoleWhere` predicate in `applications/server/src/repositories/effectiveRoles.ts`. `was_default` added in migration `1792600001_add_roles_was_default` — a STICKY companion set whenever a role becomes the team's default and never cleared when a later role takes over, so it records every role the team has ever handed out. The RSVP-eligibility queries (`findNonRespondersForEvent`, `incrementMissedForEventNonResponders`) gate on `was_default` (OR built-in `Player`, kept as a redundant floor against a row created outside `setDefaultRole`) via `holdsRsvpEligibleRoleWhere`, not on `is_default`: a team's second default change would otherwise strand the cohort that joined under the first custom default (no longer `is_default`, never `Player`), silently dropping them from reminder DMs and `missed_rsvps`. There is no unique index on `was_default` — several rows per team is the point.
...
Rules that automatically assign team members to a group based on automatic group criteria (age range, gender, and/or required pre-existing group membership — any combination, AND semantics). Evaluated by the `AgeCheckCron` (daily at 2:00 AM). Archiving a group does not touch its `age_threshold_rules` rows — `AgeThresholdRepository.findRulesByTeamId` filters them out by joining on `groups.is_archived = false`, so a rule left pointing at an archived group is simply skipped rather than deleted or continuing to fire.
...
**Notes**: `end_date` was made nullable in migration `1741600000` when the rolling horizon model was introduced. `days_of_week` replaced the single `day_of_week INTEGER` column in migration `1742400000`, allowing a series to recur on multiple days per week. `location_url` is an optional public `https://` URL attached to the location text; requires `location` to be non-empty (added in migration `1746100000`). `start_time`/`end_time` are `HH:MM[:SS]`, and their dialect is **per-row, not unconditional**: `times_are_team_local = TRUE` means wall-clock in the team's own `team_settings.timezone`; `FALSE` (the legacy, pre-fix meaning, and the DB column default — still `FALSE`, unchanged by `1792100000` — a client that omits the `timesAreTeamLocal` payload flag lands on) means a UTC time-of-day. `applications/server/src/utils/seriesTimeDialect.ts`'s `resolveSeriesOccurrenceInstant(dateStr, time, timezone, timesAreTeamLocal)` dispatches on the flag per occurrence — its `TRUE` branch delegates to `resolveOccurrenceInstant` (matching how `rsvp_reminder_time`/`rules_quiz_time` already work), its `FALSE` branch is the legacy `` `${dateStr}T${time}Z` `` expression, unchanged. This was a deliberate two-release split (`.work-plans/series-time-conversion.md`): `times_are_team_local` was added, defaulted `FALSE`, by migration `1791700000_add_series_times_team_local_flag`, converting nothing; the follow-up migration `1792100000_series_time_is_team_local` has since converted every row that was `FALSE` at the time it ran to `TRUE`-semantics (anchored at each series' own `start_date`) and re-anchored the already-materialized future, non-hand-edited `events` rows generated from those series to match. Essentially every row is `TRUE` now, but the `FALSE` branch is **permanent**, not a transitional carve-out: a client that omits the `timesAreTeamLocal` payload flag on a create still writes a `FALSE` row, and the column default is still `FALSE` — `1792100000` shipped without the column-default flip its earlier draft considered (see that migration's row below). The column is never dropped since it is the only thing that made that one-time conversion `UPDATE` safely re-runnable by hand.
...
**Notes**: `'coming_later'` ("Coming later" — a member who will attend but arrive late) was added to the CHECK constraint by migration `1790300016_rename_rsvp_maybe_to_coming_later`, which widens the constraint permissively without rewriting historical `'maybe'` rows. `coming_later` counts as full attendance everywhere `'yes'` does (roster auto-provisioning, team generation, training auto-log, player ratings, min-players headcount) and requires a non-empty `message` (enforced at the application layer, not by a CHECK constraint — see `RsvpMessageRequired` in `docs/api.md`). `'maybe'` ("Nevím" / "Not sure") is a distinct, first-class, non-attending response — RPC/HTTP read surfaces carry all four literals as-is, with no wire projection. Historical `'maybe'` rows are intentionally left exactly as stored and are never converted: before #549 the button was literally labelled "❓ Maybe", so those rows already mean "Nevím" and rewriting them would flip real answers to the opposite meaning.
...
Stored the Discord message ID of the "divider" message posted in a shared event channel to visually separate past events from upcoming ones. One row per Discord channel.
...
**Notes**: Uses an upsert (`ON CONFLICT (discord_channel_id) DO UPDATE`) so there is always at most one divider message tracked per channel. Created in migration `1744100000`. **Retired**: the shared events board (and its divider messages) was removed from code in the remove-global-events-board Release A; the bot no longer reads or writes this table. It is kept for one more release and dropped (`DROP TABLE IF EXISTS channel_event_dividers`) in the Release B follow-up migration.
...
| `bucket` | TEXT | NOT NULL, CHECK (`IN ('all','training','tournament','other')`) | `'all'` |
| `created_at` | TIMESTAMPTZ | NOT NULL | `now()` |
| `updated_at` | TIMESTAMPTZ | NOT NULL | `now()` |

**Unique**: `(team_id, team_member_id, bucket)` (widened from `(team_id, team_member_id)` in migration `1792200001_personal_event_channels_bucket`; see below)

**Notes**: Added in the personal-events rework migration. `discord_channel_id` is null until the bot creates the channel and writes back the Discord snowflake via `Guild/SavePersonalChannelId`. The provisioning worker polls `Guild/GetGuildsNeedingPersonalProvisioning` to find guilds where at least one active member has no channel, then calls `Guild/GetMembersNeedingPersonalChannel` to get the list and creates channels one at a time. A `(team_id, team_member_id, bucket)` insert is a re-claimable lease (via `Guild/ReservePersonalChannel`): `INSERT ... ON CONFLICT DO UPDATE SET updated_at = now() WHERE discord_channel_id IS NULL AND updated_at < now() - interval '15 minutes'` — a fresh row insert or a re-claim of a stale NULL reservation returns the row (`reserved = true`); a recent NULL reservation held by another in-flight worker, or an already-provisioned row, is left untouched and returns no row (`reserved = false`). This prevents a member from being permanently skipped after a worker reserves the row but crashes or fails before writing `discord_channel_id` — the next tick, more than 15 minutes later, re-claims the row and retries. `applied_channel_format` records the channel-name format template that was used when the channel was last created or renamed. When this value differs from the team's current `discord_personal_events_channel_format` setting, the provisioning worker renames the Discord channel and updates this field. Rows created before migration `1790300011` have `NULL` for this column, which is treated as drifted — those channels are renamed to the current format on the next provisioning tick. Added in migration `1790300011_add_personal_channel_applied_format`.

`bucket` added in migration `1792200001_personal_event_channels_bucket` (Nastavitelná docházka / configurable attendance) — which slice of a member's events this row's channel carries. `'all'` is the single combined channel (the only mode before this release, and still the default — every pre-existing row became `'all'` on migration); a member with `team_members.personal_channels_split = true` instead gets three rows, one per bucket: `'training'`, `'tournament'`, and `'other'`. The event→bucket mapping (`applications/server/src/repositories/personalChannelBucket.ts`, `eventBucketSql`) is `training` → `training`, `match` and `tournament` → `tournament`, everything else (including any future `event_type` value) → `other` — league matches land in the **tournaments** bucket, not **others**. The rendered channel name appends a fixed suffix per non-`'all'` bucket (`-trainings`, `-tournaments`, `-others`; `formatPersonalChannelName` in `applications/bot`), reserved from the 100-character Discord channel-name budget before the template is applied so two buckets can never truncate to the same name. Switching a member's split preference does not move channel content: the provisioning worker creates the newly-desired buckets and the de-provisioning worker deletes the no-longer-desired ones (and everything written in them) once every newly-desired bucket has a channel — see `deprovisionableBucketSql`'s gate, which refuses to tear down an obsolete channel while any desired bucket is still unprovisioned, so a stuck provisioning pass degrades to "keep the old channel" rather than leaving the member with neither. The widened unique index `uq_personal_event_channels_member_bucket` on `(team_id, team_member_id, bucket)` is created *before* the old `(team_id, team_member_id)` constraint is dropped in the same migration, so the table is never left without a uniqueness guard; because the two constraints briefly coexist under different names, an old server pod's `ON CONFLICT (team_id, team_member_id)` clause in `Guild/ReservePersonalChannel` breaks once the old constraint is dropped — the rollout deploys the server before the bot to close that window. The RPC `Guild/GetPersonalChannel` (single-channel lookup keyed on `(team_id, team_member_id)`) was deleted in the same release: with up to three rows per member it could only return an arbitrary one, and it had no remaining callers.
...
A team's catalogue of membership tiers (Slice 1 of "Setup memberships" — pricing and lifecycle only). Every team is seeded with one default plan (`name = NULL`, `currency = 'CZK'`, zero price) by migration `1792800000_create_membership_plans`, and every future team gets the same seed via an `AFTER INSERT ON teams` trigger — the same pattern as `event_types`. Slice 2 (`1792900000_membership_selection`) lets a member pick which plan they're on — see `team_members.membership_plan_id` above and `teams.membership_selection_deadline`.
...
| `recorded_by_user_id` | UUID | NOT NULL, FK → `users(id)` ON DELETE RESTRICT | — |
| `bank_transaction_id` | UUID | FK → `bank_transactions(id)` ON DELETE RESTRICT | `NULL` |
| `source` | TEXT | NOT NULL, CHECK (`'auto'`, `'manual'`) | `'manual'` |
| `voided_at` | TIMESTAMPTZ | — | `NULL` |
| `voided_by_user_id` | UUID | FK → `users(id)` ON DELETE RESTRICT | `NULL` |
| `void_reason` | TEXT | — | `NULL` |
| `created_at` | TIMESTAMPTZ | NOT NULL | `now()` |

**Constraint**: `voided_at`, `voided_by_user_id`, and `void_reason` must all be `NULL` or all non-`NULL` (atomic void state, same pattern as `payments`). `(team_member_id, currency)` FK → `member_credit_accounts(team_member_id, currency)` ON DELETE RESTRICT — a deposit cannot outlive its account. `member_credit_deposits_source_values`: `source IN ('auto', 'manual')`. `member_credit_deposits_bank_source_pair`: `(source = 'manual' AND bank_transaction_id IS NULL) OR (source = 'auto' AND bank_transaction_id IS NOT NULL)` — mirrors `payments_bank_match_pair`/`payments_matched_by_values`.

**Indexes**: `idx_member_credit_deposits_member` on `(team_member_id, currency)`; `idx_member_credit_deposits_active` on `(team_member_id, currency) WHERE voided_at IS NULL`; partial `idx_member_credit_deposits_bank_tx` on `(bank_transaction_id) WHERE bank_transaction_id IS NOT NULL`.

**Notes**: Added in migration `1792700000_create_member_credits`. `method` here is the closed `ManualPaymentMethod` (`'cash'`/`'bank_transfer'` only) — a deposit is always money the club actually received, so it is never `'credit'`. Deposits are **fungible**: nothing records which deposit funded which later fee application, so voiding one is refused (`CreditDepositSpent`, 409) once the account balance has dropped below the deposit's own amount — the caller must void the payments that spent it first. GDPR: `financial` disposition, keyed twice in `EXPORT_MANIFEST` — once on `team_members` (`team_member_id`, the member whose money this is) and once on `users` (`recorded_by_user_id`, `voided_by_user_id`). `bank_transaction_id`/`source` were added in migration `1792800001_auto_credit_bank_transfers`, mirroring `payments.bank_transaction_id`/`matched_by` (`1792000002`) — `DEFAULT 'manual'` backfills every pre-existing row correctly, no data migration needed. `source = 'auto'` rows are written exclusively by `BankTransactionMatcher`'s auto-credit path (gated on `bank_sync_config.auto_credit_enabled`) as the leftover remainder of an incoming transfer that also paid off one or more fees; `/unmatch` looks a deposit up by `bank_transaction_id` to void it (the `CreditDepositSpent`-style refusal there is `UnmatchCreditSpent`, 409) when a treasurer undoes the transaction's match.
...
As of migration `1792800001_auto_credit_bank_transfers`, `recompute_bank_match_state`'s `v_matched` sum also includes active `member_credit_deposits` rows carrying the transaction's id (in addition to active `payments`) — without this, a transfer whose remainder became credit would write fewer `payments` minor units than the movement is worth and the transaction would park at `partially_matched` (or `unmatched`) despite being fully applied. A paired trigger function, `credit_deposits_bank_recompute` (`AFTER INSERT OR UPDATE OR DELETE ON member_credit_deposits`, trigger `credit_deposits_bank_recompute_trigger`), drives the same `recompute_bank_match_state` call whenever a deposit's own `bank_transaction_id` is set — the deposit-side twin of this function's bank-transaction leg above, minus the `fee_assignments` leg (a deposit touches no assignment) and minus the re-point branch (a deposit's `bank_transaction_id` is immutable).
...
| `auto_credit_enabled` | BOOLEAN | NOT NULL | `false` |
...
| `fio_token_saved_at` | TIMESTAMPTZ | — | `NULL` |
...
**Notes**: Added in migration `1792000001_create_bank_sync_config`. `auto_credit_enabled` — added in migration `1792800001_auto_credit_bank_transfers`, opt-in, default `false`. Gates a second automatic-matching path in `BankTransactionMatcher`: when on, and the VS resolves to exactly one member with no duplicate flagged, the matcher allocates the transfer across that member's open fees oldest-due-first and writes any remainder as a `member_credit_deposits` row (`source = 'auto'`) instead of leaving the movement in the review queue. Deliberately a new column rather than reusing `auto_match_enabled` (which defaults `true` and is therefore already on for every team) — reusing it would silently retire five of the nine `match_reason` review-queue reasons for every club that never asked for it. With `auto_credit_enabled` off, the matcher's decision table is unchanged from before this column existed. `fio_token_encrypted` holds the AES-256-GCM ciphertext (`v1.<iv>.<tag>.<ct>` base64url) produced by `FioSecretCrypto` (own key, `FIO_TOKEN_ENCRYPTION_KEY` — deliberately not shared with `EmailSecretCrypto`'s `EMAIL_IMAP_ENCRYPTION_KEY`, so a compromise of one secret does not compromise the other); the plaintext is never persisted and the API never returns it (`BankSyncConfigView.fioTokenSet: boolean` is the only signal). `iban` was meant to cache Fio's own `info.iban` for a cross-check display, but nothing in the repository ever writes this column (it is read-only in `SELECT_COLUMNS`) — it is currently always `NULL`, and deliberately stays that way: the cross-check now runs live off each statement's `info.iban` (`services/bankSyncAccount.ts`), which both the probe and the poller already hold, and a stale cache would be strictly worse than the live value. `POST /teams/:teamId/bank-sync/test`'s `accountIban` response field is populated straight from a live probe's decoded statement instead, not from this column. The IBAN computed from `account_prefix`/`account_number`/`bank_code` (`CzIban.buildCzIban`) is the only authoritative value for the SPAYD `ACC` field. `fio_token_created_at` is self-reported (Fio exposes no real token metadata); `tokenExpiresAt` in API responses is simply `fio_token_created_at + 180 days`. `fio_token_saved_at`, by contrast, is server-stamped with the DB's own `now()` by `BankSyncConfigRepository`'s upsert whenever a new encrypted token is actually written (re-stamped on replace, preserved on a tokenless save) — it is the anchor for the `activating` rank in `bankSyncStatus.ts` (rule 4) and is surfaced as `BankSyncConfigView.tokenSavedAt` so the web countdown in `FioStatusBlock.tsx` measures from the same instant. `NULL` on rows written before migration `1792050000_add_fio_token_saved_at` (and on fixtures inserted straight into the table), which never report `activating` — the safe polarity, since no stamp means nothing is currently activating. `last_error_code` is free-text set by the poller/backfill on failure (`'fio_error'`, `'rate_limited'`, `'too_many_movements'`, `'history_locked'`, `'bad_request'`, `'not_configured'`, `'key_missing'`, `'coverage_gap'`, `'timeout'`, `'unreachable'` — a transport failure or unexpected non-200/500, deliberately distinct from `'fio_error'` so a transient outage alone never counts toward the `invalid` escalation in `bankSyncStatus.ts`), `'account_mismatch'` (the poller refused to ingest a statement whose `info.iban` disagrees with the IBAN computed from `account_prefix`/`account_number`/`bank_code` — written by `recordAccountMismatch`, terminal until a human edits the config, and ranked ABOVE `invalid` by `bankSyncStatus.ts` rule 2.5). `backfill_*` columns track a detached historical-import fiber that walks backwards from `backfill_from` in bounded chunks, writing `backfill_cursor` after each chunk so a crash resumes rather than restarts. `poll_leased_until`/`poll_leased_by` implement a distributed lease so only one replica polls a given team at a time (paired with a 4-minute per-team call timeout and a 6-minute lease). GDPR note: this table is entirely excluded from personal-data exports (`skip`, not `redact`, in `applications/server/src/gdpr/exportManifest.ts`) — none of it is data *about* the treasurer who last saved it, only the club's own banking configuration; team deletion cascades and removes the row.
...
| 1790300016 | `rename_rsvp_maybe_to_coming_later` | Drops and re-adds the `event_rsvps.response` CHECK constraint to permit both `'maybe'` and the new `'coming_later'` value. Historical `'maybe'` rows are left untouched; a follow-up conversion was considered but abandoned — `'maybe'` was restored as its own first-class "Nevím" response, so those rows are correct as stored and the CHECK constraint stays permissive indefinitely. |
...
| 1792050000 | `add_fio_token_saved_at` | Adds `fio_token_saved_at TIMESTAMPTZ` (nullable, `IF NOT EXISTS`) to `bank_sync_config` — see the Bank Sync section below. No backfill (deliberate — see the migration file's comment). Nothing reads this column yet. |
| 1792100000 | `series_time_is_team_local` | Converts `event_series.start_time`/`end_time` from UTC time-of-day to team-local wall-clock for every `FALSE` row, anchored at each series' own `start_date` (a correlated scalar subselect against `team_settings.timezone`, joined to `pg_timezone_names`, falling back to `'Europe/Prague'` for a missing or Postgres-unrecognised zone), and sets `times_are_team_local = TRUE` in the same guarded `UPDATE` (`WHERE NOT es.times_are_team_local`) so it is safely re-runnable by hand; `RETURNING es.id` captures exactly the series it just converted. Re-anchors already-materialized future, `active`, not-`series_modified` `events` rows, but **restricted to that just-converted set** (`es.id = ANY(convertedIds)`, not every series) — recovering each occurrence's date as `(start_at AT TIME ZONE 'UTC')::date`, the UTC date, because this statement only ever sees UTC-dialect events; `team-settings.ts`'s own re-anchor instead recovers a team-local date, because it only ever sees team-local events — the two date expressions deliberately differ. Stamps `personal_messages_dirty_at` unconditionally so Discord personal-channel messages re-render. Brackets the re-anchor with a `DROP INDEX`/`CREATE UNIQUE INDEX` of `idx_events_series_date` in the same transaction, because that non-deferrable unique index can otherwise reject a mid-statement value change even when the final state has no duplicate. Does **not** flip the column's `DEFAULT` — a final statement doing so was cut before this shipped: no reachable writer depends on the default, it would disagree with the API's decoding default (`false`), and it would create an unrecoverable rollback hazard (a `majnet deploy rollback` past this release leaves an image that has never heard of the column silently writing UTC-dialect times into rows marked `TRUE`). Originally reserved as `1791800000`; renumbered to `1792100000` because production had already applied migrations through `1792000005` by the time this id would have been used — an id at or below the highest applied one is skipped silently by the migrator (no error, no log), so the reservation moved above the waterline with headroom. |
