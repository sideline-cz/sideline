...
| 1792100000 | `series_time_is_team_local` | Converts `event_series.start_time`/`end_time` from UTC time-of-day to team-local wall-clock for every `FALSE` row, anchored at each series' own `start_date` (a correlated scalar subselect against `team_settings.timezone`, falling back to `'Europe/Prague'` for a missing or Postgres-unrecognised zone), and sets `times_are_team_local = TRUE` in the same guarded `UPDATE` so it is safely re-runnable by hand. Also re-anchors already-materialized future, `active`, not-`series_modified` `events` rows generated from just-converted series (`es.id = ANY(convertedIds)`, not every `TRUE` row) onto the corrected wall-clock time, reading each occurrence's date in the UTC dialect it was materialized under (`(e.start_at AT TIME ZONE 'UTC')::date`, not the team's zone — the two recover different populations' dates and are not interchangeable), and stamps their `personal_messages_dirty_at` so Discord personal-channel messages re-render with the fix. Brackets the re-anchor `UPDATE` with a `DROP INDEX`/`CREATE UNIQUE INDEX` of `idx_events_series_date` in the same transaction, since a forward UTC-date shift can transiently collide with a sibling row under that non-deferrable unique index. There is no third statement — see the migration's own doc comment, "Why there is no Statement C", for why the column's `DEFAULT` is deliberately left `FALSE`. Originally reserved as `1791800000`; renumbered to `1792100000` because production had already applied migrations through `1792000005` by the time this id would have been used — an id at or below the highest applied one is skipped silently by the migrator (no error, no log), so the reservation moved above the waterline with headroom. |
