...
A multi-step computation that several consumers must run identically (a rating update, a balanced-team assignment, a scoring/ranking pass) lives as a **pure algorithm module** in `src/models/` with a **paired unit test** at `packages/domain/test/<Algorithm>.test.ts`. Reference implementations: `src/models/Elo.ts` (← `test/Elo.test.ts`), `src/models/TeamGenerator.ts` (← `test/TeamGenerator.test.ts`), and the bank-sync trio `src/models/CzIban.ts` (Czech IBAN construction + the account modulo-11 checksum), `src/models/CzIco.ts` (the IČO checksum — a **different** algorithm from `CzIban`'s, with different weights and a different modulus) and `src/models/Spayd.ts` (the SPAYD v1.0 QR-payment string builder). The trio is imported by server, bot and web alike; that is the reason it lives here rather than a second copy in `applications/web/src/lib/finance/`. `src/models/SettlementPlan.ts` (`planSettlement`, ← `test/SettlementPlan.test.ts`) is the finance equivalent: the allocation of a sum across a member's outstanding fee assignments plus their credit pool, shared by the web settlement preview, `MemberCreditsRepository.settle`, and `BankTransactionMatcher.writeAutoCredit`.
...
7. **When a second server write path needs an existing path's computation but cannot reuse that path's I/O, share THIS module — never the repository method.** The pure module is the unit that is safe to share; a repository method carries its own transaction shape and `FOR UPDATE` acquisition order, and reusing it from a caller with a different order deadlocks (`40P01`). Worked example: `BankTransactionMatcher.writeAutoCredit` calls `SettlementPlan.planSettlement` directly instead of `MemberCreditsRepository.settle` — see `applications/server/AGENTS.md` → "Consistent `FOR UPDATE` Lock Ordering Within A Transaction", rule 7.
...
- **`Schema.OptionFromOptional`** for three distinct cases — decodes missing key → `Option.none()`, present value → `Option.some(decoded)`:
  ...
  3. **A field on a PUT/upsert request payload whose absence means "keep the stored value".** Web deploys LAST in the rolling order (bot → server → web), so a new server serves already-loaded web bundles for the whole rollout window; a newly REQUIRED payload field 400s every call from those bundles. Declare the new field `Schema.OptionFromOptional(...)` so an absent key decodes to `Option.none()`, and thread that `Option` unchanged through the handler into the repository — the server implements "keep stored" by `COALESCE`ing the PARAMETER against the existing column in the `DO UPDATE` clause, never against `EXCLUDED`. References: `packages/domain/src/api/EmailForwardingApi.ts` `UpsertEmailForwardingConfigRequest` (`imap_secret`, absent = keep the stored secret) and `packages/domain/src/api/BankSyncApi.ts` `UpsertBankSyncConfigRequest` (`auto_credit_enabled`). Full SQL and rollout mechanics: `applications/server/AGENTS.md` → "Wire-value projection & effective-value guards" rule 5.
...
- **Adding a new field to an existing RPC/API model with a default** — when the value must always be a plain `T` (not `Option<T>`) and older producers on the wire may omit the key, use `Schema.<Type>.pipe(Schema.withDecodingDefaultKey(() => <default>))`. Decoding a missing key fills the default; encoding still emits the field. This is the additive, wire-compatible idiom for beta.40 (there is no `Schema.optionalWith`). Precedents: `EventRpcModels.ts` `EventEmbedInfo.status` (`Schema.String.pipe(Schema.withDecodingDefaultKey(() => 'active'))`), `EventApi.ts:164` `allDay`, `GuildRpcGroup.ts:25` `is_community_enabled`. Use this instead of `OptionFromOptionalKey` when the consumer wants a concrete default rather than an `Option`. **The default must be the value that makes an old peer BEHAVE SAFELY, not the value that is most common.** `GuildRpcGroup.ts`'s `RegisterMember` verification fields are the reference for both directions: `profile_complete` defaults to `true` (an old server that omits the key must never make the bot nag a member) while `profile_gate_enabled` defaults to `false` (an omitted key must never switch a gate ON). Pick the direction by asking what a peer that does not know about the field should do, then write the default to match.
- **A read DTO exposes the RESOLVED answer, never a raw column the server's own fallback can contradict.** When the server answers a question through a single SQL expression that carries a fallback (e.g. `TeamMembersRepository.getDefaultRoleId`, whose predicate is `is_default OR (name = 'Player' AND is_built_in = true)`), the contract MUST carry that expression's OUTPUT — `RoleApi.RoleListResponse.defaultRoleId` / `.defaultRoleGrantsManage`, `RoleApi.RoleDetail.isDefaultForNewMembers` — and NOT the underlying column (`roles.is_default`). Shipping the column forces every client to re-implement the fallback, and the two copies diverge the moment the fallback changes: a team that never configured a default reports `is_default: false` on every role while it is in fact seeding built-in `Player` on every join, so the UI renders "no default configured" over a working system. Name the field for the question it answers (`isDefaultForNewMembers`), never for the column (`isDefault`), so no reader mistakes it for a mirror of the DB. Server side: `applications/server/AGENTS.md` → "The Team's Default Role Resolves In Exactly One Place".
- **Tightening a side-effect rule WITHOUT touching the schema (withhold an existing optional field)** — when a behaviour fix must narrow the condition under which a consumer performs a side effect, and the deciding value is ALREADY an optional field on the result (`Schema.OptionFromNullOr(...)` / `OptionFromOptionalKey`), express the new rule by returning `Option.none()` from the producer instead of adding a new boolean field. No `Schema.withDecodingDefaultKey`, no expand/contract release pair, and the fix takes effect the moment the producing service deploys — consumers need no redeploy and there is no rolling-deploy window in which old pods still act on the old rule. The cost is that `None` now carries two meanings (e.g. "not configured" vs. "condition not met"); choose this ONLY when no consumer needs to tell them apart. Precedent: `EventRpcModels.SubmitRsvpResult.lateRsvpChannelId` — see `applications/server/AGENTS.md` → "Before/After State Detection in Upsert Handlers" and `applications/bot/AGENTS.md` → "Late-RSVP notice".
...
- **Introducing a new value into a stored enum that can't yet go on the wire (expand/contract addition — wire-value projection)** — the inverse of the removal pattern above. When you add a new literal to a DB-backed enum (`packages/domain/src/models/<Model>.ts`) but already-deployed web/bot clients bundle a schema that only knows the old literals, you MUST NOT expose the new value on any read DTO those clients decode. Keep every wire-facing read schema pinned to the legacy literal set — define a local `const Legacy<Name> = Schema.Literals([...old...])` and use it for the DTO field — and have the server project the new stored value down to a legacy literal at the read boundary (see `applications/server/AGENTS.md` → "Wire-value projection & effective-value guards"). Only the wire DTOs stay pinned; the stored-model schema carries the full set. In the follow-up release (Release B), once every deployed consumer bundles the widened schema, add the new literal to the wire schemas and delete the projection.

  Sub-rule: when ONE consumer needs the TRUE value while the shared schema is still pinned, add a NEW additive field carrying the true value rather than un-pinning the shared one, and decode it with `Schema.OptionFromOptionalKey(<FullEnum>)` so a rolling deploy where the consumer updates before the producer tolerates the missing key as `Option.none()` instead of failing the whole decode (worked example: `UpcomingEventForUserEntry.my_response_actual`, bot-only — the bot needed the real `'coming_later'` to route which message-management buttons to render; the field completed its lifecycle and is deleted, so grep the git history, not the current schema, for it). Keep that field for one release AFTER the projection is removed — deleting it in the same change is unsafe in the reverse skew direction, because a new client reading an old server still receives the projected value on the shared field. "One release later" means the release carrying the un-pinned schema is live on EVERY deployed instance, **production included** — check the ops repo's `production.yaml` promote history, not just that the tag exists.
