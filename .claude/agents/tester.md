...
- **When the change under test adds a guard that COMPARES two values the fixtures previously set independently, align every affected fixture in the same commit.** The mismatched cases do not throw — they take the guard's branch and keep returning 200, so the failures name the subsystem instead of the fixture. Live instance: `enableBankSync`'s default account (`test/integration/bankSyncFixtures.ts`) vs `info.iban` in `validRawStatement` / `validStatement`. See the "A New Guard That COMPARES Two Fixture Values Rewrites Every Existing Case" section of `applications/server/AGENTS.md`.
